...
export const QWEN_FALLBACK_MODELS: ProviderModelsDefinition = {
  ...
  // These models exist in both Alibaba plans. The authenticated server catalog
  // replaces this degraded picker with the selected plan's complete list.
...
/**
 * Hermes has NO static client-side catalog, deliberately (B-403).
 *
 * Its models depend on the provider named in the operator's own
 * `~/.hermes/config.yaml`, so any list hard-coded here is a guess. The previous
 * one guessed `provider/model` ids that hermes does not accept in ANY form —
 * measured on 0.17.0 — and every pick died as a silent "no final response".
 * Empty is the safe fallback: the stored model sanitizes to `''`, the spawn adds
 * no `-m`, and hermes answers on its own configured pair. The real list arrives
 * from `/api/providers/hermes/models`, which reads hermes' live cache.
 */
