...
/** Epoch that supplied an authoritative direct session verdict. */
const authorityEpochs = new Map<string, number>();
/** Current WebSocket attempt. A new attempt invalidates every old verdict. */
...
export function setSessionProcessState(
  sessionId: string,
  state: string,
  options: { epoch?: number; authoritative?: boolean } = {},
): void {
  if (!sessionId || !VALID_STATES.has(state)) {
    return;
  }
  if (options.epoch !== undefined && options.epoch !== connectionEpoch) {
    return;
  }
  // A direct session frame now owns this hint. A later presence snapshot that
  // never listed this mirror-only run must not prune it by absence.
  presenceOwned.delete(sessionId);
  let changed = false;
  // Local callers historically omitted options; those writes are explicit
  // verdicts too. Callers that are only seeding a tentative hint must opt out.
  const authoritative = options.authoritative !== false;
  if (authoritative && authorityEpochs.get(sessionId) !== connectionEpoch) {
    authorityEpochs.set(sessionId, connectionEpoch);
    changed = true;
  }
  if (state === 'idle') {
...
/** Starts a socket attempt: keep the last live hint, invalidate all old authority. */
export function beginSessionProcessConnectionEpoch(): number {
  connectionEpoch = ++epochSequence;
  if (authorityEpochs.size === 0) return connectionEpoch;
  authorityEpochs.clear();
  emitChange();
  return connectionEpoch;
}

/** A close makes this attempt's direct verdicts stale before backoff reconnects. */
export function invalidateSessionProcessAuthority(epoch: number): void {
  if (epoch !== connectionEpoch || authorityEpochs.size === 0) return;
  authorityEpochs.clear();
  emitChange();
}
...
export function reconcilePresenceProcessStates(
  ...
  for (const { sessionId, state } of entries) {
    ...
    if (authorityEpochs.get(sessionId) !== connectionEpoch) {
      authorityEpochs.set(sessionId, connectionEpoch);
      changed = true;
    }
    if (states.get(sessionId) !== state) {
...
/** True only after a direct verdict for this session in the current socket epoch. */
export function isSessionProcessStateAuthoritative(sessionId?: string | null): boolean {
  return Boolean(sessionId) && authorityEpochs.get(sessionId as string) === connectionEpoch;
}
...
/** Reactive authority bit paired with `useSessionProcessState`. */
export function useSessionProcessStateAuthority(sessionId?: string | null): boolean {
  return useSyncExternalStore(
    subscribe,
    () => isSessionProcessStateAuthoritative(sessionId),
  );
}
 ...
 * count — a paused process is not "working"; it has its own rollup below.
 */
export function useAnySessionProcessing(
  sessionIds: ReadonlyArray<string | null | undefined>,
): boolean {
  return useSyncExternalStore(subscribe, () =>
    sessionIds.some((id) => Boolean(id)
      && states.get(id as string) === 'running'
      && authorityEpochs.get(id as string) === connectionEpoch),
  );
}

/**
 * Project-level rollup for frozen sessions (B-824).
 *
 * Without it the project dot was structurally blind to `kill -STOP`: a project
 * whose only live session is frozen showed NOTHING beside its name while the
 * session's own row showed amber. A frozen run is not "working", but it is the
 * one state that never ends on its own — it is exactly what a rollup is for.
 */
export function useAnySessionFrozen(
  sessionIds: ReadonlyArray<string | null | undefined>,
): boolean {
  return useSyncExternalStore(subscribe, () =>
    sessionIds.some((id) => Boolean(id)
      && states.get(id as string) === 'frozen'
      && authorityEpochs.get(id as string) === connectionEpoch),
  );
}
