...
test('presence omission neither clears nor grants authority to a mirror-only session', () => {
  const epoch = beginSessionProcessConnectionEpoch();
  setSessionProcessState('mirror-only', 'running', { epoch, authoritative: true });

  reconcilePresenceProcessStates([{ sessionId: 'a', state: 'running' }]);
  reconcilePresenceProcessStates([]);

  assert.equal(getSessionProcessState('mirror-only'), 'running');
  assert.equal(isSessionProcessStateAuthoritative('mirror-only'), true);
  assert.equal(isSessionProcessStateAuthoritative('a'), false);
});

test('a direct mirror frame takes ownership before a later presence omission', () => {
  reconcilePresenceProcessStates([{ sessionId: 'mirror-only', state: 'running' }]);
  const epoch = beginSessionProcessConnectionEpoch();
  setSessionProcessState('mirror-only', 'frozen', { epoch, authoritative: true });

  reconcilePresenceProcessStates([]);

  assert.equal(getSessionProcessState('mirror-only'), 'frozen');
  assert.equal(isSessionProcessStateAuthoritative('mirror-only'), true);
});
...
test('identity reset clears mirror-owned and presence-owned states together', () => {
  setSessionProcessState('mirror-only', 'running');
  reconcilePresenceProcessStates([{ sessionId: 'a', state: 'frozen' }]);

  resetSessionProcessStates();

  assert.equal(getSessionProcessState('mirror-only'), null);
  assert.equal(getSessionProcessState('a'), null);
});
