...
export type WorkflowEnvelope = {
  ...
  capped: boolean;
  /**
   * Workflows withheld from every list in this store because they are dormant
   * (silent past the server's threshold — see DORMANT_AFTER_MS server-side).
   * Kept as a counter so a future "show hidden" control has something honest to
   * hang off, and so the hiding is never silent.
   */
  dormant: number;
};
...
function workflowSig(w: ActiveWorkflow): string {
  ...
  // agent racking up tool calls changes nothing else in this string, and the
  // store would suppress the update as "identical snapshot". The strip would then
  // freeze on the first sample it ever received.
...
function computeSnapshotSig(next: Map<string, readonly ActiveWorkflow[]>, env: WorkflowEnvelope): string {
  const parts: string[] = [
    `e:${env.eligible},${env.scanned},${env.capped ? 1 : 0},${env.dormant}`,
  ];
...
export function setActiveWorkflows(
  ...
  for (const w of result.workflows) {
    ...
    // DORMANT rows are dropped HERE, at ingest, and nowhere else. Filtering in
    // the selectors instead would build a fresh array on every snapshot read,
    // and `useSyncExternalStore` re-renders forever when the snapshot reference
    // changes on each call. Dropping once keeps every downstream array reference
    // stable, so the badge, the strip and the card all go quiet together.
    //
    // The COUNT survives the drop (envelope.dormant): "quiet because nothing is
    // running" must stay distinguishable from "quiet because N were hidden".
    if (w.dormant) {
      dormantSeen += 1;
      continue;
    }
  ...
  const nextEnvelope: WorkflowEnvelope = {
    ...
    // Trust the server's count when it sent one; fall back to what we filtered
    // ourselves so an older server (no `dormant` field) is not reported as 0.
    dormant: Number.isFinite(result.dormant) ? result.dormant : dormantSeen,
  };
  ...
  if (
    ...
    envelope.dormant !== nextEnvelope.dormant
  ) {
