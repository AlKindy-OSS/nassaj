...
/** Aggregate queue evidence; a blocked attempt takes priority over pending rows. */
export type LiveActionStatus = PublicAction['status'] | 'execution_unresolved';

function actionStatusOf(actions: PublicAction[], actionType: string): LiveActionStatus | null {
  const matches = actions.filter(action => action.actionType === actionType);
  if (matches.some(action => action.reasonCode === 'execution_unresolved' || action.retryable === false)) {
    return 'execution_unresolved';
  }
  if (matches.some(action => action.status === 'executing')) return 'executing';
...
export function useServerActionCatalog() {
  ...
  const liveStatusOf = useCallback(
    (actionType: string): LiveActionStatus | null => actionStatusOf(liveActions, actionType),
    [liveActions],
  );
  ...
  const runAction = useCallback(
    async (actionType: string, opts?: RunActionOpts): Promise<ExecuteOutcome> => {
      const liveStatus = actionStatusOf(_liveActions, actionType);
      if (liveStatus === 'execution_unresolved') return { status: 'error', code: 'execution_unresolved' };
      if (liveStatus === 'executing') return { status: 'error', code: 'action_in_flight' };
      try {
        const res = await (
        ...
        if (!res.ok) {
          return { status: 'error', code: String(data.code ?? 'outcome_unverified') };
        }
        ...
        // A lost response does not prove whether execution started.
        return { status: 'error', code: 'outcome_unverified' };
      }
    },
    [],
