...
import { isSessionListedInProjects, isUpdateAdditive, mergeDeepLinkedSession } from './useProjectsState';
...
describe('direct session URLs outside the first sidebar page', () => {
  it('materialises only the resolved conversation in its owning project', () => {
    const original = project([session('s-newest')]);
    const untouched = { ...project([session('s-other')]), projectId: 'p-2' };
    const result = mergeDeepLinkedSession([original, untouched], {
      projectId: PROJECT_ID,
      provider: 'claude',
      session: session('s-older'),
    });

    expect(result[0]?.sessions?.map((item) => item.id)).toEqual(['s-newest', 's-older']);
    expect(result[1]).toBe(untouched);
  });

  it('does not alter the sidebar for an unknown provider', () => {
    const original = [project([session('s-newest')])];
    expect(mergeDeepLinkedSession(original, {
      projectId: PROJECT_ID,
