 ...
 *
 * Uses a REAL SVG extracted from whichever shipped wiki page carries one, to
 * avoid the false confidence of synthetic fixtures.
...
import { collapseHtmlBlocks, RAW_BY_FILE } from './wikiContent';
...
// Scan the shipped pages for the first inline diagram rather than naming one
// file. The fixture stays real (see the synthetic-fixtures lesson above) but the
// suite no longer breaks when the content index is reorganised — which it was,
// and which used to turn a content edit into a red test in code.
const realSvg = ((): string | null => {
  for (const raw of Object.values(RAW_BY_FILE)) {
    const svg = extractFirstSvg(raw);
    if (svg?.includes('wiki-diagram')) return svg;
  }
  return null;
})();
...
describe('collapseHtmlBlocks — real SVG from the shipped wiki pages', () => {
  it('extracts a real SVG from a shipped page (fixture is not synthetic)', () => {
