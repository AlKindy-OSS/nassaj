...
export function useGitPanelController({
  ...
  const workspaceGeneration = selectedSessionId
    ? readSessionWorkspaceGeneration(selectedSessionId)
    : null;
  const missingWorkspaceBindingError = selectedSessionId && !workspaceGeneration
    ? 'This legacy session has no isolated workspace binding, so its Git state cannot be read or committed safely.'
    : null;
  ...
  const fetchFileDiff = useCallback(
    async (filePath: string, signal?: AbortSignal) => {
      ...
      if (missingWorkspaceBindingError) return;

      // Git endpoints receive the DB projectId via the `project` query param.
      const projectId = selectedProject.projectId;
      const params = new URLSearchParams({ project: projectId, file: filePath });
      if (selectedSessionId && workspaceGeneration) {
        params.set('sessionId', selectedSessionId);
        params.set('generation', workspaceGeneration);
      }
    ...
    [missingWorkspaceBindingError, selectedProject, selectedSessionId, workspaceGeneration],
  );
  ...
  const fetchGitStatus = useCallback(async (signal?: AbortSignal) => {
    ...
    if (missingWorkspaceBindingError) {
      setOperationError(missingWorkspaceBindingError);
      setGitStatus({ error: missingWorkspaceBindingError });
      setGitDiff({});
      setCurrentBranch('');
      setIsLoading(false);
      return;
    ...
    setIsLoading(true);
    try {
      const response = await fetchWithAuth(`/api/git/status?${params.toString()}`, { signal });
  ...
  }, [fetchFileDiff, missingWorkspaceBindingError, selectedProject, selectedSessionId, workspaceGeneration]);
  ...
  const commitChanges = useCallback(
    async (message: string, files: string[]) => {
      ...
      try {
        ...
        const response = await fetchWithAuth(
          generation ? '/api/git/commit-session-overlay' : '/api/git/commit',
          {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    [fetchGitStatus, fetchRemoteStatus, missingWorkspaceBindingError, selectedProject, selectedSessionId, workspaceGeneration],
  );
