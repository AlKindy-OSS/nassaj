            ...
            >
              {isCreatingInitialCommit ? (
                <>
                  ...
                  <span>{t('git.changes.creatingInitialCommit')}</span>
                </>
              ) : (
                <>
                  <GitCommit className="h-4 w-4" />
                  <span>{t('git.changes.createInitialCommit')}</span>
                </>
              )}
            </button>
          </div>
          ...
          <div className={isMobile ? 'pb-4' : ''}>
            ...
            <div className="flex items-center justify-between border-b border-border/60 bg-muted/30 px-3 py-1.5">
              <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                {t('git.changes.staged', { count: selectedFiles.size })}
              </span>
            ...
            <div className="flex items-center justify-between border-b border-border/60 bg-muted/30 px-3 py-1.5">
              <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
                {t('git.changes.changed', { count: unstagedFiles.size })}
              </span>
            ...
            {unstagedFiles.size === 0 ? (
              <div className="px-3 py-2 text-xs text-muted-foreground italic">{t('git.changes.allStaged')}</div>
            ) : (
              <FileChangeList
                gitStatus={gitStatus}
                gitDiff={gitDiff}
