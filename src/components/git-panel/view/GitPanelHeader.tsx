...
export default function GitPanelHeader({
  ...
  return (
    <>
      ...
      <div className={`app-top-rail flex shrink-0 items-center justify-between ${isMobile ? 'px-3' : 'px-4'}`}>
        {/* Branch selector */}
        <div className="relative" ref={dropdownRef}>
          <button
            onClick={() => setShowBranchDropdown((prev) => !prev)}
            className={`flex items-center rounded-lg transition-colors hover:bg-accent ${isMobile ? 'gap-1 px-2 py-1' : 'gap-2 px-3 py-1.5'}`}
          >
            ...
            <div className="absolute start-0 top-full z-50 mt-1 w-64 overflow-hidden rounded-xl border border-border bg-card shadow-lg">
              ...
              <div className="border-t border-border py-1">
                <button
                  ...
                  className="flex w-full items-center gap-2 px-4 py-2 text-start text-sm transition-colors hover:bg-accent"
                >
                  <Plus className="h-3 w-3" />
                  <span>{t('git.header.createBranch')}</span>
                </button>
              </div>
            </div>
          )}
        ...
        <div className={`flex items-center ${isMobile ? 'gap-1' : 'gap-2'}`}>
          ...
          <button
            ...
            title={t('git.header.revertLatest')}
          >
            <RotateCcw
