...
type UseTerminalConnectionResult = {
  ...
  reconnect: () => void;
  /** Moves keyboard input to xterm's hidden textarea. */
  focus: () => void;
};
...
export function useTerminalConnection({
  ...
  useEffect(() => {
    ...
    // Keep xterm's default DOM renderer: the Arabic character joiner below
    // makes each Arabic run one DOM-rendered unit, supplying contextual
    // shaping/bidi. WebGL would regress to isolated glyphs.
    term.open(terminalContainerRef.current);
    const disableArabicSupport = enableXtermArabic(term);
      ...
      socket.onclose = (event) => {
        ...
        switch (classifyTerminalClose(event.code)) {
          ...
          case 'forbidden':
            ...
            // then closes with 4403, so wiping here erases the only reason the
            // user gets and lands them on a blank error state — the mirror of
            // the black-pane defect fixed in the shell hook.
            setState('error');
            return;
          case 'serverRestart':
