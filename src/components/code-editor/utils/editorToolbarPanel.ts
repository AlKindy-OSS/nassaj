...
export const createEditorToolbarPanelExtension = ({
  ...
  const createPanel = (view: EditorView) => {
    ...
    let currentIndex = 0;
    // Bug 10 (T-10): CodeMirror calls `update` on virtually every editor
    // transaction (each keystroke), but `showDiff`/`isExpanded` are captured
    // once at panel-creation time and never change for this panel instance's
    // lifetime — a NEW panel is created upstream instead when those props
    // change. So the only thing that can legitimately differ between calls is
    // the diff-chunk count/position. Track the last rendered values and skip
    // the innerHTML rebuild + listener rebinding entirely when neither moved.
    ...
    const updatePanel = () => {
      ...
      if (
        hasRenderedOnce
        && chunkCount === lastRenderedChunkCount
        && currentIndex === lastRenderedIndex
      ) {
        return;
      }
      hasRenderedOnce = true;
      lastRenderedChunkCount = chunkCount;
