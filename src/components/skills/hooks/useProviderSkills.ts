...
const deleteProviderSkill = async (
  ...
  const response = await authenticatedFetch(
    `/api/providers/${provider}/skills/${encodeURIComponent(skillName)}`,
    { method: 'DELETE' },
  );

  if (response.status === 403) {
    throw Object.assign(
      new Error('You do not have permission to delete skills.'),
      { code: 'FORBIDDEN' },
    );
  }
  ...
  if (response.status === 404) {
    throw Object.assign(
      new Error('Skill not found — it may have already been removed.'),
      { code: 'NOT_FOUND' },
    );
  }
...
export function useProviderSkills({ selectedProvider, currentProjects }: UseProviderSkillsArgs) {
  ...
  const deleteSkill = useCallback(async (skillName: string) => {
    await deleteProviderSkill(selectedProvider, skillName);
    clearProviderSkillCache(selectedProvider);
    await refreshSkills({ force: true });
  }, [refreshSkills, selectedProvider]);
