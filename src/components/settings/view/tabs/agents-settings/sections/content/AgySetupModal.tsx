      ...
      <div className="flex h-3/4 w-full max-w-4xl flex-col overflow-hidden rounded-lg bg-card shadow-xl max-md:m-0 max-md:h-full max-md:max-w-none max-md:rounded-none md:m-4">
        ...
        <div className="flex flex-shrink-0 items-center justify-between border-b border-border p-4">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-md text-muted-foreground transition-colors duration-150 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        ...
        {/* Instructions — سطحٌ واحد لا سطحان: كان `bg-muted/30` يحمل `border-b`
            بنفس صياغة حدّ الرأس فوقه (خطّان متلاصقان)، وداخله `code` بـ`bg-muted`
            (سطحٌ داخل مثله). الحدّ سقط، والسطح صار `bg-muted` كاملاً، والقيمة
            التقنية تُميَّز بخطّها الأحادي واتجاهها المعزول. */}
        <div className="flex-shrink-0 bg-muted px-4 py-3">
          <p className="text-[13px] leading-relaxed text-muted-foreground">
            {t('agyConnection.modal.instructions')}{' '}
            <code dir="ltr" style={{ unicodeBidi: 'isolate' }} className="font-mono text-[13px] text-foreground">
              {AGY_LOGIN_COMMAND}
            </code>
            {'. '}
