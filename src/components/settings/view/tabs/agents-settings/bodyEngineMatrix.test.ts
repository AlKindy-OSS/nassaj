...
describe('B-343 — a key box only where the body actually reads that slot', () => {
  it('sends the Claude body to the AES store, keyed by the engine', () => {
    expect(engineKeySlot('claude', 'glm')).toEqual({ provider: 'glm' });
    expect(engineKeySlot('claude', 'kimi')).toEqual({ provider: 'kimi' });
  });

  it('sends opencode GLM to opencode\'s own credential target, not the AES store', () => {
    // The carrier reads auth.json (opencode-config-material.js); a key written to
    // the AES record under `glm` would be stored, reported "configured", and
    // never read by anything — the exact "I entered the key and nothing changed"
    // failure this mapping exists to prevent.
    expect(engineKeySlot('opencode', 'glm')).toEqual({ provider: 'opencode', target: 'glm' });
  });

  it('offers no box for a pair whose body reads no slot at all', () => {
    // opencode has no kimi/deepseek credential target, and the kimi body is
    // handed only its OWN vendor key at spawn (resolve-provider-env.js).
    expect(engineKeySlot('opencode', 'kimi')).toBeNull();
    expect(engineKeySlot('opencode', 'deepseek')).toBeNull();
    expect(engineKeySlot('kimi', 'deepseek')).toBeNull();
    // And the axes that are not keys at all stay empty for both bodies.
    expect(engineKeySlot('claude', 'anthropic')).toBeNull();
    expect(engineKeySlot('opencode', 'zen')).toBeNull();
  });
});
