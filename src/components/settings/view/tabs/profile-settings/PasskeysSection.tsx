    ...
    >
      <div className="space-y-4 py-2">
        {isSupported ? (
          ...
          /* متصفّحٌ لا يدعم WebAuthn = **تنبيهٌ لا يمنع بقيّة الشاشة**، فصندوق
             نبرة `warning` لا نصٌّ رمادي يُقرأ وصفاً عابراً بينما هو سبب غياب
             زرٍّ بأكمله. */
        ...
        {/* النموذج بلا وعاء: حقلٌ واحد وزرّان، وحدّ الحقل نفسه يحدّه (§1). */}
        ...
        {/* التحميل والفراغ بنمطٍ واحد في كل نطاق هذه الجولة: نصٌّ رمادي
            متمركز بحشوٍ رأسي — وهو نمط الأصل حرفياً («‏No allowed tools
            configured» متمركزةً في فراغ اللوحة). كانت الصيغتان هنا مختلفتين
            (‏`text-sm` محاذٍ للبداية مقابل `text-[13px]`) وثالثةٌ في كل ملف
            آخر. */}
        {isLoading ? (
          ...
          // كل مفتاح مرور كان صندوقاً مؤطَّراً بذاته — نفس صفّ القائمة المنسوخ
          // في مفاتيح API واعتمادات GitHub. قائمة صفوف بلا صندوق (§2.2)، بحشوٍ
          // رأسي فقط لتحاذي الصفوفُ عنوان القسم فوقها (§2.3).
          <SettingsGroup>
                  ...
                  <div className="min-w-0">
                    {renamingId === credential.id ? (
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {/* نبرة واحدة: «مزامَن» و«مرتبط بالجهاز» كانا يتمايزان
                            بلون الشارة (أساسية مقابل ثانوية) بينما النصّ يقول
                            الفرق أصلاً — تمييزٌ محمول على اللون وحده (§2.8). */}
                        <StatusBadge>
                  ...
                  {confirmingDeleteId === credential.id ? (
                    ...
                    <>
                      ...
                      <Button
                        ...
                        size="icon"
                        variant="ghost"
                        className="w-9 p-0 text-danger hover:text-danger"
                        onClick={() => {
                          setRenamingId(null);
