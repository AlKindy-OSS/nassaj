...
import { Check, CircleUserRound, Loader2, UserRound } from 'lucide-react';
...
export default function AvatarIdentitySection({ t }: Props) {
  ...
  // منتقٍ مجزّأ لا شريط تبويبات: الأزرار كانت `role="tab"` بلا `role="tabpanel"`
  // ولا `aria-controls` — نمطٌ ناقص يَعِد قارئ الشاشة بلوحاتٍ لا وجود لها. وهي
  // في حقيقتها اختيار وضعٍ واحد من ثلاثة، أي `radiogroup` (§2.4).
  const modeOptions = useMemo<readonly SegmentedOption<IdentityMode>[]>(
    () => [
      { value: 'upload', label: t('profile.identity.tabs.upload') },
      { value: 'gallery', label: t('profile.identity.tabs.gallery') },
      { value: 'color', label: t('profile.identity.tabs.color') },
    ],
    [t],
  );
  ...
  return (
    ...
    >
      {/* لوحٌ داخلي: المعاينة والحصيلة ابنان متجاوران، وبطاقة القسم لا تباعد
          بين أبنائها المباشرين. */}
      <div className="space-y-3 py-2">
      <div className="flex items-start gap-4">
        {/* Live preview */}
        {showImagePreview ? (
          <img
            src={staticAssetUrl(currentAvatarUrl)}
        ...
        <div className="min-w-0 flex-1 space-y-3">
          <SegmentedControl
            options={modeOptions}
            value={mode}
            label={t('profile.identity.title')}
            onChange={(next) => {
              setMode(next);
              setFeedback(null);
            }}
          ...
          {mode === 'gallery' && (
            <div className="space-y-2">
              {/* لصيقةُ مجموعةِ اختيارٍ لا عنوانُ قسم: مقاسها من سلّم اللصائق
                  (`text-[15px] font-medium`) بدل `text-sm` الذي لا موضع له في
                  السلّم. ولا تُرفَع إلى `SettingsSection`: قسمٌ بـ`text-lg`
                  وأيقونة داخل لوحِ وضعٍ من ثلاثة يُنشئ رأساً أكبر من رأس القسم
                  الذي يحويه. */}
                ...
                {GALLERY_AVATARS.map((avatar) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'relative aspect-square overflow-hidden rounded-full ring-2 ring-offset-2 ring-offset-background transition focus-visible:outline-none focus-visible:ring-ring disabled:opacity-60',
                        selected
                          ? 'ring-primary'
                          : 'ring-transparent hover:ring-border hover:scale-105',
                      )}
                    >
                      <img src={staticAssetUrl(avatar.url)} alt="" className="h-full w-full object-cover" />
                      {selected && (
                ...
                {AVATAR_COLORS.map((color) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'relative flex h-9 w-9 items-center justify-center rounded-full text-sm font-semibold text-white ring-2 transition focus-visible:outline-none focus-visible:ring-ring disabled:opacity-60',
                        color.className,
