...
export function useSettingsController({ isOpen, initialTab }: UseSettingsControllerArgs) {
  ...
  useEffect(() => {
    ...
    // Connectors joins the open-burst instead of fetching when its own tab is
    // clicked. Measured: everything else landed at 2887ms and Connectors started
    // at 3635ms, which is the whole reason that one tab showed a spinner.
    // Cheap to call unconditionally — the store returns early once loaded.
    void loadConnectors();
