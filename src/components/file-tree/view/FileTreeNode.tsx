...
export default function FileTreeNode({
  ...
  // Depth is expressed with logical properties throughout (`border-s`, and the
  // `paddingInlineStart` below) so the hierarchy indent grows away from the same
  // edge the text starts at — under RTL a physical `border-l`/`paddingLeft` would
  // drift left while the name and icons sit right.
  const rowClassName = cn(
    viewMode === 'detailed'
      ? 'group grid grid-cols-12 gap-2 py-[3px] pe-2 hover:bg-accent/60 cursor-pointer items-center rounded-sm transition-colors duration-100'
      : viewMode === 'compact'
      ? 'group flex items-center justify-between py-[3px] pe-2 hover:bg-accent/60 cursor-pointer rounded-sm transition-colors duration-100'
      : 'group flex items-center gap-1.5 py-[3px] pe-2 cursor-pointer rounded-sm hover:bg-accent/60 transition-colors duration-100',
    isDirectory && isOpen && 'border-s-2 border-primary/30',
    (isDirectory && !isOpen) || !isDirectory ? 'border-s-2 border-transparent' : '',
  );

  // Indent for this depth level, applied on the inline-start edge.
  const indentStyle = { paddingInlineStart: `${level * 16 + 4}px` };
    ...
    <div className="select-none">
      ...
      {isDirectory && isOpen && hasChildren && (
        <div className="relative">
          <span
            className="absolute bottom-0 top-0 border-s border-border/40"
            style={{ insetInlineStart: `${level * 16 + 14}px` }}
            aria-hidden="true"
          />
