...
const saveGlobalServer = async (
  ...
  if (!response.ok || !data.success) {
    throw new Error(getApiErrorMessage(data, 'Failed to save MCP server to compatible providers'));
  }
    ...
    async (formData: McpFormState) => {
      const payload = createMcpPayloadFromForm(selectedProvider, formData, {
        ...
        unsupportedTransportMessage: (transport) =>
          `Add MCP Server supports only stdio and http for compatible providers, not ${transport}.`,
      });

      if (payload.scope === 'local') {
        throw new Error('Add MCP Server supports only user or project scope for compatible providers.');
      }
      ...
      if (failures.length > 0) {
        setSaveStatus('error');
        throw new Error(`Failed to add MCP server to one or more compatible providers. ${formatGlobalAddFailures(failures)}`);
      }
