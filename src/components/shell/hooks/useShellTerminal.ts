...
export function useShellTerminal({
  ...
  useEffect(() => {
    ...
    // Intentionally NOT loading WebglAddon. The WebGL renderer rasterises every
    // cell independently, so each Arabic letter is drawn in its ISOLATED glyph
    // form with no contextual shaping and no bidi — Arabic comes out as
    // disconnected, reversed letters (e.g. «الأعوان» → «ناوعألا»). xterm's
    // default DOM renderer instead hands each same-style run to the browser,
    // which shapes (joins) the Arabic and applies bidi within the run. This is a
    // deliberate GPU-speed-for-Arabic-correctness trade-off, acceptable for an
    // Arabic-first tool; xterm still forces an LTR cell grid, so TUI/box output
    // stays aligned. Verified in a real browser (WebGL vs DOM) on real Arabic.
