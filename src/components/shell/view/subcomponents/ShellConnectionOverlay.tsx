// design-ok-file: طبقة فوق سطح الطرفية — تتبع لونه لا لون الثيم.
...
export default function ShellConnectionOverlay({
  ...
  // Shared spinner overlay for both the first `connecting` and an auto
  // `reconnecting` after an abnormal drop — only the label differs.
  return (
    <div className="absolute inset-0 z-20 flex items-center justify-center bg-gray-950/90 p-6">
      <div className="flex w-full max-w-md flex-col items-center gap-3 text-center">
        <div className="flex items-center justify-center gap-3 text-yellow-300" role="status">
          <Loader2 className="h-5 w-5 animate-spin" aria-hidden="true" />
          <span className="text-base font-medium">
            {mode === 'reconnecting' ? reconnectingLabel : connectingLabel}
          </span>
        </div>
        <p className="max-w-md break-words px-2 text-sm leading-6 text-gray-300">{description}</p>
      </div>
