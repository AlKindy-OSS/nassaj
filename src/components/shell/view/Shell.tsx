// design-ok-file: سطح الطرفية داكن في الوضعين — لوحة xterm لها ثيمها الخاص، وتلوينها بأصناف السطح يفصلها عن محتواها.
...
type ShellProps = {
  ...
  /** Server refusal frames (`type: 'error'`), and `null` when one is
   *  superseded. Not a process exit. */
...
export default function Shell({
  ...
  // `reconnecting` outranks `connect`/`connecting`: an in-flight auto re-attach
  // should show its own affordance, never the manual connect button.
      ...
      <div className="relative flex-1 overflow-hidden p-2">
        ...
        {overlayMode && (
          <ShellConnectionOverlay
            ...
            reconnectingLabel={t('shell.reconnecting')}
