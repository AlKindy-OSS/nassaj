...
const SIZE_CLASSES: Record<SizeToken, string> = {
  ...
  // 32px avatars (settings user list rows).
  md: 'h-8 w-8 text-xs',
};
...
export default function ParticipantAvatar({
  ...
  return (
    <Tooltip content={tooltipContent ?? defaultTooltipContent} position="bottom" multiline keyboard interactive>
      <span
        role="img"
        ...
        className={cn(
          ...
          'relative inline-flex aspect-square flex-shrink-0 select-none items-center justify-center overflow-hidden rounded-full font-semibold leading-none text-white align-middle',
          SIZE_CLASSES[size],
          // القرص الملوّن للأحرف الأولى وحدها. كان يُرسم دائماً ليظهر خلف صور
          // المعرض الشفافة (DiceBear `fill="none"`)، فبدا لوناً غريباً محيطاً
          // بالصورة في صفوف الشريط (قرار المالك). الصورة الآن على خلفية شفافة.
          !showImage && (chosenColorClass ?? avatarColorForUser(participant.userId)),
          // لا حلقة حول الصورة: `ring-2 ring-background` كانت تُقرأ ظلاً باهتاً
          // خلف الصورة في صفوف الشريط (قرار المالك). الإزاحة المتداخلة (-ms-2)
          // تبقى على حاوية ParticipantAvatarStack عبر [&>*+*]:-ms-2.
        )}
      >
        {showImage ? (
          <img
            src={staticAssetUrl(avatarUrl)}
            alt=""
