...
export type SessionAgent = {
  ...
  /** T-1144: wire fingerprint of the provider that answered with this model
   * ('anthropic' | 'moonshot' | 'zai' | 'deepseek' | 'openai-compatible' |
   * 'unknown'), derived server-side from the response envelope — the model id
   * string alone can be ambiguous. Null for subagents and stale cache rows. */
