import type { ParticipantRole, SessionAgent } from './types';

/** Minimum shape `summarizeAgents` needs — keeps it testable without a full row. */
export type SessionAgentLike = Pick<SessionAgent, 'agent_kind' | 'invocation_count'> &
  Partial<SessionAgent>;

export type AgentsSummary = {
  /** Models in first-seen order. */
  models: SessionAgentLike[];
  /** Subagents by invocation count, descending. */
  subagents: SessionAgentLike[];
  /** The model answering now = the last to have appeared. */
  current: SessionAgentLike | null;
  /** Models that spoke before the current one — a switch counter, not a roster size. */
  olderModels: number;
...
/**
 * Splits a roster into its two axes and counts each one SEPARATELY (B-410).
 *
 * The header used to print `subagents.length + olderModels` as a single "+N",
 * which merged two unrelated events: a model switch (a possible engine leak —
 * the very thing B-352/T-1144 exist to surface) and delegation volume. One
 * number could not signal either.
 *
 * Models keep first-seen order — a conversation that moved kimi-k3 → opus-5
 * reads as its own history, and the model answering NOW is the last one.
 * Subagents sort by invocation count, which is how they are read.
 */
export function summarizeAgents(agents: SessionAgentLike[]): AgentsSummary {
  const models = agents.filter((a) => a.agent_kind === 'model');
  const subagents = agents
    .filter((a) => a.agent_kind === 'subagent')
    .sort((a, b) => b.invocation_count - a.invocation_count);

  return {
    models,
    subagents,
    current: models.length > 0 ? models[models.length - 1] : null,
    olderModels: Math.max(0, models.length - 1),
    distinctSubagents: subagents.length,
    totalInvocations: subagents.reduce((sum, a) => sum + a.invocation_count, 0),
  };
}

/**
 * T-1144: display name for an agent's wire-fingerprinted provider. Proper
 * nouns, deliberately NOT i18n keys — "Moonshot" reads the same in Arabic.
 * Mirrors server/services/provider-fingerprint.js PROVIDER_DISPLAY_NAME.
 */
export const PROVIDER_DISPLAY_NAME: Readonly<Record<string, string>> = Object.freeze({
  anthropic: 'Anthropic',
  moonshot: 'Moonshot',
  zai: 'Z.AI',
  deepseek: 'DeepSeek',
