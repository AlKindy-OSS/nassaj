...
/**
 * One live run the recipient may see, carried in every presence snapshot
 * (B-269). Lets any client learn which conversations are busy without having
 * opened them — the `process_state` stream only reaches a session's primary
 * socket and its registered mirrors.
 */
...
/**
 * Narrows the raw `runningSessions` field of a presence snapshot (B-269).
 * Returns [] for an absent/malformed field — an older server that does not send
 * it yields an empty list, which reconciliation treats as "presence owns
 * nothing", leaving the legacy mirror-fed behaviour exactly as it was.
 */
export function parseRunningSessions(raw: unknown): RunningSession[] {
  if (!Array.isArray(raw)) {
    return [];
  }
  const parsed: RunningSession[] = [];
...
/**
 * Last snapshot seen by ANY mounted consumer, kept at module scope (B-332).
 *
 * The server broadcasts `presence` on change only — never on a timer — so a
 * consumer that unmounts and remounts (switching to the Terminals section and
 * back) would otherwise start from an empty list and render nothing until some
 * brother happens to connect or disconnect. Seeding the state from this cache
 * makes a remount instant; the next real snapshot still overwrites it.
 */
let lastSnapshot: UsePresenceResult = { users: [], activeConversations: null };
...
export function usePresence(): UsePresenceResult {
  ...
  const [users, setUsers] = useState<PresenceUser[]>(() => lastSnapshot.users);
  const [activeConversations, setActiveConversations] = useState<ActiveConversations | null>(
    () => lastSnapshot.activeConversations,
  );
  ...
  useEffect(() => {
    ...
    lastSnapshot = { users: parsed, activeConversations: parsedConversations };
  }, [latestMessage]);
