...
export function ActiveConversationsMenu({
  ...
  const updatePosition = useCallback(() => {
    ...
    if (placement === 'bottom') {
      ...
      // DOMRect coordinates are physical.  Align to logical inline-start, then
      // write the resulting physical `left`: assigning a physical value to
      // `insetInlineStart` made RTL reinterpret it as an offset from the right.
      const rtl = document.documentElement.dir === 'rtl';
      const preferredLeft = rtl ? rect.right - popoverW : rect.left;
      const left = Math.max(spacing, Math.min(preferredLeft, vw - popoverW - spacing));
    ...
    // 'inline-end': open toward the logical inline-end of the rail, vertically
    // centred.  Coordinates stay physical after this direction-aware choice.
    const popoverW = popoverRef.current?.offsetWidth || 288;
    const rtl = document.documentElement.dir === 'rtl';
    const preferredLeft = rtl ? rect.left - spacing - popoverW : rect.right + spacing;
    const left = Math.max(spacing, Math.min(preferredLeft, window.innerWidth - popoverW - spacing));
    setPopoverStyle({
      position: 'fixed',
        ...
        createPortal(
          ...
          >
            ...
            <ul className="max-h-[60vh] overflow-y-auto py-1">
              ...
              {activeConversations.hiddenCount > 0 && (
                /* B-861: the count is global, the list is not. Runs the viewer
                   may not see are still counted, so say WHY they carry no row
                   instead of letting the total look like a lie. */
                <li
                  role="presentation"
                ...
                >
                  ...
                  <span className="mt-0.5 block text-[10px] leading-snug opacity-80">
                    {t('activeConversationsElsewhereHint', {
                      defaultValue: 'In conversations you cannot see, so no row is marked.',
                    })}
                  </span>
                </li>
              )}
