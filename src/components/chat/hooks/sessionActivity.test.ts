...
describe('single history recovery control', () => {
  for (const isLoading of [false, true]) {
    it(`keeps only the history alert recovery action (loading=${isLoading})`, () => {
      assert.equal(shouldShowManualRefresh({ hasSession: true, hasHistoryError: true,
        isLoading, activitySourceAvailable: true }), false);
      assert.equal(shouldShowManualRefresh({ hasSession: true, hasHistoryError: false,
