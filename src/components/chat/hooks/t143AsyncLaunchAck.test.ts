 ...
 * FIRST tool_result is a launch ACKNOWLEDGMENT, not the final completion.
 *
 * The two execution modes for the `Agent` tool have different result shapes:
 * the background one (the
 ...
 * out-of-process for the rest of the turn (often past it), reporting final
 ...
 *
 * This test drives the production units against a minimal shape-equivalent stream —
...
describe('T-143: background-launch acknowledgment is not final completion', () => {
  it('keeps an async-launched Agent container running (isComplete=false)', () => {
    const stream: NormalizedMessage[] = [
      ...
      // Shape-equivalent launch acknowledgment arrives before delegated work ends.
      {
  ...
  it('still resolves a genuine (foreground/blocking) completion as done', () => {
    const stream: NormalizedMessage[] = [
      ...
      // Foreground delegation: the tool_result carries the final aggregated
    ...
    assert.equal(
      container!.subagentState?.isComplete,
      true,
      'a final (non-async) completion still resolves isComplete',
    );

    const progress = runHook(() => useRunProgress(chat, /* isLoading */ true));
    assert.equal(progress.activeSubagent, null, 'no active sub-agent once genuinely done');
    assert.equal(progress.agentsDone, 1, 'counted as done on final completion');
  });
});
