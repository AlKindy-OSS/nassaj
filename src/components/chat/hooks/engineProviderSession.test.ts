...
describe('readSessionEngineProvider', () => {
  ...
  it('reads only the session-scoped key, not the global', () => {
    ...
    // and that the current registry is honoured.
...
describe('B-218: engine eligibility is independent of the disabled-BODY list', () => {
  ...
  it('deepseek stays eligible according to the current engine registry', () => {
    stampSessionEngineProvider('sess-ds', 'deepseek');
    assert.strictEqual(readSessionEngineProvider('sess-ds'), 'deepseek');
  });
...
describe('T-915 scenario (storage-level)', () => {
  ...
  it('stale-resume branch copies old session stamp to new session (B-ENG continuity)', () => {
    ...
    // DeepSeek is eligible in the current registry, so the continuity mechanism
    // must preserve it on the newly minted session key.
    assert.strictEqual(
      lsStore.get(`claude-engine-provider-${newSid}`),
      'deepseek',
      'stale resume must preserve the eligible engine on the new session',
    );
    assert.strictEqual(readSessionEngineProvider(newSid), 'deepseek');
  });
});
