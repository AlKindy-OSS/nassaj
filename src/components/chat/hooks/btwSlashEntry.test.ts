 ...
 *   • بوابة المزوّد: claude وcodex (sideChannel.supported=true) يريان المدخلة
 *   • بوابة المزوّد: hermes/… (sideChannel.supported=false) لا يرى المدخلة
...
describe('بوابة sideChannel — أي المزوّدات تُظهر /btw', () => {
  it('claude → sideChannel.supported = true (تُظهر المدخلة)', () => {
    ...
    expect(getProviderCapabilities('claude').sideChannel.supportsFork).toBe(true);
  });

  it('codex → sideChannel.supported = true (تُظهر المدخلة)', () => {
    expect(getProviderCapabilities('codex').sideChannel.supported).toBe(true);
    expect(getProviderCapabilities('codex').sideChannel.supportsFork).toBe(false);
  });
