...
import { stripChatImageNote } from '../utils/chatImageNote';
import { readServerErrorCode } from '../utils/serverErrorMessage';
...
/**
 * Copy the canonical duration from the persisted response-turn metric attached
 * to the exact final assistant row. Timestamps are audit metadata only: the UI
 * must never derive a duration from them.
 */
export function attachTurnDurations(messages: ChatMessage[]): ChatMessage[] {
  return messages.map((message) => {
    ...
    if (!isPrimaryAssistantText) return message;
    const durationMs = message.responseTurnMetric?.durationMs;
    if (
    ...
    return { ...message, turnDurationMs: durationMs };
  });
}
 ...
 *
 * Background delegation has a distinct, documented result shape: its
 * container's tool_result is an acknowledgment with `toolUseResult` shaped
 ...
 * rest of the turn, reporting final completion later via a
 ...
 * immediately after delegation, so useRunProgress's activeSubagent /
...
/**
 * True for a sub-agent's own prose row: text/thinking/stream_delta carrying
 * `parentToolUseId` (the id of the delegating `Task`/`Agent` tool_use).
 *
 * The SDK streams the delegated prompt as an `SDKUserMessage` with
 * `parent_tool_use_id` set; the server copies it onto the normalized row as
 * `kind:'text', role:'user'` (claude-sdk.js transformMessage + ~L3736), often
 * with the socket's `userId` stamped because the SDK leaves `origin` unset.
 * Rendered verbatim it became a human bubble — and, being the last `type:'user'`
 * row, the new turn boundary for useRunProgress (agents=[]) and the anchor of
 * `runStartedAt` in ChatInterface (timer restart). The prompt is already shown
 * by the container's input and the reply by its tool_result, so the row is
 * folded away here; the sub-agent's tool rows fold via the pre-pass above.
 */
function isSubagentProse(msg: NormalizedMessage): boolean {
  if (!msg.parentToolUseId) return false;
  return msg.kind === 'text' || msg.kind === 'thinking' || msg.kind === 'stream_delta';
}
...
export function normalizedToChatMessages(messages: NormalizedMessage[]): ChatMessage[] {
  ...
  for (const msg of messages) {
    if (isSubagentProse(msg)) continue;

    const sharedMetadata = {
      // Server-attested transcript UUID — distinct from the UI message id and
      // the only identifier accepted by the message-fork endpoint.
      transcriptMessageId: msg.transcriptMessageId,
      ...
      // model mid-way.
      model: msg.model,
      coordinationLevel: msg.coordinationLevel,
      responseTurnMetric: msg.responseTurnMetric,
      responseToMessageId: msg.responseToMessageId,
      sessionId: msg.sessionId,
      imagesOmitted: msg.imagesOmitted,
      deferredPayload: msg.deferredPayload,
    };

    switch (msg.kind) {
      case 'text': {
        const content = msg.content || '';
        const hasUserImagePayload = msg.role === 'user' && (
          (Array.isArray(msg.images) && msg.images.length > 0)
          || (typeof msg.imagesOmitted === 'number' && msg.imagesOmitted > 0)
        );
        // Image-only prompts are valid user messages. Codex history can carry
        // their bytes (or an explicit omission count) without any text, so the
        // ordinary empty-text filter must not erase the entire bubble.
        if (!content.trim() && !hasUserImagePayload) continue;
        ...
        if (msg.role === 'user') {
          ...
          if (taskNotifMatch) {
            ...
            // B-430: the transcript keeps only the appended path note, so the
            // attached pictures are re-derived from it and served back from the
            // durable store — without this a reloaded message shows a dead path
            // where its image used to be.
            const attachedImages = userMessageImages(content, msg.images);
            converted.push({
              id: msg.id,
              type: 'user',
              // The note itself is stripped from the bubble: it leaks the bucket
              // capability into screenshots and copies, and the image rendered
              // below says the same thing better.
              content: stripChatImageNote(unescapeWithMathProtection(decodeHtmlEntities(content))),
      ...
      case 'error':
        converted.push({
          ...
          type: 'error',
          content: '',
          serverError: { code: msg.code, error: msg.error },
          timestamp: msg.timestamp,
          ...
          errorCode: readServerErrorCode(msg),
  ...
  const timed = attachTurnDurations(converted);
  ...
  if (reconcileByWf.size === 0) return timed;
