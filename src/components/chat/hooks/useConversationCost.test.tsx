...
describe('الجلب', () => {
  it('يطلب كلفة المحادثة مرّة عند الفتح على المسار المتفَّق عليه', async () => {
    ...
    expect(authenticatedFetch).toHaveBeenCalledWith(
      '/api/providers/costs/session/sess-1',
  ...
  it('مُعرِّف الجلسة يُرمَّز في المسار', async () => {
    ...
    expect(authenticatedFetch).toHaveBeenCalledWith(
      '/api/providers/costs/session/a%2Fb%3Fc',
      expect.objectContaining({ signal: expect.any(AbortSignal) }),
    );
  });
  ...
  it('اكتمال استرجاع السجل يُجدّد اللقطة مرّة لتظهر تفاصيل الأدوار', async () => {
    respondWith({ success: true, cost: COST });

    const { result, rerender } = renderHook(
      ({ historyReady }: { historyReady: boolean }) =>
        useConversationCost('sess-1', { historyReady }),
  ...
  it('يدمج هبوط البث وجاهزية السجل المتزامنين في طلب تحديث واحد', async () => {
    respondWith({ success: true, cost: COST });

    const { result, rerender } = renderHook(
      ({ isLoading, historyReady }: { isLoading: boolean; historyReady: boolean }) =>
        useConversationCost('sess-1', { isLoading, historyReady }),
      { initialProps: { isLoading: true, historyReady: false } },
    );

    await waitFor(() => expect(result.current.status).toBe('success'));
    expect(authenticatedFetch).toHaveBeenCalledTimes(1);

    rerender({ isLoading: false, historyReady: true });
    await waitFor(() => expect(authenticatedFetch).toHaveBeenCalledTimes(2));
    await new Promise((resolve) => setTimeout(resolve, 10));
    expect(authenticatedFetch).toHaveBeenCalledTimes(2);
  });
...
describe('عزل المحادثات', () => {
  ...
  it('تبديل المحادثة يُلغي طلبها الطائر فعلياً', async () => {
    authenticatedFetch.mockReturnValue(new Promise(() => {}));
    ...
    await waitFor(() => expect(authenticatedFetch).toHaveBeenCalledTimes(1));
    const firstSignal = authenticatedFetch.mock.calls[0]?.[1]?.signal as AbortSignal;
  ...
  it('إزالة المكوّن تلغي الطلب الطائر', async () => {
    authenticatedFetch.mockReturnValue(new Promise(() => {}));
    const { unmount } = renderHook(() => useConversationCost('sess-1'));
    await waitFor(() => expect(authenticatedFetch).toHaveBeenCalledTimes(1));
  ...
  it('التحديث اليدوي يلغي الطلب الأقدم ويحافظ على آخر لقطة أثناء الانتظار', async () => {
    respondWith({ success: true, cost: COST });
    const { result } = renderHook(() => useConversationCost('sess-1'));
    await waitFor(() => expect(result.current.status).toBe('success'));
    ...
    await waitFor(() => expect(authenticatedFetch).toHaveBeenCalledTimes(2));
    const secondSignal = authenticatedFetch.mock.calls[1]?.[1]?.signal as AbortSignal;
    expect(result.current.cost).toEqual(COST);
  ...
  it('فشل تحديث لاحق لا يمحو آخر لقطة معلومة', async () => {
    respondWith({ success: true, cost: COST });
    const { result } = renderHook(() => useConversationCost('sess-1'));
    await waitFor(() => expect(result.current.status).toBe('success'));
