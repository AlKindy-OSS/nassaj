...
export const AskUserQuestionPanel: React.FC<PermissionPanelProps> = ({
  ...
  // paints, so the input receives focus before any keystrokes can fire on
  // the wrong element.  This fixes the "typed text does not appear" bug
  // caused by the async useEffect window where the input was visible but
  // unfocused: characters typed during that window went to the button.
  useLayoutEffect(() => {
    if (otherActive.get(currentStep)) {
    ...
    >
      <div className="relative overflow-hidden rounded-2xl border border-border/80 bg-card shadow-lg dark:shadow-2xl">
        ...
        <div className="px-4 pb-2 pt-3.5">
          <div className="mb-1.5 flex items-center gap-2.5">
            ...
            <div className="relative flex-shrink-0">
              ...
              <div className="absolute -end-0.5 -top-0.5 h-2 w-2 animate-pulse rounded-full bg-cyan-400 dark:bg-cyan-500" />
            </div>
          ...
          {multi && (
            <span className="text-[10px] text-muted-foreground">{t('askUser.selectAll')}</span>
          )}
        </div>
          ...
          <div className="space-y-1">
                ...
                <button
                  ...
                  className={`group flex w-full items-center gap-2.5 rounded-lg border px-3 py-2 text-start transition-all duration-150 ${
                    isSelected
                      ...
                      : ' border-border hover:border-border hover:bg-accent/60'
                  }`}
                >
                  {/* Keyboard hint */}
                  <kbd className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded font-mono text-[10px] transition-all duration-150 ${
                    isSelected
                      ...
                      : 'border border-border bg-muted text-muted-foreground group-hover:border-border'
                  }`}>
                    {optIdx + 1}
                  </kbd>
            ...
            <button
              ...
              className={`group flex w-full items-center gap-2.5 rounded-lg border px-3 py-2 text-start transition-all duration-150 ${
                isOtherOn
                  ? 'border-blue-300 bg-blue-50/80 ring-1 ring-blue-200/50 dark:border-blue-600 dark:bg-blue-900/25 dark:ring-blue-700/30'
                  : ' border-dashed border-border hover:border-border hover:bg-accent/60'
              }`}
            >
              <kbd className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded font-mono text-[10px] transition-all duration-150 ${
                isOtherOn
              ...
              <div className="pl-[30px] pr-0.5">
                <div className="relative">
                  <input
                    ref={otherInputRef}
                    type="text"
                    // autoFocus ensures focus on mount (React commit phase, before
                    // paint), which is a belt-and-suspenders companion to the
                  ...
                  {/* pointer-events-none: prevents the badge from intercepting
                      ...
                      unfocused and drop all subsequent keystrokes. */}
                  <kbd className="pointer-events-none absolute end-2 top-1/2 -translate-y-1/2 rounded border border-border bg-muted px-1 py-0.5 font-mono text-[9px] text-gray-300 dark:text-muted-foreground">
                    Enter
                  </kbd>
                </div>
              </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t border-border bg-muted/50 px-4 py-2">
          ...
          >
            {isSingle ? t('askUser.skip') : t('askUser.skipAll')}
          ...
          <div className="flex items-center gap-1.5">
            {!isSingle && !isFirst && (
              <button
                ...
                className="inline-flex items-center gap-0.5 rounded-lg px-2.5 py-1.5 text-[11px] font-medium text-muted-foreground transition-all duration-150 hover:bg-accent"
              >
