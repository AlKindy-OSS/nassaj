  ...
  return (
    <div className="my-1 border-s-2 border-s-purple-500 py-0.5 ps-3 dark:border-s-purple-400">
      ...
      >
        ...
        {/* نصّ الطلب المُمرَّر للوكيل الفرعي نثرٌ بلغة المستخدم — يحسم اتجاهه
            بمحتواه لا بلغة الواجهة. بقية الصفوف هنا أسماء أدوات ومسارات
            (لاتينية بحكم البناء) فلا تحتاج حسماً. */}
        {prompt && (
          <div
            className="mb-2 line-clamp-4 whitespace-pre-wrap break-words text-xs text-muted-foreground"
            dir={resolveTextDirection(prompt) ?? undefined}
          >
            {prompt}
          </div>
        )}
        ...
        {currentTool && !isComplete && (
          <div className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <span className="text-muted-foreground/60">{t('subagent.currently')}</span>
              ...
              <div className="mt-1 space-y-0.5 border-s border-border ps-3">
                {childTools.map((child, index) => (
                  <div key={child.toolId} className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                    {/* منطقي لا فيزيائي: ترقيم الصفوف يحاذي حافة القائمة في
                        الاتجاهين، فلا يتباعد عن اسم الأداة تحت `dir="rtl"`. */}
                    <span className="w-4 flex-shrink-0 text-end text-muted-foreground/60">{index + 1}.</span>
                    ...
                    {child.toolResult?.isError && (
                      <span className="flex-shrink-0 text-red-500">{t('subagent.error')}</span>
                    )}
                  </div>
                ))}
              </div>
