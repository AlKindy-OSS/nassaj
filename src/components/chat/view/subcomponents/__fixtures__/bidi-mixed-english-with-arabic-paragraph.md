...
2. **Secondary, worse on a slower/loaded node like fleet-node**: `ProviderSelectionEmptyState.tsx:182-194` — a race between the dialog-open effect and the button click; clicking refresh while the dialog-open's own auth check is still in flight silently no-ops, and the button's spinner/disabled state doesn't reflect this, so it looks idle rather than busy.
