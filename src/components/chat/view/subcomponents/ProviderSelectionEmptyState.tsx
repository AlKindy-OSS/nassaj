...
/**
 * الرابط العميق لإدخال اعتمادٍ ينقص — **وجهةٌ واحدة لكل مفتاح** (‏B-413 ثم
 * T-1219).
 *
 * كان الموضعان أدناه يرسلان `agent: group.id`، أي الجسم الذي يُعرض الصفّ تحته.
 * وهو صحيحٌ للجسم الذي يملك مفتاحه، وخاطئٌ تماماً للمحرّك: صفّ «أضف مفتاح GLM»
 * يعيش تحت مجموعة Claude ومفتاحه ملكُ Z.AI. فأُشتقّت الوجهة من الشركة، وكانت
 * تنقسم بين حساب وكيلٍ وتبويب المورّدين بحسب مَن يملك بلاطةً ظاهرة.
 *
 * وT-1219 أسقط القسمة: لا حقلَ إدخالٍ خارج تبويب «المورّدون والاعتمادات»، فكلّ
 * وجهةٍ هو. ويبقى الفرق بين الفرعين فرقاً حقيقياً لا شكلياً: **موضعٌ معروف**
 * يَعِد بحقلٍ ينتظره هناك، و**موضعٌ مجهول** لا شركة له في الكتالوج فلا يُوعَد
 * بحقلٍ أصلاً — يُفتح حسابُ الوكيل حيث حالتُه ومسارُ تسجيل دخوله.
 */
function credentialDeepLink(
  destination: CredentialDestination | null | undefined,
  fallbackAgent: LLMProvider,
): SettingsDeepLink {
  if (!destination) {
    // لا شركة معروفة لهذا الموضع: لا مفتاحَ يُوعَد به — حساب المجموعة نفسها.
    return { tab: 'agents', agent: fallbackAgent, category: 'account' };
  }
  return { tab: 'vendors', companyId: destination.companyId };
}
  ...
  return (
    <button
      ...
      className={[
        ...
        // design-ok: uppercase/tracking-wider intentionally omitted — Arabic
        // text (e.g. "المفضّلة") breaks when letter-spacing is applied to
        // connected script. Latin provider names remain readable without it.
...
export default function ProviderSelectionEmptyState({
  ...
  // `refresh` is destructured deliberately: the hook only fetches on mount, and
  // this dialog outlives a trip to Settings. Dropping it left a saved key
  // invisible here until a full remount — the row stayed locked after a
  // successful save, and the CTA led back to the same locked row (T-1139).
  const { statuses: vendorKeyStatuses, refresh: refreshVendorKeys } = useVendorKeyStatuses();
  ...
  }, [providerModelsRefreshing, onHardRefreshProviderModels, onRefreshAuthStatus, refreshVendorKeys]);
  ...
  const providerVisibilityMap = useMemo<Record<LLMProvider, boolean>>(() => {
    ...
    for (const p of PROVIDER_META) {
      ...
      result[p.id] = status ? isProviderVisible(status) : false;
    }
    return result;
  }, [providerAuthStatus]);
  ...
  const providerDisabledMap = useMemo<Record<LLMProvider, boolean>>(() => {
    ...
    for (const p of PROVIDER_META) {
      const status = providerAuthStatus[p.id];
      result[p.id] = status ? isProviderDisabled(status) : true;
    }
    return result;
  }, [providerAuthStatus]);
  ...
  // وضع الاتصال لكل مزوّد — يُشتقّ من providerAuthStatus ويُمرَّر إلى
  // rowsForBody ليُلصَق على كل صفّ في المنتقي.
  const authModes = useMemo<Partial<Record<string, 'api_key' | 'subscription_oauth' | 'both' | null>>>(() => {
    const result: Partial<Record<string, 'api_key' | 'subscription_oauth' | 'both' | null>> = {};
    for (const p of PROVIDER_META) {
      const status = providerAuthStatus[p.id];
      if (status?.authMode) {
        result[p.id] = status.authMode;
      }
    }
    return result;
  }, [providerAuthStatus]);
  ...
  const allFlatRows = useMemo<PickerRow[]>(() => {
    return visibleProviderGroups.flatMap((group) =>
      rowsForBody(group.id, providerModelCatalog, vendorKeyStatuses, authModes),
    );
  }, [visibleProviderGroups, providerModelCatalog, vendorKeyStatuses, authModes]);
  ...
  // B-266: cmdk's <CommandEmpty> counts the items it has RENDERED, and a
  // collapsed group renders none. With START_COLLAPSED as the default (B-245)
  // the picker therefore announced "No models found" while sitting on top of
  // six groups full of models — a false statement produced by folding, not by
  // absence.
  //
  // The fix keeps cmdk's own emptiness for the ONE case where it is truthful —
  // an active search, during which every group is force-expanded
  // (`expanded = isSearching || …`), so zero rendered items really does mean
  // zero matches — and derives the no-search case from the DATA instead of
  // from the DOM. `hasPickableContent` asks "does the picker have anything at
  // all to offer?", which no collapse state can change.
  const hasPickableContent = favoriteRows.length > 0 || visibleProviderGroups.length > 0;
  // While the catalog is still loading an empty list means "not yet", not
  // "none" — stay silent rather than assert either way.
  const showTrulyEmpty = !isSearching && !hasPickableContent && !providerModelsLoading;
            ...
            <DialogContent className="max-w-md overflow-hidden p-0">
              ...
              <Command>
                ...
                <CommandList className="max-h-[350px]">
                  {/* B-266: mounted only while a search is filtering. Outside
                      search, collapsed groups render no items and cmdk would
                      call the list empty although it is not. */}
                  {isSearching && (
                    <CommandEmpty>
                  ...
                  {!isSearching && favoriteRows.length > 0 && (() => {
                    ...
                    return (
                      <CommandGroup
                        key={FAV_GROUP_ID}
                        className="[&_[cmdk-group-heading]]:uppercase"
                      >
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  {row.authMode === 'api_key' && (
                                    <span className="shrink-0 rounded bg-muted/60 px-1 py-0.5 text-[9px] font-medium text-muted-foreground" title={t("providerSelection.authMode.apiKey", { defaultValue: "API key" })}>
                                      API
                                    </span>
                                  )}
                                  {row.authMode === 'both' && (
                                    <span className="shrink-0 rounded bg-muted/60 px-1 py-0.5 text-[9px] font-medium text-muted-foreground" title={t("providerSelection.authMode.both", { defaultValue: "Subscription + API" })}>
                                      +API
                                    </span>
                                  )}
                                </div>
                  ...
                  {visibleProviderGroups.map((group, idx) => {
                    ...
                    // agy accepts `--model` (measured on 1.1.9), so its rows are
                    // listed in full like every other body. This used to be
                    // `allRows.slice(0, 1)`, which clamped the group to the single
                    // "agy default" row and hid the whole live `agy models`
                    // catalog even though the backend was already serving it.
                    const rows: PickerRow[] = rowsForBody(
                      group.id,
                      providerModelCatalog,
                      vendorKeyStatuses,
                      authModes,
                    ...
                    return (
                      <CommandGroup
                        ...
                        className={
                          idx > 0
                            ? "border-t border-border/40 [&_[cmdk-group-heading]]:mt-1 [&_[cmdk-group-heading]]:uppercase"
                          ...
                          <>
                            {isLockedVendor ? (
                              <CommandItem
                                ...
                                onSelect={() => {
                                  setDialogOpen(false);
                                  // T-1205 — الاعتماد عاد إلى تبويب «الحساب»، وهذه
                                  // المرّة **بالنموذج نفسه** لا بفقرة إشارة: بطاقة
                                  // الشركة تُعرض في صفحة كل وكيلٍ يقرأ مفتاحها.
                                  // وB-413 — والوجهة **منزلُ الشركة** لا اسمُ
                                  // المجموعة: مجموعةٌ مقفولة لشركةٍ بلا بلاطة ظاهرة
                                  // تقود إلى تبويب المورّدين حيث بطاقتُها فعلاً.
                                  onShowSettings?.(
                                    credentialDeepLink(bodyCredentialDestination(group.id), group.id),
                                  );
                                }}
                              ...
                              return (
                                <CommandItem
                                  ...
                                  onSelect={() => {
                                    if (row.locked) {
                                      setDialogOpen(false);
                                      // ‏B-413 — كان هنا «‏the only surface that writes one
                                      // is this agent's Account tab (T-1205)»، وT-1206
                                      // أبطلته في الساعات نفسها: حساب الوكيل يحمل بطاقة
                                      // شركته الأصيلة وحدها، ومفتاح المحرّك ملكُ شركةٍ
                                      // أخرى بطاقتُها في صفحة وكيلٍ آخر (أو في تبويب
                                      // المورّدين). الوجهة محسوبةٌ على الصفّ من
                                      // `companyHomeAgent`، فلا يقول الزرُّ مكاناً غير
                                      // الذي تُصيَّر البطاقة فيه.
                                      onShowSettings?.(
                                        credentialDeepLink(row.credentialDestination, group.id),
                                      );
                                      return;
                                    }
                                    if (row.engineProvider) {
