...
export default function ClaudeStatus({
  ...
  // Distinguish a run that started while this view was mounted from attaching
  // to an already-running session. The former has a truthful local start even
  // before its user-message timestamp reaches the transcript; the latter does
  // not, so it must remain hidden until `runStartedAt` resolves.
  ...
  /**
   * نسخة state من `anchorIsFallbackRef` (الـref لا يُعيد الرسم).
   *
   * B-208 (بند 8 — فيتو): بعد تحديث الصفحة قد يقع آخر صفّ مستخدم خارج نافذة
   * الجلب فيعود `runStartedAt = null`؛ كان المؤقّت حينها يبدأ من الآن فيعرض
   * «0:03» لتشغيل عمره 20 دقيقة. يُخفى المؤقّت حتى تُحلّ المرساة الحقيقية.
   */
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 mb-3 w-full duration-500">
      <div className="mx-auto flex max-w-4xl items-center justify-between gap-3 overflow-hidden rounded-full border border-border/50 bg-muted px-3 py-1.5 shadow-sm backdrop-blur-md">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Current-run elapsed time is activity evidence, not an abort
              control. Keep it visible when interruption is unavailable and on
              narrow screens; completed response totals live elsewhere. */}
          {isLoading && anchorResolved && (
            <div
              data-testid="current-run-elapsed"
              className="flex shrink-0 items-center rounded-md bg-muted/50 px-2 py-0.5 text-[10px] font-medium tabular-nums text-muted-foreground"
            >
              {formatWorkDuration(
                elapsedTime * 1_000,
