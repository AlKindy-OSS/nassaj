...
describe('btwCommand — كشف أمر /btw', () => {
  ...
  it('«/side <سؤال>» مرادف Codex صالح ويستخرج السؤال نفسه', () => {
    expect(isBtwCommand('/side inspect the current diff')).toBe(true);
    expect(parseBtwQuestion('/SIDE inspect the current diff')).toBe('inspect the current diff');
  });

  it('يقبل /side لـCodex فقط مع إبقاء /btw مشتركاً', () => {
    expect(isSideChannelCommandForProvider('/side inspect', 'codex')).toBe(true);
    expect(isSideChannelCommandForProvider('/side inspect', 'claude')).toBe(false);
    expect(isSideChannelCommandForProvider('/btw inspect', 'codex')).toBe(true);
    expect(isSideChannelCommandForProvider('/btw inspect', 'claude')).toBe(true);
  });

  it('يحجز token الجانبي قبل التحقق من السؤال أو الجلسة', () => {
    expect(isReservedSideChannelCommandForProvider('/side', 'codex')).toBe(true);
    expect(isReservedSideChannelCommandForProvider('/side سؤال', 'codex')).toBe(true);
  ...
  it('«/btw» وحدها أو ببلا سؤال ليست أمراً (تسقط للمسار العادي)', () => {
    ...
    expect(isBtwCommand('/side')).toBe(false);
    expect(isBtwCommand('/side   ')).toBe(false);
  });
  ...
  it('«/btwx» ليست أمر btw (لا مسافة بعد الأمر)', () => {
    ...
    expect(isBtwCommand('/sidebar hello')).toBe(false);
  });
