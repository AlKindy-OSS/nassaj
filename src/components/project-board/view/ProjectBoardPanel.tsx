...
export default function ProjectBoardPanel({ selectedProject, onFileOpen }: ProjectBoardPanelProps) {
  ...
  const {
    cost: projectCost,
    stats: projectStats,
    codeStats: projectCodeStats,
    isLoading: projectStatsLoading,
    loadError: projectStatsError,
    refresh: refreshProjectStats,
  } = useProjectStats(selectedProject?.projectId);
  ...
  // Once the board has established a real project, its statistics destination
  // is stable. The three measurements arrive independently and must not make the
  // tab pop into the header late (or disappear again after a failed refresh).
  const hasStats = Boolean(selectedProject?.projectId);
    ...
    <div className="flex h-full min-h-0 flex-col bg-background">
      ...
      <div className="min-h-0 flex-1">
        ...
        {activeSection === 'tasks' && board.state && (
          <TasksView state={board.state} onIssueClick={handleIssueClick} />
        )}
        ...
        {activeSection === 'visual' && board.state && (
          <VisualChecksView state={board.state} />
        )}
        {activeSection === 'stats' && (
          <ProjectStatsTab
            stats={projectStats}
            ...
            isLoading={projectStatsLoading}
            loadError={projectStatsError}
            onRefreshCodeStats={refreshProjectStats}
          />
        )}
