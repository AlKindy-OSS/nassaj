...
vi.mock('../hooks/useProjectStats', () => ({
  useProjectStats: () => projectStatsState.current,
}));
...
describe('board tabs', () => {
  it('shows Statistics immediately while its independent measurements are loading', () => {
    renderPanel();

    click(tab('Statistics'));

    expect(screen.getByRole('status', { name: 'Loading project statistics' })).toBeTruthy();
  });
