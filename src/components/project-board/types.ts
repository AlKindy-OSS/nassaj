...
export type BoardPhase = {
  ...
  status: PhaseStatus;
  /**
   * @deprecated Manual, file-authored figure — board files on disk still carry
   * it, but no computation reads it anymore (T-1809: it drifted from the
   * phase's own tasks and printed 100% on a phase that was 60% done). Real
   * progress comes from `phaseTaskStats`/`overallProgress` in `lib/boardStats`.
   */
  progress?: number;
};
