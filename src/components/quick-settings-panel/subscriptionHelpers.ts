...
export type SubscriptionCost = {
  ...
  /**
   * الرصيد المتبقّي كما يعلنه المزوّد (‏moonshot وحده اليوم). غيابه أو `null`
   * يعني «لا مصدر» فلا يُرسم السطر أصلاً — ولا يُعرض صفراً، لأن صفر الرصيد
   * يُقرأ «نَفِد حسابك».
   *
   * مستقلٌّ عن `totalUsd` قصداً: ذاك ما أُنفِق في هذه الدورة محسوباً من
   * السجلّ، وهذا ما بقي في الحساب مقروءاً من المزوّد.
   */
