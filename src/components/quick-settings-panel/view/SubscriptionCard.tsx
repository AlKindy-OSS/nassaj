import { AlertCircle, AlertTriangle, ChevronDown, Info, Wallet } from 'lucide-react';
...
// One colour per note, never a base + an override: a class emitted later in the
// compiled stylesheet wins regardless of JSX order (measured in
// dist/assets/index-*.css), so a tone class appended to a muted base rendered
// grey. Both notes now name their own single colour token (B-399).
const NOTE_BASE_CLASS = 'flex items-start gap-1.5 text-[11px]';
const NOTE_CLASS = `${NOTE_BASE_CLASS} text-muted-foreground`;
const WARNING_NOTE_CLASS = `${NOTE_BASE_CLASS} text-warning`;
...
export default function SubscriptionCard({
  ...
  // الرصيد المتبقّي كما يعلنه المزوّد. الشرط رقميّ صريح لا `!= null`: الخادم
  // يرسل `null` عند تعذّر القراءة، و`undefined` من خادم أقدم، وكلاهما «لا
  // نعرف» — أما صفرٌ حقيقي فيُعرض، لأنه هنا خبرٌ مهمّ («نَفِد رصيدك») لا غياب.
  const balanceUsd = subscription.balanceUsd;
  const hasBalance = typeof balanceUsd === 'number' && Number.isFinite(balanceUsd);
  // رصيدٌ نَفِد يُوسَم: المزوّد يرفض النداءات عند الصفر أو دونه.
  const balanceDepleted = hasBalance && balanceUsd <= 0;
  const balanceExhaustedNote = t('subscriptions.balanceDepleted', {
    defaultValue: 'Balance exhausted — the provider will refuse new requests',
  });
      ...
      >
        ...
        {state.kind === 'amount' ? (
          ...
          >
            ...
            {/* رصيدٌ نَفِد يجب ألّا ينتظر نقرة: المزوّد يرفض النداءات عنده،
                فالبطاقة المطويّة تقول ذلك بمحفظةٍ حمراء. أما المبلغ نفسه فيبقى
                في الجسم — الرأس للمقارنة بين البطاقات، ورقمان فيه لا يُقارَنان. */}
            {balanceDepleted && (
              <span title={balanceExhaustedNote} className="self-center">
                <Wallet
            ...
            {balanceDepleted && <span className="sr-only">{balanceExhaustedNote}</span>}
          </span>
        ) : (
      ...
      <div id={bodyId}>
        {isExpanded && (
          <div className="space-y-1.5 border-t border-border/60 px-3 py-2">
            ...
            {/* الرصيد المتبقّي — رقمٌ من المزوّد لا من مسحنا، فيقف بذاته فوق
                التفصيل. يُرسم فقط حين يُعلنه المزوّد؛ ولا بديل نصّي عند غيابه،
                فالصمت هنا صدقٌ لا نقص. */}
            {hasBalance && (
              <p
                className={balanceDepleted ? WARNING_NOTE_CLASS : NOTE_CLASS}
                data-testid="subscription-balance"
              >
                <Wallet className="mt-0.5 h-3 w-3 shrink-0" aria-hidden="true" />
                <span className="flex flex-1 items-baseline justify-between gap-2">
                  <span>
                    {t('subscriptions.balanceRemaining', { defaultValue: 'Remaining balance' })}
                  </span>
                  <bdi className="shrink-0 tabular-nums font-medium">
                    {formatUsd(balanceUsd, locale)}
                  </bdi>
                </span>
              </p>
            )}
            {balanceDepleted && (
              <p className={WARNING_NOTE_CLASS}>
                ...
                <span>{balanceExhaustedNote}</span>
              </p>
            )}
