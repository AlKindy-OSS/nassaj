...
describe('B-247 — the raw queue is visible in the panel', () => {
  ...
  it('counts BOTH queues in the header badge', () => {
    ...
    // 1 restartRequired + 1 server action + 2 raw commands — in the panel
    // header and, since T-1684, on the queue tab beside it.
    const heading = screen.getByRole('heading', { name: enSidebar.pendingActions.title });
    expect(heading.parentElement?.querySelector('span')?.textContent).toBe('4');
    expect(screen.getByRole('tab', { name: new RegExp(enSidebar.pendingActions.tabQueue) }).textContent)
      .toContain('4');
  });
});
...
describe('raw response uncertainty', () => {
  it.each(['network', 'proxy'])('does not claim execution or repeat after %s failure', async (failure) => {
    vi.stubGlobal('crypto', {});
    if (failure === 'network') authenticatedFetch.mockRejectedValue(new TypeError('connection lost'));
    else authenticatedFetch.mockResolvedValue({ ok: false, json: async () => ({}) });
    render(<PendingActionsPanel {...baseProps()} />);
    fireEvent.click(screen.getByRole('button', { name: new RegExp(`^${enSidebar.pendingActions.rawReview}`) }));
    fireEvent.click(await screen.findByRole('checkbox'));
    const button = screen.getByRole('button', { name: enSettings.commandBoardSettings.rawExec.dialog.execute });
    await waitFor(() => expect((button as HTMLButtonElement).disabled).toBe(false));
    fireEvent.click(button);
    await waitFor(() => expect(screen.getByText(enSettings.commandBoardSettings.rawExec.dialog.outcomeUnverified)).toBeTruthy());
