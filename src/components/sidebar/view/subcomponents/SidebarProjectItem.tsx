      ...
      <div className="md:group group" onMouseEnter={markInteracted} onTouchStart={markInteracted}>
        ...
        >
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              {isEditing ? (
                // The LTR pin belongs to the filesystem path alone (below), not
                // to this wrapper: an Arabic project name typed into the input
                // was being force-aligned to the left edge while every other
                // label in the row reads from the right. The name field gets
                // `dir="auto"` instead — it is a static value, so first-strong
                // matches whichever script the user actually typed.
                <div className="space-y-1">
                  ...
                  />
                  {/* The path stays LTR-pinned; `text-start` under `dir="ltr"`
                      resolves to the left edge in both UI directions. */}
                  <div dir="ltr" className="truncate text-start text-xs text-muted-foreground" title={project.fullPath}>
                    {project.fullPath}
                  </div>
                </div>
              ) : (
