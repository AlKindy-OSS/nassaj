          ...
          content={
            tooltip.shortcut ? (
              <span className="flex items-center gap-1.5">
                {tooltip.content}
                {/* داخل سطح التلميح المقلوب (`bg-foreground`)، فالنبرة من
                    `background` لا من الأبيض الثابت. */}
                <kbd className="rounded bg-background/20 px-1 text-[10px]">{tooltip.shortcut}</kbd>
              </span>
            ) : (
              tooltip.content
            )
    ...
    return (
      ...
      >
        {children ?? (isActive ? (
          ...
          // سهم الإرسال أيقونة اتجاهية في التدفّق، فيُعكس أفقياً تحت rtl ليشير
          // إلى جهة سير القراءة بدل أن يشير عكسها.
          <SendHorizonalIcon className="h-4 w-4 rtl:-scale-x-100" />
        ))}
      </Button>
    );
  }
