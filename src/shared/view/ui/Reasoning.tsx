...
/**
 * النصّ الافتراضي مترجَم، والمترجِم يصل بالتمرير لا بخطّاف: هذه دالّة عادية لا
 * مكوّن، فاستدعاء `useTranslation` بداخلها يخالف قواعد الخطّافات — والمكوّن
 * `ReasoningTrigger` يملك الخطّاف ويمرّر `t`.
 */
const defaultGetThinkingMessage = (
  ...
  if (duration === undefined) {
    return <p>{t('reasoning.thoughtBrief')}</p>;
  }
  return <p>{t('reasoning.thoughtFor', { count: duration })}</p>;
};
...
export const ReasoningTrigger = React.memo<ReasoningTriggerProps>(
  ({
    ...
    const renderThinking = getThinkingMessage
      ?? ((streaming: boolean, ms?: number) => defaultGetThinkingMessage(t, streaming, ms));
      ...
      >
        {children ?? (
          <>
            ...
            {renderThinking(isStreaming, duration)}
            <ChevronDownIcon
              className={cn(
