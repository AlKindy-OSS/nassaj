 ...
 * (see deriveWorkflowUiState). Consumers may limit which workflow states are
 * visible when a surface needs to communicate one specific condition.
 *
 * Two modes:
...
type WorkflowStatusBadgeProps = {
  ...
  rollup?: boolean;
  /** Optional allow-list of workflow statuses this surface should show. */
  visibleStatuses?: ReadonlyArray<ActiveWorkflow['status']>;
  className?: string;
};
...
function WorkflowSessionBadge({
  ...
  const primary = pickPrimaryWorkflow(
    visibleStatuses
      ? workflows.filter((workflow) => visibleStatuses.includes(workflow.status))
      : workflows,
  );
