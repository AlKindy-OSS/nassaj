...
// ----------------------------------------------------------------------------
// عدّاد «الفقد العضوي» لظاهرة B-103. أداة قراءة-فقط: تمسح سجلّات PM2 للعملية
// المضبوطة، تستخرج أحداث فقد البثّ (stream-orphaned)، وتصنّف كل حدث:
...
// configured PM2 logs, extracts stream-loss events, and classifies each as
...
const PROCESS_NAME = process.env.PROC_NAME || process.env.NASSAJ_PROCESS_NAME || 'nassaj-dev';
...
function defaultLogGlob() {
  ...
  const logPattern = new RegExp(`^${escapeRegExp(PROCESS_NAME)}-out(__.*)?\\.log$`);
  let files = [];
  try {
    files = fs.readdirSync(dir)
      .filter((f) => logPattern.test(f))
