...
# ----------------------------------------------------------------------------
# الغرض / Purpose:
#   مراقبة قراءة-فقط لاستهلاك ذاكرة (RSS) عملية PM2 المضبوطة وعملياتها
...
#
#   Read-only RSS monitor for the configured PM2 process and its child
...
#   مجموع RSS قد يتجاوز سقف العملية → قد يبدأ PM2 إعادة تشغيل تقطع الجلسات،
#   أو يضغط ذاكرة النظام. راجع قسم الذاكرة في دليل التشغيل.
#
...
# لأن السقف الحيّ قد يكون None رغم وجود قيمة في الملف).
...
lines.append(f"  {d['proc']} RSS   = {mb(rss)}")
...
if cap:
    ...
    if pct >= thr:
        status = "WARN"
        lines.append(f"  ⚠️ {d['proc']} RSS = {pct:.0f}% من السقف (عتبة {thr:.0f}%) → خطر restart وقطع الجلسات")
    ...
    lines.append("  ℹ️ لا سقف حيّ — اعتمد على عتبة ذاكرة النظام أدناه")
