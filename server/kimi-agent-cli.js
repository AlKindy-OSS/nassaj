 ...
 *           neutralGovernanceSource?: Function,
 *           isGovernanceExempt?: Function }} [deps] test seam (spies/overrides)
...
export function prepareKimiAgentLaunch(params, deps = {}) {
  const {
    ...
    cwd,
    baseEnv = process.env,
    // T-1315: مستوى تنسيق الجولة. يُطبَّق على المطالبة عند بناء argv أدناه —
    // قناة kimi الوحيدة في وضع الوكيل. الإنفاذ **نصّي**: أعلام السقف (SL-4)
    // تحدّ الأذونات لا عمقَ الوكلاء، ولا مفتاح عمقٍ موثَّقاً في kimi-code.
  ...
  // — UNLESS this user exempted kimi (owner decision 2026-08-08).
  //
  // The check is here, in the launch seam, and not inside ensureVendorCliGovernance:
  // that gate is vendor-NEUTRAL and takes no user id by contract, and this seam is
  // the layer that holds both. Skipping the gate (rather than letting it run and
  // ignoring the throw) is the point: the gate self-heals by re-writing the 0444
  // copy, so merely catching its error would restore the very file the exemption
  // removed. An exemption is a decision, not a fault — so the turn launches, in
  // kimi's own default posture.
  const kimiHome = resolveKimiHome(resolvedEnv);
  const kimiExempt = (deps.isGovernanceExempt ?? isGovernanceExempt)(userId, 'kimi');
  if (!kimiExempt) {
  ...
  const args = buildKimiAgentArgs({
    command: withCoordinationDirective(command, coordinationLevel),
    cliSessionId,
    model,
...
export async function spawnKimiAgent(command, options = {}, ws) {
  ...
  try {
    prepared = prepareKimiAgentLaunch({
      ...
      cwd: workingDir,
      baseEnv: process.env,
      coordinationLevel: options?.coordinationLevel,
    });
  } catch (err) {
  ...
  let assistantBlocks = [];
  const transcriptMessages = command ? [{ role: 'user', content: command }] : [];
  // Final timing uses the acknowledged vendor transcript ID that history reads.
  ...
  return new Promise((resolve, reject) => {
    ...
    // B-395: register with the process monitor so a Kimi turn shows the same
    // "Running" badge / busy dot / active-conversations entry a Claude turn
    // does, plus frozen (kill -STOP) detection from the known pid.
    ...
    kimiProcess.on('close', async (code, signal) => {
      if (!providerSucceeded(code, terminalFailureReason, kimiProcess.nassajAborted || signal) && code === 0) code = 1;
