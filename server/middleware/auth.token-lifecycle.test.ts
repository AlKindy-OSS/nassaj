 ...
 *    mints of the same identity in the same JWT second produced different tokens
...
test('B-164: different milliseconds in one JWT second mint byte-identical tokens', (t) => {
  let nowMs = 1_700_000_000_100;
  t.mock.method(Date, 'now', () => nowMs);
  // Model a refresh burst at distinct milliseconds without crossing
  // jsonwebtoken's second-level iat. The old `|| Date.now()` fallback made
  // every pwd_iat payload (and therefore every token) different.
  const burst = Array.from({ length: 64 }, () => {
    const token = generateToken({ id: 1, username: 'a', role: 'user' });
    nowMs += 5;
    return token;
  });
...
test('B-164: crossing a JWT second advances iat and exp while pwd_iat stays deterministic', (t) => {
  let nowMs = 1_700_000_000_999;
  t.mock.method(Date, 'now', () => nowMs);
  const first = generateToken({ id: 1, username: 'a', role: 'user' });
  nowMs += 1;
  const second = generateToken({ id: 1, username: 'a', role: 'user' });
  const firstClaims = decode(first);
  const secondClaims = decode(second);
  assert.notEqual(first, second, 'JWT lifetime must not be frozen across issue seconds');
  assert.equal(secondClaims.iat, (firstClaims.iat as number) + 1);
  assert.equal(secondClaims.exp, (firstClaims.exp as number) + 1);
  assert.equal(firstClaims.pwd_iat, 0);
  assert.equal(secondClaims.pwd_iat, 0);
});
