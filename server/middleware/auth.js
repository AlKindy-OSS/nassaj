...
const authenticateToken = async (req, res, next) => {
  ...
  if (IS_PLATFORM) {
    try {
      ...
      req.user = user;
      // Mirrors authenticateWebSocket's platform branch: this principal was
      // never verified, so the launch actor must refuse it as
      // PLATFORM_ACTOR_UNVERIFIED rather than as a malformed actor (B-820).
      req.user.authenticationKind = 'platform_unverified';
  ...
  try {
    ...
    req.user = user;
    // Canonical launch-actor fields (B-820). createAuthenticatedLaunchActor()
    // reads `authenticationKind` and camelCase `authorizationGeneration`, but a
    // raw users row carries neither — it only has snake_case
    // `authorization_generation`. authenticateWebSocket() below already shapes
    // both; the REST verifier never did, so every route that mints an execution
    // permit (provider catalog, session launch, message send) threw
    // ACTOR_AUTHENTICATION_KIND_INVALID for token-authenticated callers. This is
    // the same JWT-verified session the WebSocket path labels 'session'.
    req.user.authenticationKind = 'session';
    req.user.authorizationGeneration = user.authorization_generation;
...
// within the same JWT second — the amplifier that turned the B-163 refresh burst
// hysterical. Falling back to 0 instead makes the password-version claim
// deterministic while the standard JWT iat/exp still advance each second. It
// is also strictly safer: the moment the row gains a
    ...
    try {
      ...
      if (user) {
        return {
          ...
          username: user.username,
          role: user.role,
          authenticationKind: 'platform_unverified',
          authorizationGeneration: user.authorization_generation,
        };
      }
      return null;
