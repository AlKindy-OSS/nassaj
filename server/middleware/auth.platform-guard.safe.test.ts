...
test('ADR-134: platform websocket identity is explicitly unverified for provider gates', () => {
  const actor = authenticateWebSocket(null);
  assert.equal(actor?.id, FIRST_USER.id);
  assert.equal(actor?.authenticationKind, 'platform_unverified');
  assert.equal(actor?.authorizationGeneration, undefined);
});
