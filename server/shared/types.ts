...
export type NormalizedMessage = {
  ...
  kind: MessageKind;
  /**
   * Ephemeral browser turn identity echoed on live events by a runner that can
   * attest which submitted prompt owns the run. It is never inferred from
   * transcript order. Canonical history may expose it only after authenticated
   * requester-scoped receipt filtering; shared history caches must not contain it.
   */
  clientMsgId?: string;
  /**
   * Render-only pairing of a canonical user text row with the requester's
   * optimistic bubble (B-1078). Set from an exact requester-scoped receipt
   * identity match regardless of turn lifecycle or verdict; never proof that
   * the outbox may delete (that stays `clientMsgId`). Absent from shared caches.
   */
  displayClientMsgId?: string;
  /** Exact optimistic user row answered by this live run. */
  responseToMessageId?: string;
  /** Immutable server-recorded coordination policy for the user turn. */
  coordinationLevel?: 'direct' | 'delegate' | 'delegate_review';
  /** Server verdict: whether transport retry may safely reuse clientMsgId. */
  sameClientMsgIdRetryable?: boolean;
  /** Durable server-attested metric; absent until persistence succeeds. */
  responseTurnMetric?: {
    durationMs: number;
    startedAt: string;
    completedAt: string;
  };
  /** Durable total for the whole conversation, emitted on terminal live frames.
   * Null when the conversation has no measurement at all — "unknown", not zero. */
  responseTurnDurationTotalMs?: number | null;
  /**
   * Provider-attested final-answer marker retained only while loading history.
   *
   * Codex sets it from the rollout `phase`. It is descriptive only: the timing
   * sidecar joins on the durable message id and never consults this flag, so
   * nothing may start depending on it for repair without wiring it first
   * (B-822 — the previous comment claimed a repair path that did not exist).
   */
  ...
  /**
   * The model identifier that produced this assistant message, verbatim from
   * the provider (B-352). Present on assistant-authored text/thinking rows on
   * both the live path and history loads; absent on user rows, on provider
   * events, and wherever the provider does not name a model.
   *
   * A session is NOT one model: the engine axis (ADR-037/ADR-073) is stamped
   * per turn, so one transcript can hold kimi-k3 turns followed by
   * claude-opus-5 ones. Attribution therefore belongs on the message, exactly
   * like `coordinatorId` — a session-level model field would be a guess.
   */
  model?: string;
  /**
   * Canonical provider transcript UUID for this assistant turn. This is NOT the
   * UI envelope id: only Claude's persisted primary assistant rows populate it,
   * so a client can request a server-validated fork at that exact reply.
   */
  ...
  agentsTotal?: number;
  images?: unknown;
  /** Number of transcript-embedded images omitted by provider history safety caps. */
  ...
  text?: string;
  /**
   * Raw elapsed work time for a transient provider status event.  UI surfaces
   * must format this number through the shared duration formatter rather than
   * parsing `text`, whose wording is provider-specific.
   */
  elapsedMs?: number;
  tokens?: number;
  ...
  reason?: string;
  newSessionId?: string;
  /** الجلسة الأمّ لحدث `session_created`: معرّفها عند الاستئناف، وnull لمحادثة وُلدت الآن (B-426). */
...
export type FetchHistoryOptions = {
  /** Explicit owner for bounded native history reads; never shared across requests. */
  historyLease?: import('../modules/providers/services/history-budget.service.js').HistoryReadLease;
  projectPath?: string;
  limit?: number | null;
  offset?: number;
  /** Provider-owned stable anchor for loading rows before an earlier page. */
  cursor?: string;
};
...
export type FetchHistoryResult = {
  ...
  offset: number;
  limit: number | null;
  /** Opaque anchor for the next older page; null when the beginning is loaded. */
  nextCursor?: string | null;
  tokenUsage?: unknown;
  /** Sum of durable completed response durations for the whole conversation.
   * Null when nothing was ever measured; the client renders that as unknown. */
  responseTurnDurationTotalMs?: number | null;
};
...
/** Mutable aggregate byte ceiling shared by one provider-skill scan. */
export type ProviderSkillScanBudget = {
  maxBytes: number;
  bytesRead: number;
};
...
export type ProviderSkillListOptions = {
  ...
  userId?: string | number | null;
  signal?: AbortSignal;
  readBudget?: ProviderSkillScanBudget;
  scanLimits?: {
    deadline: number;
    maxEntries: number;
    maxDepth: number;
    maxBytes: number;
    onIncomplete?: (reason: 'deadline' | 'count' | 'depth' | 'bytes' | 'unreadable') => void;
  };
};
...
export type ProviderSkillCreateInput = {
  ...
  userId?: string | number | null;
  /** Refuse an existing target and publish each new directory atomically. */
  createOnly?: boolean;
};
...
export type ProviderSkill = {
  ...
  pluginName?: string;
  pluginId?: string;
  /** Native metadata preserved for consumers; does not grant or enforce invocation permission. */
...
export type ProviderAuthStatus = {
  ...
  error?: string;
  /**
   * وضع الاتصال المكتشَف: `'api_key'` مفتاح API مخزَّن، `'subscription_oauth'`
   * توكن OAuth صالح من CLI المزوّد، `'both'` الاثنان معاً. `null` = لا وضع
   * مكتشَف بعد. لا يُستنتج — يُكتشف من القرص أو المخزن.
   */
  authMode?: 'api_key' | 'subscription_oauth' | 'both' | null;
  /**
   * موعدُ انقطاع الربط، أو `null`/غياب إن لم يكن هناك ما يُقال — صمتٌ لا صفر.
   * يُعرض في كل حال لا عند الخطر وحده: حجبُه خارج نافذة التحذير كان يجعل
   * تسجيلَ دخولٍ ناجحاً يبدو بلا أثر، إذ يختفي السطرُ بدل أن يُظهر الموعد الجديد.
   * و`daysLeft` هو ما تُبنى عليه نبرةُ العرض عند العميل.
   *
   * الفرقُ الذي يقوم عليه هذا الحقل (B-586): ملفُّ اعتماد كلود يحمل ميقاتَين لا
   * واحداً. `expiresAt` عمرُ **توكن الوصول** — ثماني ساعاتٍ متدحرجة يجدّدها الـCLI
   * صامتاً، فلا يقرّر شيئاً عن الربط. و`refreshTokenExpiresAt` موعدُ انقطاع توكن
   * التحديث (‏≈ثلاثون يوماً من أول تسجيل دخول)، وعنده يلزم `claude /login` يدوياً.
   * وهو رقمٌ **يُعاد حسابه عند كل تجديد** من متبقٍّ يرسله الخادم (`refresh_token_expires_in`)
   * فيبقى مشيراً إلى الموعد نفسه؛ ولا يُحمَل القديمُ إلا احتياطاً حين يُغفله الخادم.
   * فلا يتمدّد بالاستعمال — وهذا ما يفرّقه عن `expiresAt`. قِيس 2026-08-07 على
   * ثلاثة اعتمادات حيّة: الأول ثابتٌ على 8.0000 ساعة في كلّها، والثاني يتراوح
   * بين 0.67 و24.5 يوماً.
   *
   * وهو إشعارٌ لا بوّابة: انقضاؤه وحده لا يقطع الاتصال ما دام توكنُ الوصول حيّاً
   * — الـCLI نفسه لا يقارنه بالساعة إلا في دالّة عرض التحذير.
   *
   * وهو خاصٌّ بكلود عمداً ولا يُعمَّم: لا مزوّدَ آخر يُصدِّر ميقاتاً لتوكن التحديث
   * (كودكس وأنتيجرافيتي يُخفيانه، وكيمي يُصدِّر توكن وصولٍ عمرُه دقائق) — فأيُّ
...
/**
 * تحذيرُ قربِ انقطاع الربط. `daysLeft` مُقرَّبٌ لأعلى (يوم كامل) مطابقةً لما
 * يعرضه الـCLI نفسه في الطرفية، فلا يرى العضو رقمين مختلفين لنفس الحقيقة.
 */
export type ProviderLinkExpiry = {
  /** ISO-8601 */
  expiresAt: string;
  daysLeft: number;
};
 ...
 *
 * `assistantMessageId` هو **`raw.uuid` المجرّد** لسطر الرسالة النهائية في سجل
 * Claude JSONL — أي ما تحمله `message.transcriptMessageId` في الواجهة (وهو
 * مفتاح البحث المفضَّل هناك: `transcriptMessageId ?? message.id`)، **وليس**
 * `message.id` المطبَّع (‏`${raw.uuid}_${partIndex}` لـClaude). مصدره: للأدوار
 * ذات صف مقياس (‏`response_turn_metrics`, ADR-126) يُطبَّع مُعرِّفه المخزَّن
 * المطبَّع إلى هذا الـUUID المجرّد؛ ولما قبل الميزة يكون `raw.uuid` لآخر رسالة
 * مساعد في المقطع الواقع بين رسالتَي مستخدم بشريّتين.
 *
 * `startedAt`/`completedAt` من نافذة صف المقياس إن وُجد، وإلا أدنى/أقصى طابع
 * زمني لطلبات الدور. `models` كل النماذج التي أجابت في الدور مرتّبةً. `costUsd`
 * **أرضية مسعّرة** تحاكي `totalUsd`: مجموع كلف النماذج المسعّرة في الدور، مع
 * تجاهل ما لا سعر رسمي له (لا يُصفَّر ولا يُبطِل الدور)، و`null` فقط حين لا
 * نموذج مسعّر البتّة فيه. مجموع توكنات `turns` يساوي `perModel` تماماً، ومجموع
 * كلفها يساوي `totalUsd` — كل طلب فريد يُنسَب إلى دور واحد لا غير.
 */
 ...
 *
 * Four fields carry the honesty guarantees and must never be collapsed:
 ...
 *  • `snapshotStatus` + `snapshotAsOf` — freshness/completeness of transcript
 *    measurement, independently of whether every model has an official price.
 *
...
export type SessionCostSummary = {
  ...
  /** Pricing coverage only: false means at least one model has no known price. */
  complete: boolean;
  ...
  /**
   * تفصيل الكلفة لكل دور ردٍّ، مرتّباً زمنياً. **اختياري ومتوافق مع العملاء
   * الحاليين**: يغيب في مسارات القواعد (opencode/hermes) وعند الاتصال بخادم
   * أقدم، فتتجاهله الواجهة دون تغيّر في السلوك. حين يحضر، مجموع توكناته يطابق
   * `perModel`. الشرح الكامل للحقول في `SessionCostTurn`.
   */
...
/**
 * One quota window as a provider itself reports it (`GET /api/providers/:provider/quota`).
 *
 * `key` is the provider's OWN name for the window (`primary`/`secondary` for
 * Codex, `tokens1`/`tokens2`/`tools` for GLM) — deliberately not "weekly" or
 * "5-hour": the window length arrives in `windowSeconds` when the provider sends
 * it, and a hard-coded label would lie on a plan measured differently.
 *
 * `usedPercent` is CONSUMED percent (0-100), clamped. Verified against GLM's own
 * payload, where the tool row states it outright: currentValue=2 / usage=100 /
 * remaining=98 / percentage=2.
 */
export type ProviderQuotaWindow = {
  key: string;
  usedPercent: number;
  /** ISO 8601 instant the window resets. A window without one is dropped, never zero-filled. */
  resetsAt: string;
 ...
 *
 * Carries no identity. For Codex, the official usage response may also contain
 * a prepaid extra-usage credit balance. When its shape is complete, only that
 * bounded display datum crosses this contract; account identifiers, email, and
 * the provider's raw `credits` object never do.
 */
export type ProviderQuotaWindows = {
  provider: string;
  /** Plan label as the provider names it (`plus`, `lite`, `pro`…), or null. */
  plan: string | null;
...
/**
 * Response row of `GET /api/providers/costs/cycle`.
 *
 * The BILLING CYCLE only — deliberately no amount, no `metered`, no
 * `available`/`reason`. This is the cheap half of the subscription surface:
 * `/costs/subscriptions` scans every transcript in the cycle window (measured
 * ~14s per call on live data) and therefore may not back an always-visible
 * component; the anchor + window need no transcript at all.
 *
 * Keyed by HARNESS provider (`claude`, `codex`, …) — the same string the UI
 * already holds as the session/global provider — NOT by vendor. That is what
 * lets the header look a row up without re-deriving the vendor client-side.
 *
 * `anchorSource` is REQUIRED and never normalized: `unknown`/`derived` mean the
 * window is an assumption, and a consumer that renders a renewal date from one
 * of those is inventing a date. The gate lives in the consumer, so the honesty
...
export type ProviderSubscriptionCost = {
  ...
  unpricedModels: string[];
  /**
   * الرصيد المتبقّي لدى المزوّد حين يعلنه رسمياً (‏moonshot عبر
   * `/v1/users/me/balance`). `null`/غياب = لا مصدر أو تعذّرت القراءة — ولا
   * يُلفَّق صفراً، لأن صفر الرصيد يُقرأ «نَفِد» وهو ادّعاء مختلف تماماً.
   *
   * هذا **ليس** `totalUsd`: ذاك ما أُنفِق في الدورة محسوباً من السجلّ، وهذا ما
   * بقي في الحساب مقروءاً من المزوّد. اجتماعهما في البطاقة مقصود — الأول يقول
   * «كم كلّفك» والثاني «كم يتبقّى لك».
   */
...
/** A known usage source that cannot currently be attributed to project cost. */
export type ProjectStatsGap = {
  harness: string;
  reason: string;
};
...
export type ProjectStatsSummary = {
  skills?: import('../../shared/skillObservations.js').ProjectSkillProjection;
  projectId: string;
  totalUsd: number | null;
  complete: boolean;
  unpricedModels: string[];
  ...
  /** Lifetime days with recorded usage; independent from a narrowed chart window. */
  activeDays: number;
  /** `YYYY-MM-DD` of the first/last day with activity; null when the ledger is empty. */
  firstActivity: string | null;
  lastActivity: string | null;
  /** Latest activity day represented by the persisted usage ledger. */
  dataThrough: string | null;
  /** Latest activity day in the indexed project sessions, independent from ledger activity. */
