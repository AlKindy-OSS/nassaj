...
// B-456 made every resume fail closed on an absent server pin. Give this B-222
// suite a real isolated sessions row so it continues testing engine-key routing,
// not the earlier pin gate.
const originalDatabasePath = process.env.DATABASE_PATH;
const databaseDir = fs.mkdtempSync('/var/tmp/b222-engine-db-');
...
async function runSideQuery(params: Record<string, unknown>, serverPin: string) {
  database.sessionsDb.setSessionEnginePin(LIVE_SID, serverPin, 'user_switch', { intent: true });
...
test('B-222: an engine with NO stored key fails visibly — the run is never constructed', async () => {
  const { errors, completed } = await runSideQuery({ engineProvider: 'glm' }, 'glm');
...
test('B-222: the silent-fallback env is never produced — no official-Anthropic run for a keyless engine', async () => {
  await runSideQuery({ engineProvider: 'kimi' }, 'kimi');
  assert.equal(lastQueryArg, null, 'no query at all — not one pointed at api.anthropic.com');
});

test('B-222: a pinned DeepSeek engine without a key fails visibly instead of running on Anthropic', async () => {
  const { errors } = await runSideQuery({ engineProvider: 'deepseek' }, 'deepseek');
  assert.equal(lastQueryArg, null, 'a keyless pinned engine never silently downgrades to the official path');
...
test('B-222: an engine WITH a stored key routes the run to that engine endpoint', async () => {
  ...
  const { errors, options } = await runSideQuery({ engineProvider: 'glm' }, 'glm');
...
test('B-222/z.ai: a long-stream timeout is applied for engine runs and stays overridable', async () => {
  ...
  const injected = await runSideQuery({ engineProvider: 'glm' }, 'glm');
  ...
  const overridden = await runSideQuery({ engineProvider: 'glm' }, 'glm');
...
test('B-222: NO engine requested ⇒ the official Claude path runs exactly as before', async () => {
  const { errors, completed, options } = await runSideQuery({}, 'anthropic');
