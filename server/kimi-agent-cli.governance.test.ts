...
describe('KM-5 badge honesty — kimi governance badge is honest (E)', () => {
  // ADR-093 §2.3-1 (T-1195) CORRECTED this expectation. The old assertion
  // (enforced:false / mechanism:'none') described kimi's RUNTIME OBEDIENCE — the CLI
  // has no native permission-bypass block (KM-4) — but the badge does not answer
  // that question: it answers "is this engine's launch gated on authentic nassaj
  // governance". For kimi it IS, fail-closed, on the live agent path traced above
  // (prepareKimiAgentLaunch → ensureVendorCliGovernance, which throws). Declaring
  // 'none' told the owner an engine that REFUSES to start ungoverned was ungoverned.
  it('providerGovernanceService returns enforced:true, mechanism:kimi-agents for kimi', async () => {
    ...
    assert.equal(
      result.enforced,
      true,
      'the kimi launch gate is fail-closed (VendorGovernanceMissingError), so the badge says so',
    );
    assert.equal(
      result.mechanism,
      'kimi-agents',
      'kimi ingests the neutral AGENTS.md COPY from KIMI_CODE_HOME',
    );
...
// ---------------------------------------------------------------------------
// (F) EXEMPTION — the per-engine governance switch (owner decision 2026-08-08)
// ---------------------------------------------------------------------------
describe('KM-5 governance gate — a user exemption skips the gate (F)', () => {
  // The POSITIVE half of the acceptance criterion: an exemption is a CHOICE, not
  // a fault, so the turn must launch. Proved against the case (A) above already
  // establishes is fail-closed — the neutral source is deliberately absent, so
  // the real gate WOULD throw. If the exemption merely caught that error instead
  // of skipping the gate, this test would still pass while the gate's self-heal
  // silently rewrote the very file the user asked to have removed; so the
  // assertion is that the gate is NOT INVOKED AT ALL.
  it('does not invoke the gate, and launches, when the engine is exempt', () => {
    const kimiHome = path.join(sandbox, 'kimi-home-F');
    fs.mkdirSync(kimiHome, { recursive: true });
    ...
    const prepared = prepareKimiAgentLaunch(
      ...
      {
        ...allPassDeps(kimiHome, path.join(sandboxHome, '.claude', 'AGENTS.md')),
        // The REAL gate would throw here: the source does not exist.
        ensureVendorCliGovernance: () => {
          gateCalls += 1;
    ...
    assert.equal(gateCalls, 0, 'an exempt engine must SKIP the gate, not catch its throw');
    assert.ok(prepared.launch, 'the launch must be prepared — an exemption never blocks');
    assert.equal(prepared.kimiHome, kimiHome);
    // The rest of the seam is untouched: the exemption drops nassaj INSTRUCTIONS,
    // never the isolation or the secret hygiene around the spawn.
    assert.equal(prepared.env.ANTHROPIC_API_KEY, undefined, 'sanitization still applies');
    assert.equal(prepared.env.KIMI_API_KEY, 'kimi-target-api-key-GOVERNANCE');
  });

  it('still refuses an ungoverned launch when the engine is NOT exempt', () => {
    const kimiHome = path.join(sandbox, 'kimi-home-F2');
    fs.mkdirSync(kimiHome, { recursive: true });
    ...
    assert.throws(
      ...
      (err: unknown) => err instanceof VendorGovernanceMissingError,
      'without an exemption the gate must still be fail-closed',
    );
    assert.equal(gateCalls, 1, 'the gate must run for a governed engine');
  });
});
