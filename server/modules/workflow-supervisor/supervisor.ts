...
import { acquireApplicationWriterLease } from '@/services/update-writer-lease.js';
...
type WorkflowWriterLease = { release(): void };
const workflowWriterLeases = new Map<string, WorkflowWriterLease>();
const workflowPermissionExecutions = new Map<string, PermissionExecutionHandle>();

/** Release only after systemd reports the launched unit terminal. */
async function releaseTerminalWorkflowWriterLeases(): Promise<void> {
  for (const [unit, lease] of workflowWriterLeases) {
    const state = await systemctlShowState(unit);
    if (state === 'inactive' || state === 'failed' || state === 'gone') {
      const execution = workflowPermissionExecutions.get(unit);
      if (execution) {
        try {
          execution.settle(
            state === 'failed'
          ...
          // ADR-134 gateway durably blocks the generation on terminal ambiguity.
        }
        workflowPermissionExecutions.delete(unit);
      }
      lease.release();
      workflowWriterLeases.delete(unit);
    }
  }
}
...
const tierBDeps: TierBDeps = {
  ...
  verifyDeliveryTarget,
  runResumeTurn: async () => {
    // ADR-134 forbids turning a persisted human row into an internal-service
    // principal. Tier-B remains unavailable until a registered, purpose-limited
    // service principal is introduced by its own decision.
    return {
      ok: false, exitCode: null, timedOut: false, resultObj: null,
      error: 'PERMISSION_INTERNAL_SERVICE_PRINCIPAL_REQUIRED',
    };
  },
...
async function handleIntentFile(
  ...
  let writerLease: WorkflowWriterLease;
  try {
    writerLease = await acquireApplicationWriterLease('workflow-cycle', { waitMs: 100 });
  } catch {
    // Leave the durable intent queued. It will be retried after maintenance.
    return 'queued';
  }

  let launchedUnit: string | null = null;
  ...
  const outcome = await processIntent(raw, {
    ...
    listAllActiveScopes,
    launchScope: async () => {
      throw new Error('PERMISSION_INTERNAL_SERVICE_PRINCIPAL_REQUIRED');
    },
  ...
  if (launchedUnit) {
    const replaced = workflowWriterLeases.get(launchedUnit);
    replaced?.release();
    workflowWriterLeases.set(launchedUnit, writerLease);
  } else {
    writerLease.release();
  }
...
export async function runSupervisor(): Promise<void> {
  ...
  while (true) {
    ...
    await releaseTerminalWorkflowWriterLeases();
    try {
