...
function injectDeps(env: NodeJS.ProcessEnv, runResumeTurn: (params: ResumeTurnParams) => Promise<ResumeTurnResult>) {
  return {
    env,
    runResumeTurn,
    resolveProfile: async (input: { baseEnv?: NodeJS.ProcessEnv }) => ({
      env: { ...(input.baseEnv ?? {}) },
...
test('happy path: one leaf-only turn, ledger committed, spend recorded, ref+tools in prompt', async () => {
  ...
  try {
    ...
    const r = await injectForConversation(injectDeps(env, stub.run), input);
...
test('resume profile unknown/ambiguous/read failure denies before the handoff child', async () => {
  for (const code of ['ENGINE_PIN_UNKNOWN', 'ENGINE_PIN_AMBIGUOUS', 'ENGINE_PIN_READ_FAILED']) {
    const root = await mkdtemp(path.join(tmpdir(), `inj-pin-${code}-`));
    const env = envFor(root);
    let turns = 0;
    let profileInput: Record<string, unknown> | null = null;
    try {
      const input = inputFor(root, 'conv-pin', [mkTask(root, `task-${code}`)]);
      const result = await injectForConversation({
        env,
        runResumeTurn: async () => {
          turns += 1;
      ...
      assert.equal(result.event, 'denied');
...
test('exactly-once: a second pass after delivery is a ledger-hit no-op', async () => {
  ...
  try {
    ...
    const stub = makeStub(input.jsonlPath);
    await injectForConversation(injectDeps(env, stub.run), input);
    // Same task again (fresh InjectTaskInput, same id) ⇒ nothing to do.
    const r2 = await injectForConversation(injectDeps(env, stub.run), inputFor(root, 'conv3', [mkTask(root, 'o1')]));
