...
test('(c) coalesced crash: WHOLE batch committed, ledger absent ⇒ all repaired, NO re-turn, no double', async () => {
  ...
  try {
    ...
    const r = await injectForConversation(injectDeps(env, run), input);
    ...
    const r2 = await injectForConversation(injectDeps(env, run), inputFor(root, 'convK', [mkTask(root, 'k1'), mkTask(root, 'k2'), mkTask(root, 'k3')]));
...
test('(d) concurrent race on ONE conversation ⇒ injector flock serializes: one delivers, one defers, charged once', async () => {
  ...
  try {
    ...
    const [ra, rb] = await Promise.all([
      injectForConversation(injectDeps(env, run), inA),
      injectForConversation(injectDeps(env, run), inB),
    ]);
