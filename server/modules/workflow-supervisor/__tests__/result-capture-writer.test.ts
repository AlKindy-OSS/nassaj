...
/**
 * The T-819 spike evidence is OPERATOR research material, not product code, so it
 * is excluded from the public export. These tests deliberately run on those REAL
 * captured bytes (feedback_synthetic_fixtures_false_confidence) — so when the
 * evidence is absent the honest outcome is an explicit SKIP, never a synthetic
 * substitute that would prove nothing about real captured output.
 */
const NO_FIXTURES = existsSync(FIXTURE_DIR)
  ? undefined
  : `no T-819 spike fixtures in this checkout (operator evidence, excluded from the public export)`;
...
test('seal: clean exit renames .partial to a BYTE-IDENTICAL result.json + writes DONE last', { skip: NO_FIXTURES }, async () => {
...
test('seal: a non-zero exit NEVER produces result.json (partial stays) and DONE carries the code', { skip: NO_FIXTURES }, async () => {
...
test('seal: re-sealing a clean exit is idempotent (result.json + DONE remain, no throw)', { skip: NO_FIXTURES }, async () => {
