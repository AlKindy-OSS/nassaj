 ...
 *
 * WHY A PER-USER CAP (not a single global lock)
 * ---------------------------------------------
 * A global lock admits ONE cycle at a time and has no notion of a per-user
 * count. Interactive, multi-user workflows need concurrency > 1, so unbounded
 * parallel `claude -p` scopes would exhaust host memory (the 2026-06-06 OOM
 * lesson). This module counts the user's currently ACTIVE scopes and rejects
 * (or lets the caller queue) an (N+1)th launch.
 *
