...
export type TierBDeps = {
  ...
  /** Test seam forwarded to the central fail-closed resume profile resolver. */
  resolveProfile?: InjectDeps['resolveProfile'];
  graceMs?: number;
  now?: () => number;
...
export async function deliverTierBOnce(deps: TierBDeps): Promise<TierBPassResult> {
  ...
  for (const [conversationId, items] of byConv) {
    ...
    const r = await injectForConversation(
      { env, runResumeTurn, resolveProfile: deps.resolveProfile, now: deps.now, onAudit: deps.onAudit },
