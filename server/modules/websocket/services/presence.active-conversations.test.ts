 ...
 *
 *   - `total` is GLOBAL — every actually-running command of every user,
 *     regardless of visibility (frozen processes do not count as active);
...
test('frozen processes stay available to the per-session state but do not inflate the active count', async () => {
  connectedClients.clear();
  ...
  // Schedule a fresh snapshot after the observer joins.
  presence.presenceRunState({
    userId: 1,
    sessionId: 's-working',
    processState: 'frozen',
  ...
  assert.deepEqual(recipient.last?.activeConversations.byProject, [
    { projectPath: SHARED_PROJECT, count: 1 },
  ]);

  presence.presenceRunStopped({ userId: 1, sessionId: 's-working' });
  presence.presenceRunStopped({ userId: 1, sessionId: 's-frozen' });
  connectedClients.clear();
});
