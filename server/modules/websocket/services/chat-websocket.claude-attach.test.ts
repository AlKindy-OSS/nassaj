...
// (6) B-736: an active session whose primary socket has already died must let the
// reconnecting socket reclaim the writer. Differential replay has to finish
// first so the new socket never observes live output before its missing gap.
test('B-736: orphaned active claude writer replays delta before one reclaim and stays processing', () => {
  const reg = makeReplayDouble();
  const sid = 'claude-orphaned-writer';
  const events: string[] = [];
  let reconnects = 0;
  reg.open();
  reg.record({ kind: 'stream_delta', content: 'seen', provider: 'claude', sequence: 1 });
  reg.record({ kind: 'stream_delta', content: 'missing-2', provider: 'claude', sequence: 2 });
  reg.record({ kind: 'stream_delta', content: 'missing-3', provider: 'claude', sequence: 3 });

  const ws = makeFakeWs();
  const deps = depsForRegistry(reg, {
    isPrimarySocketAlive: (s: string) => {
      assert.equal(s, sid);
      return false;
    },
    attachClaudeSDKSession: (s: string, lastSeq: number, send: (p: unknown) => void) => {
      assert.equal(s, sid);
      return reg.attach(lastSeq, (payload) => {
        events.push(`replay:${(payload as { content: string }).content}`);
        send(payload);
      });
    },
    reconnectSessionWriter: (s: string) => {
  ...
  assert.deepEqual(events, ['replay:missing-2', 'replay:missing-3', 'reconnect']);
  assert.equal(reconnects, 1, 'orphaned writer is reclaimed exactly once');
  assert.deepEqual(
    ws.sent.filter((m: any) => m.kind === 'stream_delta').map((m: any) => m.content),
    ['missing-2', 'missing-3'],
    'new socket receives only seq > lastSeq before the reclaim',
  );
  const status = ws.sent.find((m: any) => m.type === 'session-status');
  assert.equal(status.isProcessing, true, 'stream remains processing after writer reclaim');
  assert.equal(reg.isActive(), true, 'active stream is not aborted during reclaim');
});

// A failed reclaim must not undo the replay or report an active run as idle: the
// SDK can reject the swap after the previous socket was found dead.
test('B-736: failed orphaned-writer reclaim preserves replay and active status', () => {
  const reg = makeReplayDouble();
  const sid = 'claude-orphaned-reclaim-false';
  let reconnects = 0;
  reg.open();
  reg.record({ kind: 'stream_delta', content: 'old', provider: 'claude', sequence: 1 });
  reg.record({ kind: 'stream_delta', content: 'replayed', provider: 'claude', sequence: 2 });

  const ws = makeFakeWs();
  const deps = depsForRegistry(reg, {
    isPrimarySocketAlive: () => false,
  ...
  assert.equal(reconnects, 1, 'failed reclaim is attempted once only');
  assert.deepEqual(
    ws.sent.filter((m: any) => m.kind === 'stream_delta').map((m: any) => m.content),
    ['replayed'],
    'differential replay remains valid even when reclaim returns false',
  );
  const status = ws.sent.find((m: any) => m.type === 'session-status');
  assert.equal(status.isProcessing, true, 'failed reclaim does not mark the live stream idle');
  assert.equal(reg.isActive(), true, 'failed reclaim does not abort the stream');
});

// Keep the original no-swap veto explicit for the complementary live-primary
// case; a future refactor must not turn every active reconnect into a reclaim.
test('B-736: active claude session with live primary socket never swaps writer', () => {
  const reg = makeReplayDouble();
  const sid = 'claude-live-primary';
  let reconnects = 0;
  reg.open();
  reg.record({ kind: 'stream_delta', content: 'delta', provider: 'claude', sequence: 1 });
  ...
  assert.equal(reconnects, 0, 'live primary socket keeps the active writer intact');
  assert.deepEqual(
    ws.sent.filter((m: any) => m.kind === 'stream_delta').map((m: any) => m.content),
    ['delta'],
    'live-primary guard does not suppress differential replay',
  );
  const status = ws.sent.find((m: any) => m.type === 'session-status');
