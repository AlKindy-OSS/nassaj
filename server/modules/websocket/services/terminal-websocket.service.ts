...
export function handleTerminalConnection(
  ...
  // Mirror of the project shell: announce a denial ONCE per admission cycle,
  // not once per inbound frame (this lease wraps `input` too). Re-armed by the
  // next ADMITTED frame, so a gate that cycles its lock still yields one notice
  // per cycle — a large reduction, not a guarantee of exactly one.
  ...
  ws.on('message', async (rawMessage: RawData) => {
    ...
    try {
      ...
      // Classify before blaming the gate: this catch wraps the whole frame
      // handler, so an ordinary handler rejection reaches it too and must not be
      // reported as maintenance. The frame is refused in both branches.
      const refusal = reportWriterLeaseRefusal('Terminal WebSocket rejected: frame', frameError, {
