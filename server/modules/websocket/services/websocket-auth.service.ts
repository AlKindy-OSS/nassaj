...
function classifyWsRejection(
  ...
  try {
    // SEC-JWT-ALG: pin HS256, mirroring middleware/auth.js. Without it, verify
...
export function verifyWebSocketClient(
  ...
  try {
    ...
    if (process.env.NASSAJ_UPDATE_MODE === 'local-main' && dependencies.canAcceptApplications?.() !== true) return false;
    if (admission && (admission.phase !== 'serving' || dependencies.canAcceptApplications?.() !== true)) return false;
  } catch { return false; }
