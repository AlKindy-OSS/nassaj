...
type PresenceRun = {
  ...
  /**
   * B-861 memo: the project path the session's SIDEBAR ROW lives under, once
   * resolved. Only a found row is memoised — a run registered before the row
   * exists must keep asking, or it would stay invisible for its whole life.
   */
  rowProjectPath?: string;
 ...
 *
 * `total` is GLOBAL across provider commands that are actually executing
 * (`state === 'running'`). A frozen child remains in `runningSessions` so its
 * conversation can show the paused state, but it is not an active conversation
 * and must not inflate the "N active" badge. `byProject` is filtered to the
 * recipient's visible projects only; everything not surfaced there (private
 * project runs the recipient may not see + runs with no resolved project path)
 * is absorbed into `hiddenCount`, which therefore satisfies
...
/**
 * B-861 — the project path a run must be JUDGED by: the one its sidebar row
 * lives under, not the directory the process happens to execute in.
 *
 * A session launched into a session overlay runs with cwd
 * `<repo>/.git/nassaj-session-overlays/instances/<id>/workspace`, and every
 * provider hands that cwd to `registerSessionProcess`. That path is not a
 * project, so the visibility filter below judged the run invisible: it was
 * counted globally in `activeConversations.total` but dropped from `byProject`
 * (landing in `hiddenCount`) and — the visible symptom — dropped from
 * `runningSessions` entirely, so no client ever learned the session id and NO
 * ROW could light up. The sidebar read "4 active" with nothing marked.
 *
 * `sessions.project_path` is the same column the sidebar groups rows by, so
 * resolving through it makes the count and the row indicators agree by
 * construction: a run is surfaced exactly when its row is.
 *
 * Falls back to the reported path when no row exists yet (a brand-new session
 * whose row the synchronizer has not written): the old behaviour, never worse.
 */
function visibleProjectPathForRun(run: PresenceRun): string | null {
  if (run.rowProjectPath !== undefined) {
    return run.rowProjectPath;
  }
  let rowPath = '';
  try {
    const row = sessionsDb?.getSessionById?.(run.sessionId);
    rowPath = typeof row?.project_path === 'string' ? row.project_path.trim() : '';
  } catch {
    // Presence must keep broadcasting even if the lookup fails.
    rowPath = '';
  }
  if (rowPath.length === 0) {
    return run.projectPath;
  }
  run.rowProjectPath = rowPath;
  return rowPath;
}
...
function buildSnapshot(visiblePaths: Set<string>): PresenceEntry[] {
  ...
  for (const state of users.values()) {
    ...
    const activePath = active ? visibleProjectPathForRun(active) : null;
    const activeVisible =
      active !== null &&
      (activePath === null || visiblePaths.has(activePath));
    ...
    entries.push({
      ...
      activeProjectPath: surfacedActive ? activePath : null,
...
/**
 * Builds the active-conversations detail for ONE recipient from every currently
 * executing run. Unlike buildSnapshot (which surfaces a single headline run per
 * user), this counts every `running` command so the global `total` matches the
 * badge. Frozen processes are intentionally excluded: they are still live and
 * appear in `runningSessions`, but are not doing work now.
 *
...
function buildActiveConversations(visiblePaths: Set<string>): ActiveConversations {
  ...
  for (const state of users.values()) {
    for (const run of state.runs.values()) {
      ...
      total += 1;
      const path = visibleProjectPathForRun(run);
 ...
 */
/**
 * ‏B-577 — دلتا حالة المحادثة: حكمٌ واحد إلى من يراه، لحظةَ وقوعه.
 *
 * منفصلةٌ عن لقطة presence عمداً: تلك مجموعةٌ حيّة صغيرة تُبنى لكل مستخدم عند
 * كل نبضة، والأحكام مجموعةٌ تاريخية تنمو (مئات المحادثات) — فحشرُها هناك يعني
 * حمولةً بلا سقف وقراءةَ قاعدةٍ على أسخن مسار. وهذه O(1): جلسةٌ واحدة.
 *
 * والتصفية عند الإرسال لا في العميل، بنفس آلية `broadcastNow`: الحكمُ لا يُرسل
 * إلا لمن يرى مشروع جلسته. ولا يُرسل شيءٌ عن جلسةٍ لا مسارَ لها ولا صفَّ —
 * فشلٌ مغلقٌ عند الجهل.
 */
export function broadcastSessionOutcome(sessionId: string): void {
  ...
  try {
    ...
    console.error('[presence] failed to resolve session outcome:', error);
    return;
  }
  ...
  const visibilityCache = new Map<number | null, boolean>();

  connectedClients.forEach((client) => {
    if (client.readyState !== WS_OPEN_STATE) return;
    const recipientId = toRecipientUserId(client.userId);
    let visible = visibilityCache.get(recipientId);
    if (visible === undefined) {
      visible = new Set(projectsDb.getVisibleProjectPaths(recipientId)).has(projectPath);
      visibilityCache.set(recipientId, visible);
    }
    if (!visible) return;
    try {
      client.send(payload);
