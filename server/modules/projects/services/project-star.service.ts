...
/**
 * Sets the shared project-star state explicitly.
 *
 * Unlike {@link toggleProjectStar}, retrying the same request cannot invert the
 * result. The explicit value is written on every call so concurrent opposite
 * requests retain normal last-write-wins semantics instead of a stale read
 * incorrectly suppressing one writer.
 */
export function setProjectStar(projectId: string, starred: boolean): SetProjectStarResult {
  const normalizedProjectId = normalizeProjectId(projectId);
  if (!normalizedProjectId) {
    throw new AppError('projectId is required', {
      code: 'PROJECT_ID_REQUIRED',
