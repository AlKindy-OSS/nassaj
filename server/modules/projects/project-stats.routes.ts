...
import { getCodebaseStats } from '@/modules/projects/services/codebase-stats.service.js';
...
import { costLedgerService, readProjectSkills, PRICES_AS_OF, vendorDisplayName, type VendorKey } from '@/modules/providers/index.js';
import type {
...
function readGapRows(value: unknown): ProjectStatsGap[] {
  const rows: ProjectStatsGap[] = [];
  for (const entry of toArray(value)) {
    const row = asRecord(entry);
    const harness = readNonEmptyString(row.harness);
    const reason = readNonEmptyString(row.reason);
    if (harness !== null && reason !== null) rows.push({ harness, reason });
  }
  return rows;
}
...
function readDailyPoints(value: unknown): ProjectStatsDailyPoint[] {
  ...
  for (const entry of toArray(value)) {
    ...
    if (day !== null && costUsd !== null) {
      const complete = readComplete(row.complete);
      points.push({
        day,
        costUsd,
        complete,
        unpriced: readBoolean(row.unpriced),
        assumed: readBoolean(row.assumed),
      });
    }
  }
  return points;
...
function readVendorRows(value: unknown): ProjectStatsVendorRow[] {
  ...
  for (const entry of toArray(value)) {
    ...
    const unpriced = readBoolean(row.unpriced);
    const totalUsd = readNullableNumber(row.totalUsd);
    rows.push({
      vendor,
      ...
      totalUsd: unpriced && totalUsd === 0 ? null : totalUsd,
      requests: readCount(row.requests),
      complete: readComplete(row.complete),
      unpriced,
      assumed: readBoolean(row.assumed),
    });
  }
...
function readProjectAgents(projectPath: string): ProjectStatsAgentRow[] | null {
  ...
  return sessionAgentsDb
    ...
    .map((agent) => ({
      ...
      invocations: readCount(agent.invocation_count),
    }));
}

/** Latest active-session timestamp represented as an ISO day, or null. */
function readProjectActivityThrough(projectPath: string): string | null {
  let latest = Number.NEGATIVE_INFINITY;
  for (const session of sessionsDb.getSessionsByProjectPath(projectPath)) {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const cost: ProjectCostSummary = {
      projectId,
      totalUsd: total.measured === false ? null : readNullableNumber(total.totalUsd),
      complete: readComplete(total.complete),
      unpricedModels: readStringArray(total.unpricedModels),
      assumedModels: readStringArray(total.assumedModels),
      firstDay: readDay(total.firstDay),
      lastDay: readDay(total.lastDay),
      pricesAsOf: readNonEmptyString(total.pricesAsOf) ?? PRICES_AS_OF,
    };
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const gaps = readGapRows(stats.gaps);
    const lastActivity = readDay(total.lastDay);
    ...
    const body: ProjectStatsSummary = {
      ...
      gaps,
      daily,
      activeDays: readCount(stats.activeDays ?? total.activeDays),
      firstActivity: readDay(total.firstDay),
      lastActivity,
      dataThrough: lastActivity,
      projectActivityThrough: readProjectActivityThrough(projectPath),
      conversations: readCount(total.sessions),
      agents: readProjectAgents(projectPath),
      skills: readProjectSkills(projectPath, Number(userId), window),
...
/**
 * GET /api/projects/:projectId/code-stats
 *
 * The working tree's own measurements — size, lines, files, extension mix,
 * largest and most recently modified files (T-1169). Same read gate as every
 * route above: a caller who may not see the project gets the 404 it would get
 * for an id that does not exist, BEFORE any directory is opened.
 *
 * `?force=1` bypasses the 60s memo for the tab's refresh action. It is a GET
 * because it is a read: it mutates nothing but this process's cache, and the
 * walk is bounded (see the service header).
 */
router.get(
  '/:projectId/code-stats',
  asyncHandler(async (req: Request, res: Response) => {
    const projectId = parseProjectId(req.params.projectId);
    const userId = readAuthenticatedUserId(req);
    ...
    const codeStats = await getCodebaseStats(projectPath, { force });

    res.json({ success: true, codeStats });
  }),
);
