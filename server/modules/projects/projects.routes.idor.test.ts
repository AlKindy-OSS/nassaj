...
test('the explicit project-star contract is idempotent and validates starred', async () => {
  const firstStar = await call('POST', `/api/projects/${projectId}/toggle-star`, ownerUser, {
    starred: true,
  });
  const retryStar = await call('POST', `/api/projects/${projectId}/toggle-star`, ownerUser, {
    starred: true,
  });
  assert.equal(firstStar.status, 200);
  assert.equal(retryStar.status, 200);
  assert.equal(firstStar.json.isStarred, true);
  assert.equal(retryStar.json.isStarred, true);
  assert.equal(Boolean(projectsDb.getProjectById(projectId)?.isStarred), true);

  const firstUnstar = await call('POST', `/api/projects/${projectId}/toggle-star`, ownerUser, {
    starred: false,
  });
  const retryUnstar = await call('POST', `/api/projects/${projectId}/toggle-star`, ownerUser, {
    starred: false,
  });
  assert.equal(firstUnstar.status, 200);
  assert.equal(retryUnstar.status, 200);
  assert.equal(firstUnstar.json.isStarred, false);
  assert.equal(retryUnstar.json.isStarred, false);
  assert.equal(Boolean(projectsDb.getProjectById(projectId)?.isStarred), false);

  const invalid = await call('POST', `/api/projects/${projectId}/toggle-star`, ownerUser, {
    starred: 'true',
  });
  assert.equal(invalid.status, 400);
  assert.equal(Boolean(projectsDb.getProjectById(projectId)?.isStarred), false, 'invalid body does not write');
});
