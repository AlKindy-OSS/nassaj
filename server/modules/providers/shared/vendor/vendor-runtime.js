...
// B-395: the run-lifecycle seam lives in server/services (it wraps the /proc
// monitor, which is not a module). Same blessed cross-tree exception
// session-resources.service.js takes for the pid registry next door.
...
import { createVendorResponseModelReader } from '../response-model.js';
import { createNormalizedMessage } from '../../../../shared/utils.js';

import { VENDOR_RUNTIME } from './vendor-config.js';
import { appendVendorTranscript as appendTranscript, writeVendorTranscriptMeta as writeTranscriptMeta } from './vendor-transcript.js';
...
function createVendorSpawn(provider) {
  ...
  return async function spawnVendor(command, options = {}, ws) {
    ...
    // B-395: hosted vendors run over HTTP with no child process, so there is no
    // pid to hand the monitor — registering anyway still buys the "Running"
    // badge, the project busy dot and the active-conversations entry, which this
    // path had none of. The only thing a pid would add is frozen (kill -STOP)
    // detection, which is meaningless for an in-process fetch.
    ...
    // T-1315 (الموجة الثانية): مستوى تنسيق الجولة يصل هذه المزوّدات عبر حقل
    // `system` من صيغة Anthropic Messages — حقلٌ رسمي في الصيغة التي تخدمها هذه
    // البوّابات أصلاً (‏`/anthropic`)، لا اسمٌ مخترَع. كان الجسم بلا رسالة نظام
    // إطلاقاً، فهذه أرخص قناة وأنظفها: التوجيه لا يختلط برسالة المستخدم فلا يظهر
    // في السجلّ نصّاً كتبه ولم يكتبه.
    //
    // درجة الإنفاذ **نصّية** لا ميكانيكية: هذه نقاط نهاية إكمالٍ بلا أدوات ولا
    // وكلاء فرعيين، فلا حدَّ عمقٍ يُفرض هنا أصلاً — والواصف يقول ذلك للمستخدم.
    // The sharing guide is present even when coordination is direct.
    const coordinationDirective = getRuntimeInstructions(options?.coordinationLevel);
    const requestBody = {
      model: chosenModel,
      max_tokens: DEFAULT_MAX_TOKENS,
      stream: true,
      ...(coordinationDirective ? { system: coordinationDirective } : {}),
