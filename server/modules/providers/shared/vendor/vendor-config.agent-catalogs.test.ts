...
describe('vendor-config — chat path is NOT broken by the additive agent exports', () => {
  ...
  /**
   * B-347 and B-422: two vendors serve their model LISTING off the Anthropic
   * prefix while serving messages on it. Both were found the same way — a live
   * request with a real key — and B-422 survived B-347's repair only because
   * nothing here pinned `modelsUrl`, so the second instance of a known pattern
   * had no guard standing in front of it.
   *
   * Pinned as a PAIR per vendor, deliberately: the defect is never a wrong URL in
   * isolation, it is the two wires silently agreeing when they must not. Testing
   * either one alone would still pass with the other misrouted.
   */
  it('the catalog wire is pinned per vendor, apart from the messages wire', () => {
    // Off the /anthropic prefix — measured 404 there, 200 here.
    assert.equal(VENDOR_RUNTIME.kimi.modelsUrl, 'https://api.moonshot.ai/v1/models');
    assert.equal(VENDOR_RUNTIME.deepseek.modelsUrl, 'https://api.deepseek.com/v1/models');
    // z.ai is the exception that proves the rule: it DOES answer under the prefix.
    assert.equal(VENDOR_RUNTIME.glm.modelsUrl, 'https://api.z.ai/api/anthropic/v1/models');

    // The messages wire is untouched by that move, for every vendor.
    assert.equal(VENDOR_RUNTIME.deepseek.messagesUrl, 'https://api.deepseek.com/anthropic/v1/messages');
    assert.equal(VENDOR_RUNTIME.glm.messagesUrl, 'https://api.z.ai/api/anthropic/v1/messages');

    // …which is the whole point: for kimi and deepseek the two must DIFFER in
    // origin path, and a future edit that collapses them back is the regression.
    for (const vendor of ['kimi', 'deepseek'] as const) {
      assert.notEqual(
        new URL(VENDOR_RUNTIME[vendor].modelsUrl).pathname,
        `${new URL(VENDOR_RUNTIME[vendor].baseUrl).pathname}/v1/models`,
        `${vendor}: catalog must not be derived from the /anthropic base`,
      );
    }
  });
