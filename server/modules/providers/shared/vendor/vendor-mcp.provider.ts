...
export class VendorMcpProvider extends McpProvider {
  ...
  protected scopedConfigPath(): string {
    // Unreachable: the empty scope/transport lists reject before the base asks
    // for a transaction key. Keep a deterministic sentinel for contract parity.
    return `/nonexistent/nassaj-vendor-mcp/${this.constructor.name}`;
  }
