 ...
 *   GET /costs/cycle                    → per-provider billing cycle, NO amounts
...
import { providerRunFailuresDb } from '@/modules/database/index.js';
...
import { applySubscriptionAmountVisibility } from '@/modules/providers/services/cost/subscription-visibility.js';
...
import type { ProviderBillingCycle, ProviderSubscriptionCost, SessionCostSummary } from '@/shared/types.js';
import { AppError, asyncHandler } from '@/shared/utils.js';

const router = express.Router();

/**
 * Providers whose exhaustion nassaj can currently observe (T-1191).
 *
 * antigravity alone, and the reason is evidence rather than scope-cutting: agy
 * exposes no usage command at all, so the ONLY quota signal it emits anywhere is
 * the `Resets in …` clause on the stderr line of the run it refuses. Providers
 * with a real usage endpoint (claude, codex) report live windows through their
 * own surfaces and must not be shadowed by a stale marker recorded here.
 *
 * A provider joins this list when its adapter starts recording failures, not
 * before: listing one that never writes a row makes the endpoint promise a
 * signal it will never deliver.
 *
 * B-411 — the eligible ENGINES join on exactly those terms: `claude-sdk.js` now
 * classifies an engine-driven failure and writes a `provider_run_failures` row
 * under the engine's own id when the vendor reports an exhausted allowance, so
 * these two names are no longer promises without a writer. They are engines, not
 * bodies, and that is the point: a body reports its own usage through its own
 * surface, whereas a vendor endpoint driving the Claude body emits one HTTP
 * refusal and nothing else.
 *
 * Deliberately NOT recorded for a per-second throttle: that clears itself within
 * a moment, and a marker for it would present a working engine as a dead one.
 */
const QUOTA_BLOCK_PROVIDERS = ['antigravity', 'glm', 'kimi'] as const;
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const abort = (): void => controller.abort(new DOMException('HTTP client disconnected', 'AbortError'));
    const abortOnClose = (): void => { if (!res.writableEnded) abort(); };
    ...
    try {
      ...
      if (!controller.signal.aborted) res.json({ success: true, cost });
    } finally {
...
//
// T-1098: a SHARED provider's total is the whole team's spend (sessionScope
// returns null = no filter), so the amount is redacted here for anyone who is
// not owner/admin. Isolated rows pass through untouched — their total is already
// scoped to the reader's own conversations, which makes the reader the
// subscription holder. Enforced at the ROUTE, not in the card: hiding it client
// side would leave this endpoint answering the full figure to anyone who called
// it directly. Per-conversation costs (/costs/session/:id) and the project
// boards are deliberately NOT gated — the owner's decision was per surface.
router.get(
  '/subscriptions',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = readAuthenticatedUserId(req);
    ...
    res.json({
      ...
      subscriptions: applySubscriptionAmountVisibility(subscriptions, role),
    });
  }),
);

// The CHEAP half of the same surface: the billing cycle with no amount attached.
//
// Why a second endpoint instead of a query parameter on /subscriptions: the cost
// there is in the SCAN, not in the serialization — every measurable harness is
// walked before anything can be filtered, so `?vendor=` would have saved nothing
// (measured: ~14s per call, 54s each when four arrive together). This route
// touches no transcript at all, which is what allows an always-visible component
// (the header indicator) to consume it. It answers ONLY what the anchor knows;
// amounts stay on /subscriptions, where the user opens a section to ask for them.
router.get(
  '/cycle',
  asyncHandler(async (req: Request, res: Response) => {
    const userId = readAuthenticatedUserId(req);
    const cycles: ProviderBillingCycle[] = await providerCycleService.list(userId);

    // T-1191: quota blocks ride along on the cycle payload rather than getting
    // their own endpoint. The header surface already fetches this once and does
    // not poll, the read is a single indexed row per provider, and the two facts
    // answer the same question a user asks at that spot in the UI — "can I use
    // this provider right now, and if not, until when?".
    //
    // Only providers with a recorded exhaustion appear here; a provider that is
    // fine is simply absent rather than present with a null, so a client that
    // iterates the list can never render an empty countdown.
    const quotaBlocks = QUOTA_BLOCK_PROVIDERS.flatMap((provider) => {
      const block = providerRunFailuresDb.getActiveQuotaBlock(provider);
      return block
        ? [{ provider, resetsAt: new Date(block.resetsAtMs).toISOString(), reason: block.reason }]
        : [];
    });

    res.json({ success: true, cycles, quotaBlocks });
  }),
);
...
router.put(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // B-597: الكتابة في شجرة صاحب الطلب. المعرّف من `req.user` لا من العميل،
    // ودورةُ الفوترة شأنُ اشتراكه هو — لا إعدادَ خادمٍ يضبطه أحدُهم للجميع.
    subscriptionConfigService.update(provider, patch, userId);
