...
/**
 * DORMANCY — the bound that stops this endpoint's result set from growing forever.
 *
 * THE LEAK (measured 2026-07-29 on 35 real workflow dirs, not fixtures): 27 were
 * retired by a terminal notification and 2 were COMPLETED, but 6 sat in `unknown`
 * with NO exit at all — 0.8 to 20.2 days old, every one of them silent. `unknown`
 * is by construction unretirable: it is not COMPLETED so the drop above cannot
 * take it, and its coordinator died before writing the terminal notification that
 * `terminalWorkflowIds` looks for. Every killed coordinator therefore adds one
 * permanent resident, and the list only ever grows.
 *
 * WHY A SILENCE THRESHOLD AND NOT A DEATH CLAIM. The honest verdict for these is
 * still "liveness unproven" — nassaj cannot prove death here, and the one time it
 * guessed (incident wf_synthetic_ef5ba242) it announced a stop six minutes before the run
 * actually finished. So dormancy asserts NOTHING about the process: it says only
 * "nothing has been written anywhere in this workflow for a very long time, so it
 * is no longer worth a live indicator". The status stays `unknown`; this is a
 * SEPARATE flag, deliberately not a sixth WorkflowUiState — the UI state records
 * are exhaustive, and a new state would mean icons, colours and ten locale files
 * for what is a visibility decision, not a liveness one.
 *
 * The row is still returned in the payload (and counted in `dormant`), so this is
 * a default-hidden, not a deletion: nothing on disk is touched and a reveal
 * control is a client-only change.
 *
 * 24h, not the 15m above: that guard demotes a claim of life, which is cheap to
 * get wrong in the safe direction. This one removes an indicator, so it is set
 * far beyond any plausible long-running-but-quiet workflow.
 */
...
export type ActiveWorkflow = {
  ...
  /**
   * True when this workflow has written NOTHING anywhere for DORMANT_AFTER_MS.
   * Orthogonal to `status` (which stays `unknown`): it is a visibility signal,
   * not a liveness claim — see DORMANT_AFTER_MS. Clients hide these by default;
   * the row is still delivered so a reveal control needs no server change.
   */
  dormant: boolean;
};
...
export type ActiveWorkflowsResult = {
  ...
  capped: boolean;
  /**
   * How many of `workflows` carry `dormant: true`. Exposed so "quiet because
   * nothing is running" stays distinguishable from "quiet because N indicators
   * were hidden" — the honesty guard on default-hiding.
   */
  dormant: number;
};
...
async function scanSessionWorkflows(
  ...
  for (const entry of entries) {
    ...
    // TERMINAL SIGNAL PRECEDES EVERYTHING (measured: wf_synthetic_f66564da-ad8 finished with
    ...
    try {
      const detail = await readWorkflowAgents(path.join(workflowsDir, entry), { now });
    ...
    // Measured: wf_synthetic_1a7845b5-de1's 9th agent was interrupted mid-schema-retry and
    ...
    // that is exactly the false-stop of incident wf_synthetic_ef5ba242.
    ...
    // DORMANCY (see DORMANT_AFTER_MS): only ever applied to `unknown` — the one
    // state with no exit. `running` and `frozen` have a live process behind them,
    // and `orphan` is already a settled verdict the user is meant to act on; none
    // of the three may be hidden by a clock.
    //
    // `lastSignOfLife === 0` means neither the journal nor any agent transcript
    // was readable — no evidence of silence either, so it is NOT dormant. Absence
    // of a timestamp must not be read as an old timestamp.
    const dormant =
      status === 'unknown'
      && lastSignOfLife > 0
      && now - lastSignOfLife >= DORMANT_AFTER_MS;
...
export const workflowStatusService = {
  ...
  async getActiveWorkflows(
    ...
    try {
      ...
      const dormant = workflows.reduce((n, wf) => (wf.dormant ? n + 1 : n), 0);
      return { workflows, eligible, scanned, capped, dormant };
    } catch (error) {
