...
// eslint-disable-next-line boundaries/no-unknown
import { COMPLETED_JOURNAL_LINES, INCIDENT_JOURNAL_LINES, toJsonl } from '@/services/__tests__/__fixtures__/wf-liveness.synthetic.js';
...
test('fail-closed: userId=null returns an empty envelope and does not scan', async () => {
  await withHarness(async ({ createUser, addSessionWithWorkflow }) => {
    ...
    assert.deepEqual(result, { workflows: [], eligible: 0, scanned: 0, capped: false, dormant: 0 });
  });
});
...
test('ownership: a caller never sees another user\'s workflow', async () => {
  await withHarness(async ({ createUser, addSessionWithWorkflow }) => {
    ...
    assert.deepEqual(bobResult, { workflows: [], eligible: 0, scanned: 0, capped: false, dormant: 0 });
...
test('a session with no workflows dir is scanned but contributes nothing (no false orphan)', async () => {
  await withHarness(async ({ projectDir, createUser, addSessionWithWorkflow }) => {
    const u = createUser('u1');
    await addSessionWithWorkflow({ userId: u, sessionId: 's-empty', wfId: 'wf_synthetic_placeholder', lines: COMPLETED_JOURNAL_LINES });
...
// ── DORMANCY (the unbounded-list fix) ───────────────────────────────────────
//
// Measured on 35 real workflow dirs (2026-07-29): 27 retired by a terminal
// notification, 2 COMPLETED, and 6 stuck in `unknown` with NO exit — 0.8 to 20.2
// days old, all silent. `unknown` cannot be retired by any existing path, so
// every killed coordinator left one permanent resident. The ages below are taken
// from those six, not invented.
test('dormancy: an unknown workflow silent past the threshold is flagged, not deleted', async () => {
  await withHarness(async ({ createUser, addSessionWithWorkflow }) => {
    const u = createUser('u1');
    // 20.2 days — the age of wf_synthetic_9f6c1898-dbb / wf_synthetic_82dee2ed-6f4 on disk.
    const twentyDaysAgo = NOW_MS - Math.round(20.2 * 24 * 60 * 60 * 1000);
    await addSessionWithWorkflow({
      userId: u, sessionId: 's-dormant', wfId: 'wf_synthetic_9f6c1898-dbb',
      lines: INCIDENT_JOURNAL_LINES, mtimeMs: twentyDaysAgo,
    });
    forgetWorkflowPid('s-dormant'); // no pid ever recorded => UNKNOWN

    const result = await workflowStatusService.getActiveWorkflows(u, { now: NOW_MS, quietMs: QUIET_MS });
    assert.equal(result.workflows.length, 1, 'the row is still delivered, not dropped server-side');
    assert.equal(result.workflows[0].status, 'unknown', 'dormancy must NOT become a death claim');
    assert.equal(result.workflows[0].dormant, true);
    assert.equal(result.dormant, 1, 'the count makes the hiding non-silent');
  });
});

test('dormancy: a freshly silent unknown workflow is NOT dormant', async () => {
  await withHarness(async ({ createUser, addSessionWithWorkflow }) => {
    const u = createUser('u1');
    // Well past the 15-minute stale guard, well inside the 24-hour dormancy one:
    // this is the window where a run is quiet but still worth an indicator.
    await addSessionWithWorkflow({
      userId: u, sessionId: 's-fresh', wfId: 'wf_synthetic_1a7845b5-de1',
      lines: INCIDENT_JOURNAL_LINES, mtimeMs: NOW_MS - 3 * 60 * 60 * 1000,
    });
    forgetWorkflowPid('s-fresh');
