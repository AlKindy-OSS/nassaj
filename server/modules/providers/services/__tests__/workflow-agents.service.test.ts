 ...
 * run wf_synthetic_1a7845b5-de1: nine `started`, eight `result`, the ninth agent killed
...
describe('readWorkflowAgents', () => {
  ...
  it('reports an unfinished-but-silent agent as `stale`, never as running', async () => {
    // The exact shape that shipped a lie: the coordinator process died, so two
    // agents never wrote a `result` — and the strip rendered them as a pulsing
    // green "running · now Bash" for eighteen hours. Journal presence alone is
    // not a heartbeat.
    await fsp.writeFile(
      path.join(dir, 'journal.jsonl'),
      [started('alive'), started('dead'), result('alive')].join('\n') + '\n',
      'utf8',
    );
    await fsp.writeFile(path.join(dir, 'agent-alive.jsonl'), [promptRow('finished'), toolRow('Bash')].join('\n') + '\n', 'utf8');
    await fsp.writeFile(path.join(dir, 'agent-dead.jsonl'), [promptRow('killed mid-flight'), toolRow('Bash')].join('\n') + '\n', 'utf8');

    // Read as if 18 hours had passed since those files were written.
    const { agents } = await readWorkflowAgents(dir, { now: Date.now() + 18 * 60 * 60 * 1000 });

    const deadRow = agents.find((a) => a.agentId === 'dead');
    assert.equal(deadRow?.status, 'stale');
    assert.equal(deadRow?.currentTool, null, 'the tool it died on must not render as "now"');
    assert.equal(deadRow?.callCount, 1, 'what it really did is still reported');
    assert.equal(agents.find((a) => a.agentId === 'alive')?.status, 'done');
  });

  it('keeps a freshly-written unfinished agent `running`', async () => {
    await fsp.writeFile(path.join(dir, 'journal.jsonl'), `${started('a1')}\n`, 'utf8');
    await fsp.writeFile(path.join(dir, 'agent-a1.jsonl'), [promptRow('busy'), toolRow('Bash')].join('\n') + '\n', 'utf8');

    // 14 minutes of silence is under the threshold: one long tool call must not
    // be mislabelled as a death (the false-stop this window exists to avoid).
    const { agents } = await readWorkflowAgents(dir, { now: Date.now() + 14 * 60 * 1000 });
    assert.equal(agents[0]?.status, 'running');
    assert.equal(agents[0]?.currentTool, 'Bash');
  });

  it('treats an unfinished agent with NO transcript as stale, not running', async () => {
    await fsp.writeFile(path.join(dir, 'journal.jsonl'), `${started('ghost')}\n`, 'utf8');
    const { agents } = await readWorkflowAgents(dir);
    assert.equal(agents[0]?.status, 'stale', 'a missing file is not a heartbeat');
  });
