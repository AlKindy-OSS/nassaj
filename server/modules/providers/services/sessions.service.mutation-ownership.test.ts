...
test('B-IDOR-ARCHIVED / ADR-089: the archived listing is shared, but still needs an identity', () => {
  ...
  assert.ok(rawIds.includes(PRIVATE_SESSION), 'the raw query returns both sessions');

  // ADR-089: project visibility is retired, so a teammate sees every archived
  // session. The boundary that survives is identity itself (asserted below).
  const strangerIds = sessionsService.listArchivedSessions(strangerId).map((item) => item.sessionId);
  assert.equal(strangerIds.includes(PRIVATE_SESSION), true, 'the team shares archived sessions too');
  assert.equal(strangerIds.includes(PUBLIC_SESSION), true, 'and the public one');
