...
// HTTP status for "this member's Claude credential is missing/expired/refused".
// NEVER 401 (B-1043 / T-1699): the client treats a 401 carrying a nassaj JWT as
// a nassaj session rejection and signs the member out. A provider credential
// state is a dependency failure of the usage read, not an identity failure —
// 424 Failed Dependency keeps the member signed in and lets the usage UI show
...
class ClaudeUsageService {
  ...
  async getUsage(
    ...
    const liveRefresh = () => this.refresh(credPath);
    const refresh = (runEffect ? runEffect(liveRefresh) : liveRefresh()).finally(() => {
  ...
  private async refreshAccessToken(credPath: string, credential: OAuthCredential): Promise<OAuthCredential> {
    const inFlight = this.refreshInFlight.get(credPath);
    if (inFlight) return inFlight;

    const run = this.performTokenRefresh(credPath, credential)
      .finally(() => { this.refreshInFlight.delete(credPath); });
    this.refreshInFlight.set(credPath, run);
    return run;
  }

  /**
   * تجديدٌ واحدٌ في الطيران لكل ملفِّ اعتماد (B-579).
   *
   * التبادلُ يُدوِّر توكن التحديث: الخادم قد يُعيد `refresh_token` جديداً ويُبطل
   * سلفه. فطلبان متزامنان — والهيدرُ والشريطُ الجانبي يطلبان الاستهلاك معاً —
   * يتبادلان **نفس** التوكن مرّتين، فيُبطل الثاني ما ظفر به الأول، وأولُ تجديدٍ
   * تالٍ يعود بـ`invalid_grant` ⇒ خروجٌ نهائيٌّ يوجب `/login` يدوياً.
   *
   * والخريطةُ لكل مسارٍ لا عامّة: أعضاءٌ معزولون يجدّدون بالتوازي بلا تعارض.
   */
  private readonly refreshInFlight = new Map<string, Promise<OAuthCredential>>();

  private async performTokenRefresh(credPath: string, credential: OAuthCredential): Promise<OAuthCredential> {
    ...
    // B-578: هذا التبادل يُدوِّر توكن التحديث نفسه، فترْكُ ميقاتِه على قيمته
    // القديمة يُبقي على القرص توكناً جديداً بختمٍ يخصّ سلفه. والـCLI يُعيد حسابه
    // من ردّ الخادم عند كل تجديد، ونحن نحاكيه: يُكتب حين يُرسله الخادم، ويُترك
    // القديم حين يُغفله — وهو احتياطُ الـCLI نفسه.
    const refreshExpiresInSeconds = typeof record?.refresh_token_expires_in === 'number'
      ? record.refresh_token_expires_in
      : null;
    const refreshTokenExpiresAt = refreshExpiresInSeconds
      ? Date.now() + refreshExpiresInSeconds * 1000
      : null;

    await this.persistRefreshedCredential(credPath, {
      accessToken,
      refreshToken,
      expiresAt,
  ...
  private async persistRefreshedCredential(credPath: string, update: {
    ...
    try {
      const before = await stat(credPath);
      ...
      // B-579: الـCLI يملك هذا الملف ويكتبه استبدالاً ذرّياً (قِيس `birth == mtime`
      // في كل الاعتمادات الحيّة). فالكتابةُ في المكان كانت تفعل أمرين: تترك قارئاً
      // متزامناً على JSON مبتور، وتدهس توكناً أحدثَ كتبه الـCLI بينما كنّا نُركّب
      // حمولتنا — والمدهوسُ توكنُ تحديثٍ صالح، فيصير ما على القرص مُبطَلاً.
      //
      // فالكتابةُ الآن ذرّية (‏tmp + rename في نفس المجلّد ليكون النقل داخل نفس
      // نظام الملفات)، ومشروطةٌ بأن الملف لم يتبدّل منذ قراءتنا. وعند التصادم
      // نتنحّى ونُسجّل: توكنُ الـCLI أحدثُ من توكننا، ودهسُه هو الضرر بعينه.
      const beforeRename = await stat(credPath);
      if (beforeRename.mtimeMs !== before.mtimeMs) {
        console.warn(
          '[claude-usage] credential file changed under us; skipping write '
          + `(path=${credPath}) — the provider CLI rotated it first`,
        );
        return;
      }

      tmpPath = `${credPath}.nassaj-${process.pid}.tmp`;
      ...
      // لكن لا يُترك ملفٌّ مؤقّتٌ خلفنا في شجرة اعتماد العضو.
      if (tmpPath) {
