...
import {
  ...
  removeProviderSessionActiveModelChange,
  writeProviderSessionActiveModelChange,
} from '@/shared/utils.js';

/**
 * Authoritative provider catalogs may change as an account receives a newly
 * enabled model. One day keeps normal picker opens inexpensive while ensuring
 * those changes are discovered without a manual refresh at least daily.
 */
export const PROVIDER_MODELS_CACHE_TTL_MS = 24 * 60 * 60 * 1000;
...
/**
 * Schema/content version of the persisted catalog file. A mismatch makes
 * {@link readProviderModelsCacheFile} discard the WHOLE file, so this constant is
 * the only lever that retires already-written catalogs before their TTL.
 *
 * ⚠️ BUMP THIS whenever a model id that OUR OWN CODE declares is added, renamed or
 * REMOVED — carrier ids in particular (`GLM_CARRIER_MODELS` → `opencode.json` →
 * `glm/<id>` in the opencode listing). Deleting an id from the code does not reach
 * a catalog already on disk: entries live for {@link PROVIDER_MODELS_CACHE_TTL_MS}
 * (24 hours) under `~/.cloudcli/provider-models-cache.json`.
 *
 * B-234 (2026-07-27) — measured, not hypothetical: B-220 deleted the phantom
 * `glm-5.2[1m]` (z.ai answers `1211 Unknown Model`) from every code path and the
 * live `opencode models` listing was clean, yet the cached `opencode::user:*`
 * entries written at 09:59Z kept offering `glm/glm-5.2[1m]` until 2026-07-30. The
 * picker showed it, the operator chose it, and the run died with "OpenCode CLI
 * exited with code 1". A code-only deletion is not a deletion while this number
 * stays put. `provider-models-cache-version.test.ts` fails if the carrier catalog
 * changes without this number changing.
 *
 * 2 — B-234: retires every catalog written before the `glm-5.2[1m]` deletion.
 * 3 — B-342: retires every vendor catalog written while the live fetch could not
 *     run (identity dropped before the key lookup). Those entries are frozen
 *     fallbacks; without this bump the SWR path serves them once more on first
 *     open after the fix and the repair looks like it did nothing.
...
type ProviderModelsServiceDependencies = {
  ...
  runCatalogProbe?: typeof runAuthorizedProviderCatalog;
  cachePath?: string;
...
const buildCacheKey = (
  ...
  userId === null || userId === undefined || userId === ''
    // B-342: an identity-less lookup gets its OWN bucket, never the bare
    // provider key. The bare key used to be shared with nothing else, which was
    // fine while catalogs were identity-blind; once they became per-user, a
    // system-scoped fetch and a user-scoped fetch would have overwritten each
    // other in one entry — and the loser is served to whoever asks next, for
    // 24 hours, from a file on disk.
    ? `${provider}::system`
...
export const createProviderModelsService = (dependencies: ProviderModelsServiceDependencies = {}) => {
  ...
  const runCatalogProbe = dependencies.runCatalogProbe ?? runAuthorizedProviderCatalog;
  ...
  const triggerBackgroundRefresh = (
    ...
    void loadAndCacheModels(provider, cacheKey, userId, authenticatedPrincipal).catch(() => {
  ...
  const loadAndCacheModels = (
    ...
    const loadCatalog = () => resolveProvider(provider).models.getSupportedModels(userId ?? null);
    const request = (STATIC_CATALOG_PROVIDERS.has(provider)
      ? loadCatalog()
      : runCatalogProbe(provider, userId, authenticatedPrincipal, loadCatalog))
      .then(async (models) => {
  ...
  const getProviderModels = async (
    ...
    if (options.bypassCache) {
      ...
      return loadAndCacheModels(provider, cacheKey, userId, authenticatedPrincipal);
    }
  ...
  const getCurrentActiveModel = async (
    ...
    sessionId?: string,
    userId?: string | number | null,
  ): Promise<ProviderCurrentActiveModel> => resolveProvider(provider).models.getCurrentActiveModel(sessionId, userId);

  /**
   * Pins a session's model.
   *
   * A provider WITHOUT an active-model command of its own (hermes) answers
   * `NOT_IMPLEMENTED` here, and that 501 used to travel all the way to the UI,
   * where it became `modelSwitch.supported: false` and a hidden picker (T-1198).
   * That was a wrong reading of the constraint: the pin does not need the
   * vendor's cooperation at all. {@link resolveResumeModel} reads it back from
   * NASSAJ'S OWN provider-agnostic change store on every turn, and the spawn
   * passes it as a per-run flag — which is exactly what {@link seedSessionModel}
   * already relies on. So a refusal falls through to that store instead of
   * failing the request; a provider that DOES own its store keeps writing there.
   */
  const changeActiveModel = async (
    ...
    try {
      return await resolveProvider(provider).models.changeActiveModel(input);
    } catch (error) {
      if ((error as { code?: string })?.code !== 'NOT_IMPLEMENTED') {
        throw error;
      }
      return writeProviderSessionActiveModelChange(provider, input, {
  ...
  // Unpins a session (B-252): removes its (provider, sessionId) override so the
  // resume path stops replaying an explicit re-pick. Reports whether an override
  // actually existed and was removed; a no-op when none was stored. Writes to the
  // SAME store {@link getChangedActiveModel} reads (honouring the injected test
  // path), so a subsequent read reports `changed: false`.
  const clearChangedActiveModel = async (
    provider: LLMProvider,
    sessionId: string,
  ): Promise<{ cleared: boolean }> => removeProviderSessionActiveModelChange(provider, sessionId, {
    filePath: activeModelChangesPath,
  });
  ...
  const resolveResumeModel = async (
    ...
    try {
      const sessionModel = (await getCurrentActiveModel(provider, sessionId, userId)).model?.trim();
      if (sessionModel) {
        return sessionModel;
      }
    } catch {
  ...
  return {
    /**
     * Local catalog is overlaid after the cache so deletion/revocation is immediate.
     * B-1268: a caller with no runnable local server — which includes every caller while
    ...
    getProviderModels: async (...args: Parameters<typeof getProviderModels>): Promise<ProviderModelsResult> => {
      ...
      const localOptions = authorizedLocalModelServers(args[2]).flatMap(server => server.models.map(model => ({
        ...
        description: 'local', ...(model.contextWindow ? { contextWindow: model.contextWindow } : {}),
      })));
      const all = [...options, ...localOptions];
      return { ...result, models: { ...result.models, OPTIONS: all,
        DEFAULT: all.some(option => option.value === result.models.DEFAULT) ? result.models.DEFAULT : all[0]?.value ?? '',
      } };
    },
    getCurrentActiveModel,
    getChangedActiveModel,
    clearChangedActiveModel,
    changeActiveModel,
    resolveResumeModel,
    seedSessionModel,
    clearCache,
