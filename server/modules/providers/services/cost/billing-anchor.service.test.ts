...
test('كلود: يوم المرساة من subscriptionCreatedAt — مشتقٌّ لا مكتشَف (B-596)', async () => {
  await withTempRoot(async (root) => {
    ...
    assert.ok(anchor);
    // B-596: الحقلُ تاريخُ إنشاء الاشتراك لا موعدُ فوترته. قِيس على حسابٍ حيّ:
    // الإنشاء يوم 9 و claude.ai يعلن التجديد يوم 10 — فرقُ يوم. ووسمُ `derived`
    // هو ما يمنع `providerCycleHelpers` من عرضه رقماً مؤكّداً.
...
test('حقلُ الاشتراك يعلو على مسح السجلّات حين يتوفّر الاثنان', async () => {
  await withTempRoot(async (root) => {
    await writeJson(path.join(root, '.claude', '.claude.json'), {
    ...
    const anchor = await billingAnchorService.discover('claude', 7, { resolveEnv: envFor(root) });
    assert.ok(anchor);
    // كلاهما `derived` بعد B-596 — والمهمّ أن حقل الاشتراك يسبق أقدمَ ملفٍّ في
    // السجلّات، فلا يصير تاريخُ أول استعمالٍ يومَ فوترة.
    assert.equal(anchor.source, 'derived');
