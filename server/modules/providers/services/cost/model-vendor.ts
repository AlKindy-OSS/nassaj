...
/**
 * عكس `harnessVendor`: كل الأجسام التي يُجمَّع استهلاكها تحت صفّ هذا المورّد.
 *
 * العلاقة **ليست واحداً لواحد**: صفّ `google` وحده يضمّ `gemini` و`antigravity`
 * و`agy`. وكل من يقرّر شيئاً عن صفّ المورّد انطلاقاً من سياسةٍ مكتوبة بمفاتيح
 * الأجسام (‏`provider_sharing`) يحتاج هذه القائمة كاملةً لا مفتاحاً واحداً —
 * وإلّا حكَم على المجموع بحال جسمٍ واحد من عدّة أجسام أسهمت فيه.
 *
 * ‏`unknown` تُعيد قائمة فارغة: لا جسم معروفاً وراءها، وهو ما يجب أن يقود
 * المستهلك إلى الفرع المتحفّظ لا إلى افتراض.
 */
export const vendorHarnesses = (vendor: VendorKey | string): string[] =>
  Object.entries(HARNESS_VENDORS)
    .filter(([, mapped]) => mapped === vendor)
    .map(([harness]) => harness);
