...
test('المخزَّن يعلو على الافتراضي، والمخفيّ يظهر في القائمة لا في النشط', async () => {
  await withIsolatedDatabase(async () => {
    subscriptionConfigService.update('claude', { anchorDay: 9, plan: 'Max 20x' }, 1);
...
test('كتابة مزوّد لا تمحو جيرانه (قراءة-دمج-كتابة على مفتاح واحد)', async () => {
  await withIsolatedDatabase(async () => {
    subscriptionConfigService.update('claude', { anchorDay: 9 }, 1);
    subscriptionConfigService.update('codex', { anchorDay: 21, plan: 'Plus' }, 1);
    subscriptionConfigService.update('claude', { plan: 'Max 20x' }, 1);

    // من قاعدة البيانات نفسها لا من الكاش: الكاش قد يخفي فقداً حدث في التخزين.
    subscriptionConfigService._resetCaches();
    const stored = subscriptionConfigService.getStored(1);
...
test('التحقّق يرفض ما يُفسد الحساب بدل أن يُطبّعه صامتاً', async () => {
  await withIsolatedDatabase(() => {
    ...
    rejects(() => subscriptionConfigService.update('nope', { anchorDay: 1 }, 1), 'UNKNOWN_SUBSCRIPTION_PROVIDER');
    rejects(() => subscriptionConfigService.update('claude', { anchorDay: 0 }, 1), 'INVALID_ANCHOR_DAY');
    rejects(() => subscriptionConfigService.update('claude', { anchorDay: 32 }, 1), 'INVALID_ANCHOR_DAY');
    rejects(() => subscriptionConfigService.update('claude', { anchorDay: 9.5 }, 1), 'INVALID_ANCHOR_DAY');
    rejects(() => subscriptionConfigService.update('claude', { anchorDay: '9' }, 1), 'INVALID_ANCHOR_DAY');
    rejects(() => subscriptionConfigService.update('claude', { plan: 'x'.repeat(65) }, 1), 'INVALID_SUBSCRIPTION_PLAN');
    rejects(() => subscriptionConfigService.update('claude', { plan: 7 }, 1), 'INVALID_SUBSCRIPTION_PLAN');
    rejects(() => subscriptionConfigService.update('claude', { hidden: 'yes' }, 1), 'INVALID_SUBSCRIPTION_HIDDEN');

    // لا شيء من الرفض تسرّب إلى التخزين.
    assert.deepEqual(subscriptionConfigService.getStored(1), {});
  });
});

test('اسم خطة فارغ أو بمسافات = «بلا خطة» لا نصّ فارغ', async () => {
  await withIsolatedDatabase(() => {
    assert.equal(subscriptionConfigService.update('claude', { plan: '   ' }, 1).plan, null);
    assert.equal(subscriptionConfigService.update('claude', { plan: '  Max 20x  ' }, 1).plan, 'Max 20x');
    assert.equal(subscriptionConfigService.update('claude', { plan: null }, 1).plan, null);
  });
});
...
test('حقل مخزَّن خارج المدى يُطبَّع عند القراءة (قيمة كُتبت بيد أو بنسخة أقدم)', async () => {
  await withIsolatedDatabase(async () => {
    ...
    const settings = subscriptionConfigService.getSettings('claude', 1);
    ...
    assert.equal(Object.hasOwn(subscriptionConfigService.getStored(1), 'ghost'), false);
  });
});
...
test('اختيار المالك يعلو على المكتشَف ولا يُنقَض به', async () => {
  await withIsolatedDatabase(async () => {
    subscriptionConfigService.update('codex', { anchorDay: 3 }, 1);
...
test('إرجاع اليوم إلى null يعيد الاكتشاف بعد ضبط يدوي', async () => {
  await withIsolatedDatabase(async () => {
    subscriptionConfigService.update('codex', { anchorDay: 3 }, 1);
    assert.equal(subscriptionConfigService.getSettings('codex', 1).anchorDay, 3);

    subscriptionConfigService.update('codex', { anchorDay: null }, 1);
    assert.equal(subscriptionConfigService.getSettings('codex', 1).anchorDay, null);
...
test('خطة المالك تعلو على خطة المصدر حتى مع مرساة مكتشَفة', async () => {
  await withIsolatedDatabase(async () => {
    subscriptionConfigService.update('codex', { plan: 'Pro (فاتورة الشركة)' }, 1);
...
test('B-597: اختيارُ عضوٍ لا يُطبَّق على غيره', () => {
  return withIsolatedDatabase(async () => {
    // الاكتشافُ يقرأ شجرةَ كل عضو على حدة، فالتجاوزُ اليدوي يجب أن يُعزل مثله.
    // كان مفتاحاً واحداً مشتركاً: يضبط عضوٌ يومَ فوترته فيُحسب به الجميع.
    subscriptionConfigService.update('codex', { anchorDay: 3, plan: 'Plus (مِلكي)' }, 1);
    ...
    const mine = await subscriptionConfigService.list(1, deps);
    assert.equal(mine[0].anchorDay, 3, 'صاحبُ الضبط يراه');
    assert.equal(mine[0].anchorSource, 'manual');

    const theirs = await subscriptionConfigService.list(2, deps);
    assert.equal(theirs[0].anchorDay, 11, 'وغيرُه يعود إلى المكتشَف من شجرته هو');
    assert.equal(theirs[0].anchorSource, 'detected');
    assert.equal(theirs[0].plan, 'Plus', 'ولا تتسرّب خطةُ أحدهما إلى الآخر');
  });
});
