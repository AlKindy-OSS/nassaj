...
async function detectClaudeAnchor(
  ...
  for (const candidate of candidates) {
    ...
    return {
      ...
      // B-596: `derived` لا `detected` — الحقلُ تاريخُ **إنشاء** الاشتراك، وليس
      // موعدَ فوترته. قِيس على حسابٍ حيّ 2026-08-08: الإنشاء 2026-06-09 فيُشتقّ
      // يومُ 9، بينما claude.ai يعلن التجديد يومَ **10**. فرقُ يومٍ واحدٍ لا
      // يُصحَّح بإضافة ثابتٍ على عيّنةٍ واحدة — القاعدةُ العامّة غير معروفة.
      //
      // والفرقُ بين الوسمين حاسمٌ لا لفظيّ: `providerCycleHelpers` يثق بـ
      // `detected` فيعرضه رقماً مؤكّداً في بطاقة الاعتماد وشارة الهيدر معاً،
      // ويصمت عن `derived`. وتاريخٌ يبدو دقيقاً وهو مخطئٌ بيوم أسوأُ من الصمت:
      // العضوُ يبني عليه ولا يشكّ. ومَن أراد الرقم الصحيح ضبطه يدوياً
      // (`anchorSource: 'manual'`) من مصدرٍ يراه بعينه في موقع المزوّد.
      source: 'derived',
      evidence: '.claude.json#oauthAccount.subscriptionCreatedAt (created, not billed)',
