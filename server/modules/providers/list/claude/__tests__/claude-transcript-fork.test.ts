 ...
 *
 * The last test forks a production-shaped synthetic transcript from this host when one exists (the
...
const SOURCE_SESSION = '00000001-0000-4000-8000-000000000001';
...
describe('forkClaudeTranscript', () => {
  ...
  test('strict reply forks accept only one primary assistant transcript UUID', async () => {
    const dir = await tempDir();
    const sidechainAssistant = assistantEntry('side-a', 'u1', 'subagent answer');
    sidechainAssistant.isSidechain = true;
    const source = await writeTranscript(dir, SOURCE_SESSION, [
      userEntry('u1', null, 'question'),
      assistantEntry('a1', 'u1', 'answer'),
      sidechainAssistant,
    ...
    await assert.rejects(
      ...
      (error: unknown) => error instanceof TranscriptForkError && error.code === 'message_not_found',
    );
    await assert.rejects(
      () => forkClaudeTranscript({
        sourceFilePath: source,
        sourceSessionId: SOURCE_SESSION,
        upToMessageId: 'side-a',
        requireAssistantCutoff: true,
      }),
      (error: unknown) => error instanceof TranscriptForkError && error.code === 'message_not_found',
    );
    ...
    const result = await forkClaudeTranscript({
      ...
      requireAssistantCutoff: true,
      extraMessages: [],
      generateId: makeIdGen('strict'),
    });
    assert.equal(result.entryCount, 2, 'the selected assistant reply is the branch tip');
  });
  ...
  test('forks a production-shaped synthetic transcript from this host into a well-formed branch', async (t) => {
    ...
    assert.ok(rows > 2, 'a production-shaped synthetic transcript must yield a non-trivial branch');
