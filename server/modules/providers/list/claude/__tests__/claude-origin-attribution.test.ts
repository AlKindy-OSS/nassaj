...
test('B-1077: stampHumanUserId never stamps sub-agent prompt/prose carrying parentToolUseId', () => {
  const subagentPrompt = normalizedUserText({ parentToolUseId: 'toolu_task_1' });
  stampHumanUserId(subagentPrompt, 7);
  assert.equal(subagentPrompt.userId, undefined);

  const childTool = normalizedUserText({ kind: 'tool_result', role: undefined, parentToolUseId: 'toolu_task_1' });
  stampHumanUserId(childTool, 7);
  assert.equal(childTool.userId, undefined, 'tool rows were never stamped and stay that way');

  const topLevel = normalizedUserText({ parentToolUseId: null });
  stampHumanUserId(topLevel, 7);
  assert.equal(topLevel.userId, 7, 'top-level human text is still attributed');
});
