/**
 * ADR-048 / C5 — Workflow Completion Reconcile (incident 2026-06-27, wf_synthetic_ef5ba242-b4b).
 *
 ...
 *   - INCIDENT_JOURNAL_LINES   ← wf_synthetic_ef5ba242-b4b: 17 started lines / 16 unique
 *     started keys, 15 result lines / 15 unique result keys → ONE orphan key
 *     (the last `started` row, never matched) → status 'settled', 15/16.
 *   - COMPLETED_JOURNAL_LINES  ← wf_synthetic_1ea9f41d-bdf: 11 started lines / 7 unique
...
const INCIDENT_WF_ID = 'wf_synthetic_ef5ba242-b4b';
const COMPLETED_WF_ID = 'wf_synthetic_1ea9f41d-bdf';

/**
 * synthetic rows from the incident journal on disk
 * (.../230ab538-.../subagents/workflows/wf_synthetic_ef5ba242-b4b/journal.jsonl).
...
/**
 * synthetic rows from the completed journal on disk
 * (.../41d650d6-.../subagents/workflows/wf_synthetic_1ea9f41d-bdf/journal.jsonl).
...
test('(ز.1) a DurableTask-shaped completion (result.json + DONE, no journal) is NOT reconciled', async () => {
  ...
  try {
    ...
    await writeDurableTaskShapedDir(sessionDir, 'wf_synthetic_durable_task_abc', JOURNAL_DONE_MS);
...
test('(ز.2) an inline journal and a DurableTask-shaped dir coexist ⇒ ONLY the inline one reconciles', async () => {
  ...
  try {
    ...
    await writeDurableTaskShapedDir(sessionDir, 'wf_synthetic_durable_task_xyz', JOURNAL_DONE_MS);
...
test('(ز.3) reconcileWorkflowMessages (flag ON) emits NO card for a DurableTask-shaped dir', async () => {
  ...
  try {
    ...
    const sessionDir = path.join(tempDir, 'sess-1');
    await mkdir(sessionDir, { recursive: true });
    await writeDurableTaskShapedDir(sessionDir, 'wf_synthetic_durable_task_only', JOURNAL_DONE_MS);
