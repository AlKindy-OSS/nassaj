...
import { reconcileAgentMessages } from '@/modules/providers/list/claude/agent-reconcile.service.js';
 ...
 */
async function readAgentFileNames(subagentsDir: string, lease?: HistoryReadLease): Promise<string[]> {
  try {
    const files = lease ? await lease.directory(subagentsDir) : await fsp.readdir(subagentsDir);
...
async function parseAgentTools(filePath: string, lease?: HistoryReadLease): Promise<AnyRecord[]> {
  const tools: AnyRecord[] = [];

  try {
    const fileStream = lease ? lease.stream(filePath) : fs.createReadStream(filePath);
              ...
              content: typeof part.content === 'string'
                ...
                : Array.isArray(part.content)
                  ? part.content
                    ...
                    .join('\n')
                  : (lease ? lease.stringify(part.content) : JSON.stringify(part.content)),
...
async function getSessionMessages(
  ...
  limit: number | null,
  offset: number,
  identity?: ReturnType<typeof createClaudeRawIdentityCollector>,
  ...
  try {
    ...
    const agentFiles = await readAgentFileNames(subagentsDir, lease);

    const messages: AnyRecord[] = [];
    const agentToolsCache = new Map<string, AnyRecord[]>();

    const fileStream = lease ? lease.stream(jsonLPath) : fs.createReadStream(jsonLPath);
    const rl = readline.createInterface({
      input: fileStream,
      crlfDelay: Infinity,
    });
    ...
    for await (const line of rl) {
      ...
      try {
        const entry = (lease ? lease.parse(line) : JSON.parse(line)) as AnyRecord;
        identity?.observe(entry, Buffer.byteLength(line));
    ...
    for (const agentId of agentIds) {
      ...
      const tools = await parseAgentTools(agentFilePath, lease);
    ...
    // T-873(2): recover `Agent` task-notifications the harness enqueued to this
    // transcript but never handed to the conversation (measured: 100% of
    // `killed` and 24% of `completed` notifications in this project were lost in
    // that in-memory hop). Purely a fold over the `messages` array already
    // parsed above — no extra disk read, no path coupling. Read-only, behind the
    // AGENT_RECONCILE flag (no-op when OFF), and strictly informational: the
    // derived card states the fact and never resumes anything.
    const agentMessages = reconcileAgentMessages(sessionId, messages);
    for (const agentMessage of agentMessages) {
      messages.push(agentMessage as unknown as AnyRecord);
    }
...
export class ClaudeSessionsProvider implements IProviderSessions {
  ...
  normalizeMessage(rawMessage: unknown, sessionId: string | null, lease?: HistoryReadLease): NormalizedMessage[] {
    ...
    // correction, and T-873(2)'s undelivered-`Agent` card) into the same raw
    // history stream that fetchHistory re-normalizes. Such a row carries no
    // `type` and no `message`, so it matches NO branch below and would be
    // dropped as an unrecognized entry — silently turning the whole feature into
    // dead code. Every derived `kind` MUST therefore be named here.
    //
    // Collision safety: `kind` is a field this codebase adds; it never appears on
    // a raw Claude JSONL row or SDK stream event (verified across the corpus), so
    // this guard cannot swallow a genuine transcript row.
    if (
      raw.kind === 'task_reconcile'
      || raw.kind === 'workflow_reconciled'
      || raw.kind === 'task_notification'
    ) {
        ...
        for (let partIndex = 0; partIndex < raw.message.content.length; partIndex++) {
          ...
          if (part.type === 'tool_result') {
            messages.push(createNormalizedMessage({
              ...
              content: typeof part.content === 'string' ? part.content : (lease ? lease.stringify(part.content) : JSON.stringify(part.content)),
              isError: Boolean(part.is_error),
    ...
    if (raw.message?.role === 'assistant' && raw.message?.content) {
      /**
       * B-352: the model that produced THIS turn, verbatim. The SDK writes it on
       * every assistant entry (live event and JSONL row alike) and it changes
       * within a single session whenever the engine axis is re-stamped, so it is
       * per-message attribution, not a session property. `<synthetic>` is the
       * SDK's placeholder for tool-only/interrupted turns — it names no model
       * and must not be shown.
       */
      const turnModel =
        typeof raw.message.model === 'string' &&
        raw.message.model.length > 0 &&
        raw.message.model !== '<synthetic>'
          ? raw.message.model
          : undefined;
  ...
  async fetchHistory(
    ...
    const identity = createClaudeRawIdentityCollector(sessionId);
    let result: ClaudeHistoryResult;
    try {
      // Load full history first so `total` reflects frontend-normalized messages,
      // not raw JSONL records.
      result = await getSessionMessages(sessionId, null, 0, identity, options.historyLease);
    } catch (error) {
      ...
      if (msg.kind === 'tool_use' && msg.toolId && toolResultMap.has(msg.toolId)) {
        ...
        msg.toolResult = {
          content: typeof toolResult.content === 'string'
            ? toolResult.content
            : (options.historyLease ? options.historyLease.stringify(toolResult.content) : JSON.stringify(toolResult.content)),
    ...
    return identity.attach({
      messages,
      total,
      hasMore,
      offset: normalizedOffset,
      limit: normalizedLimit,
    }, raw => this.normalizeMessage(raw, sessionId, options.historyLease));
  }
}
