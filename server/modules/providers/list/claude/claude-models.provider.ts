...
/**
 * B-313: Claude Code stamps its OWN system-generated assistant rows with a
 * sentinel in the `model` field instead of a model id — measured value
 * `<synthetic>`, written for quota notices ("You've reached your Fable 5
 * limit…"), interrupts and API errors. Those rows are the LAST model-bearing
 * line in a transcript whenever a session ended on such a notice (measured: 5 of
 * 179 local transcripts), and this reader walks backwards taking the first
 * `model` it sees — so the session reported `<synthetic>` as its active model.
 *
 * That is not cosmetic: `resolveResumeModel` falls back to this value when the
 * session has no explicit override, so the next turn was spawned with
 * `--model <synthetic>`.
 *
 * The rule is shape-based, deliberately: any `<…>` value is a placeholder by
 * construction, never a model id. It is NOT a catalog check — validating against
 * the catalog would silently substitute a legitimately new/unlisted model id,
 * which is the failure B-235 already paid for.
 */
const isSentinelModelId = (model: string): boolean => /^<.*>$/.test(model.trim());
...
/**
 * Exported for the B-313 regression test only — the reader is the unit that owns
 * the sentinel rule, and `getCurrentActiveModel` around it needs the sessions DB.
 */
export const __readClaudeSessionModelFromJsonlForTests = (
  sessionId: string,
  jsonlPath: string,
): Promise<ProviderCurrentActiveModel | null> => readClaudeSessionModelFromJsonl(sessionId, jsonlPath);
...
const readClaudeSessionModelFromJsonl = async (
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    try {
      ...
      // B-313: a sentinel row is not an answer — keep walking back to the last
      // row that names a real model, rather than reporting the placeholder.
      if (model && !isSentinelModelId(model)) {
        return { model };
      }
    } catch {
