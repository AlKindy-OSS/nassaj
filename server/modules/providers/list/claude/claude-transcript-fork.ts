...
export interface ForkClaudeTranscriptOptions {
  ...
  /**
   * Require the cutoff to identify exactly one primary assistant entry belonging
   * to `sourceSessionId`. Used by the user-visible “fork from this reply” path.
   */
  requireAssistantCutoff?: boolean;
  /**
...
async function scanSource(
  ...
  upToMessageId: string | null,
  generateId: () => string,
  options: { requireAssistantCutoff: boolean; sourceSessionId: string },
): Promise<{
  entries: Map<string, ScannedEntry>;
  order: string[];
  foundCutoff: boolean;
  canonicalCutoff: boolean;
  cutoffOccurrences: number;
}> {
  const entries = new Map<string, ScannedEntry>();
  const order: string[] = [];
  let foundCutoff = false;
  let canonicalCutoff = false;
  let cutoffOccurrences = 0;
  ...
  try {
    for await (const line of rl) {
      ...
      const entryUuid = asString(entry.uuid);
      if (upToMessageId && entryUuid === upToMessageId) {
        cutoffOccurrences += 1;
        const message = entry.message as { role?: unknown } | undefined;
        canonicalCutoff = isBranchableEntry(entry)
          && entry.type === 'assistant'
          && message?.role === 'assistant'
          && entry.sessionId === options.sourceSessionId;
        if (options.requireAssistantCutoff && !canonicalCutoff) {
          // Keep scanning to reject ambiguous duplicate UUIDs too, but never
          // carry a non-canonical target into the fork.
          continue;
        }
      }
      if (!isBranchableEntry(entry) || foundCutoff) {
        continue;
      }
  ...
  return { entries, order, foundCutoff, canonicalCutoff, cutoffOccurrences };
}
...
export async function forkClaudeTranscript(
  ...
  let directoryHandle;
  try {
    sourceHandle = await open(sourceFilePath, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
    directoryHandle = await open(
      path.dirname(sourceFilePath),
      fsConstants.O_RDONLY | fsConstants.O_DIRECTORY | fsConstants.O_NOFOLLOW,
    );
    const [sourceTarget, directoryTarget, sourceStats, directoryStats] = await Promise.all([
      realpath(`/proc/self/fd/${sourceHandle.fd}`),
      realpath(`/proc/self/fd/${directoryHandle.fd}`),
      sourceHandle.stat(),
      directoryHandle.stat(),
    ]);
    if (!sourceStats.isFile() || sourceStats.nlink !== 1 || !directoryStats.isDirectory()
      || path.dirname(sourceTarget) !== directoryTarget
      || path.basename(sourceTarget) !== path.basename(sourceFilePath)) {
      throw new TranscriptForkError('source_unreadable', 'Transcript file identity is not trusted.');
    }
  } catch (error) {
    await sourceHandle?.close().catch(() => {});
    await directoryHandle?.close().catch(() => {});
    if (error instanceof TranscriptForkError) throw error;
    throw new TranscriptForkError('source_unreadable', 'Transcript file could not be opened safely.');
  }
  ...
  if (carryHistory) {
    const scanned = await scanSource(pinnedSourcePath, upToMessageId, generateId, {
      requireAssistantCutoff,
      sourceSessionId,
    });
    ...
    if (
      requireAssistantCutoff
      && (!scanned.canonicalCutoff || scanned.cutoffOccurrences !== 1)
    ) {
      throw new TranscriptForkError(
        'message_not_found',
        'The selected assistant message is not a canonical message in this session.',
      );
    }
    ...
    seededTemplate = await readTranscriptTemplate(pinnedSourcePath);
  ...
  const targetDir = await realpath(pinnedDirectoryPath);
  const filePath = path.join(targetDir, `${forkedSessionId}.jsonl`);
  const pinnedFilePath = path.join(pinnedDirectoryPath, `${forkedSessionId}.jsonl`);
  const pinnedTempPath = path.join(pinnedDirectoryPath, `${forkedSessionId}.jsonl.tmp`);
  ...
  const output = createWriteStream(pinnedTempPath, { encoding: 'utf8', flags: 'wx', mode: 0o600 });
  ...
  const copySourceEntries = async (): Promise<void> => {
    const stream = createReadStream(pinnedSourcePath, { encoding: 'utf8' });
    const rl = readline.createInterface({ input: stream, crlfDelay: Infinity });
  ...
  try {
    ...
    await rename(pinnedTempPath, pinnedFilePath);
  } catch (error) {
    output.destroy();
    await unlink(pinnedTempPath).catch(() => {});
    if (error instanceof TranscriptForkError) {
      throw error;
    }
    throw new TranscriptForkError(
