...
export type GovernanceResult = {
  ...
  reason?: string;
  /**
   * True when this user deliberately exempted codex (owner decision 2026-08-08):
   * the launch proceeds in Codex's own default posture. Distinct from `ok:true`
   * after attestation — same permission to spawn, opposite governance fact — so
   * a caller that logs or badges the result can tell "governed" from "exempt by
   * choice" instead of reporting the second as the first.
   */
  exempt?: boolean;
};
...
export function ensureCodexGovernance(userId: string | number | null): GovernanceResult {
  ...
  // EXEMPTION FIRST, BEFORE ANY ATTESTATION OR REPAIR (owner decision
  // 2026-08-08). This is a launch gate, so this is where the switch belongs —
  // NOT in resolveProviderEnv, whose 33 callers are mostly reads (one of them a
  // loop over every user on the node) and would fire this decision on a settings
  // page load (qa-critic veto V1).
  //
  // The early return is what makes the exemption HOLD: everything below this
  // line is a repair path — invalidateProvisioned + provisionUserDirs, then a
  // direct re-materialization — and reaching it would rewrite the very file the
  // user asked to have removed, silently re-governing them on their next turn.
  // Exempt therefore never blocks and never writes: an exemption is a choice,
  // not a fault, so refusing the launch would punish the user for exercising it.
  if (isGovernanceExempt(uid, 'codex')) {
    return { ok: true, codexHome, agentsPath, exempt: true };
  }
