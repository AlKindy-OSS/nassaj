  ...
  try {
    ...
    for (const msg of sourceMessages) {
      ...
      messages.push({
        ...
        message: { role, content: msg.content, model: readResponseModel(msg.model) },
      ...
      if (role) {
        ...
        if (textContent.trim()) {
          messages.push({
            ...
            message: { role, content: textContent, model: readResponseModel(entry.model) },
...
export class GeminiSessionsProvider implements IProviderSessions {
    ...
    if (raw.type === 'message' && raw.role === 'assistant') {
      ...
      if (content) {
        messages.push(createNormalizedMessage({
          ...
          kind: 'stream_delta',
          content,
          model: readResponseModel(raw.model),
        }));
      }
      if (raw.delta !== true) {
        messages.push(createNormalizedMessage({
  ...
  async fetchHistory(
    ...
    for (let i = 0; i < rawMessages.length; i++) {
      ...
      const model = normalizedRole === 'assistant' ? readResponseModel(raw.message?.model) : undefined;
