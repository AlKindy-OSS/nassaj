...
export class GeminiProviderAuth implements IProviderAuth {
  /**
   * The home root this status check inspects.
   *
   * This used to read `process.env.GEMINI_CLI_HOME`, which claimed to "match
   * runtime behavior". It never did, on two counts (B-548): the CLI reads no
   * such variable at all (measured), and the variable was set on the CHILD's
   * env by resolveProviderEnv, never on the server's own `process.env` — so the
   * `?? os.homedir()` branch was the only one that ever ran.
   *
   * B-587 settles what that note left open: the status is now resolved against
   * the SAME env a gemini spawn for this user would get. `HOME` is the only knob
   * that moves this CLI's tree (see resolve-provider-env.js `case 'gemini'`), so
   * asking resolveProviderEnv for it is exactly what the turn itself will do.
   *
   * `userId == null` (shared policy, anonymous probe, install-only check) keeps
   * the operator's home deliberately — resolveProviderEnv returns the base env
   * unchanged there, so the fallback is the honest answer, not a silent guess.
   */
  private getGeminiCliHome(userId?: string | number | null): string {
    const env = resolveProviderEnv(userId ?? null, 'gemini', process.env);
    return readOptionalString(env.HOME) ?? os.homedir();
  }
  ...
  async getStatus(userId?: string | number | null): Promise<ProviderAuthStatus> {
    ...
    const credentials = await this.checkCredentials(userId);
   ...
   */
  private async loadUserLevelAuthEnv(userId?: string | number | null): Promise<Record<string, string>> {
    const geminiCliHome = this.getGeminiCliHome(userId);
    const envCandidates = [
   ...
   */
  private async readSelectedAuthType(userId?: string | number | null): Promise<GeminiAuthType> {
    try {
      const settingsPath = path.join(this.getGeminiCliHome(userId), '.gemini', 'settings.json');
   ...
   */
  private async checkCredentials(userId?: string | number | null): Promise<GeminiCredentialsStatus> {
    if (process.env.GEMINI_API_KEY?.trim()) {
      return { authenticated: true, email: 'API Key Auth', method: 'api_key' };
    }

    const userEnv = await this.loadUserLevelAuthEnv(userId);
    if (readOptionalString(userEnv.GEMINI_API_KEY)) {
      return { authenticated: true, email: 'API Key Auth', method: 'api_key' };
    }

    const selectedType = await this.readSelectedAuthType(userId);
    ...
    try {
      const credsPath = path.join(this.getGeminiCliHome(userId), '.gemini', 'oauth_creds.json');
      ...
      return {
        authenticated: true,
        email: await this.getActiveAccountEmail(userId) || 'OAuth Session',
        method: 'credentials_file',
      };
    } catch {
   ...
   */
  private async getActiveAccountEmail(userId?: string | number | null): Promise<string | null> {
    try {
      const accPath = path.join(this.getGeminiCliHome(userId), '.gemini', 'google_accounts.json');
