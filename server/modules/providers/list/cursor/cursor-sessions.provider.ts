  ...
  normalizeMessage(rawMessage: unknown, sessionId: string | null): NormalizedMessage[] {
    ...
    if (raw?.type === 'assistant' && raw.message?.content?.[0]?.text) {
      return [createNormalizedMessage({
        ...
        model: readResponseModel(raw.message.model ?? raw.model),
        sessionId,
        provider: PROVIDER,
      })];
    }
          ...
          if (content?.message?.role && content?.message?.content) {
            ...
            if (text?.trim()) {
              messages.push(createNormalizedMessage({
                ...
                kind: 'text',
                role,
                model: role === 'assistant' ? readResponseModel(content.message.model) : undefined,
                content: text,
          ...
          for (let partIdx = 0; partIdx < content.content.length; partIdx++) {
            ...
            if (part?.type === 'text' && part?.text) {
              ...
              messages.push(createNormalizedMessage({
                ...
                kind: 'text',
                role,
                model: role === 'assistant' ? readResponseModel(content.model) : undefined,
