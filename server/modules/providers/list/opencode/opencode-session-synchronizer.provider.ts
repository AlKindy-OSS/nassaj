...
export class OpenCodeSessionSynchronizer implements IProviderSessionSynchronizer {
  ...
  private upsertSession(
    ...
    //
    // B-477 — the row is written as attribution='provenance', which is the whole
    // point of this call: it NAMES the conversation without granting anyone
    // rights over it. In shared mode this resolves to the platform owner for
    // every member's session (the data dir is the operator's), and this write
    // BEATS the spawn path on every new session — opencode commits the row to
    // its own DB before it prints the id the spawn path is still waiting for, so
    // "synchronizer first" is the normal order, not a rare race. Marked as
    // inferred, that no longer matters: the real spawner takes the owner badge
    // when their id finally arrives, and until then nobody has gained anything.
