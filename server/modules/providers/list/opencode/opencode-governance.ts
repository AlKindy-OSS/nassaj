...
const operatorHome = (): string => readOptionalString(process.env.HOME) ?? os.homedir();
...
export function neutralOpenCodeGovernanceSource(): string {
  return path.join(operatorHome(), '.claude', OPENCODE_AGENTS_FILENAME);
}
 ...
 */
export function ensureOpenCodeGovernance(userId: string | number | null, configHome?: string): OpenCodeGovernanceResult {
  const uid = userId === undefined || userId === '' ? null : userId;
  const home = configHome ?? resolveOpenCodeConfigHomeForUser(uid);

  // EXEMPTION FIRST (owner decision 2026-08-08). Checked HERE, in the vendor
  // facade, and not inside ensureVendorCliGovernance: that gate is deliberately
  // vendor-NEUTRAL and takes no user id (vendor-cli-governance.js §NEUTRALITY),
  // so teaching it about per-user exemptions would give it the one concept its
  // whole contract excludes. The facade is the layer that knows both the user
  // and the engine, so it is where the two meet.
  //
  // Returning BEFORE the shared gate is what makes the exemption hold: that gate
  // self-heals by re-materializing the 0444 copy, which would put back the exact
  // file the user asked to remove. It also THROWS when it cannot attest, so
  // falling through would block an exempt user's launch — punishing them for a
  // choice they were entitled to make.
  if (isGovernanceExempt(uid, OPENCODE_VENDOR_ID)) {
    return {
      ok: true,
      vendorId: OPENCODE_VENDOR_ID,
      home,
      governancePath: path.join(home, OPENCODE_AGENTS_FILENAME),
      repaired: false,
    };
  }
