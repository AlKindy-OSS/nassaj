...
import { resolveOpenCodeDatabasePathForUser } from './opencode-home.js';
 ...
 * owner's session ses_synthetic_05aab10c: turn 1 (new session, id passed through intact)
...
export class OpenCodeProviderModels implements IProviderModels {
  ...
  async getCurrentActiveModel(sessionId?: string, userId?: string | number | null): Promise<ProviderCurrentActiveModel> {
    ...
    try {
      // B-1268: the per-user opencode database is read ONLY for a caller who actually
      // has local servers under an activated feature — a local turn runs against the
      // caller's own XDG tree. Every other caller keeps the pre-ADR-163 shared path.
      const dbPath = hasRunnableLocalServers(userId ?? null)
        ? resolveOpenCodeDatabasePathForUser(userId ?? null)
        : getOpenCodeDatabasePath();
