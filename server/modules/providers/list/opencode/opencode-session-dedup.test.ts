...
test('B-172: an ORPHAN child (parent absent) is folded silently and creates no row', { concurrency: false }, async () => {
  ...
  try {
    ...
    await createOpenCodeDatabase(tempRoot, workspacePath, [
      {
        id: 'ses_synthetic_orphan_child',
        parentId: 'ses_synthetic_missing_parent',
        title: 'Orphan child',
        timeCreated: 1_700_000_001_000,
        timeUpdated: 1_700_000_002_000,
      },
    ...
    await withIsolatedDatabase(async () => {
      ...
      assert.equal(sessionsDb.getSessionById('ses_synthetic_orphan_child'), null, 'orphan child gets no row');
      assert.equal(sessionsDb.getSessionById('ses_synthetic_missing_parent'), null, 'bump never resurrects a missing parent');
    });
  } finally {
    restoreHomeDir();
    await rm(tempRoot, { recursive: true, force: true });
