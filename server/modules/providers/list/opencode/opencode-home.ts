 ...
 */
export function resolveOpenCodeDataHomeForUser(
  userId: string | number | null,
  options: { honorGrants?: boolean } = {},
): string {
  const env = resolveProviderEnv(userId, 'opencode', process.env, 'chat', options);
...
export function resolveOpenCodeDataHomes(): string[] {
  ...
  try {
    for (const user of userDb.listUsers()) {
      ...
      // T-1675: keep the member's OWN dir indexed while they run on a grant.
      homes.add(resolveOpenCodeDataHomeForUser(user.id, { honorGrants: false }));
    }
  } catch (error) {
...
export function resolveOpenCodeDataHomeOwner(dataHome: string): number | null {
  ...
  if (target !== operatorDir) {
    try {
      // T-1675: match each member's OWN dir, not the grantor's dir they may be
      // running on — a dir belongs to the member whose tree it is in.
      for (const user of userDb.listUsers()) {
        if (path.resolve(resolveOpenCodeDataHomeForUser(user.id, { honorGrants: false })) === target) {
