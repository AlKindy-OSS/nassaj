...
import { authorizedLocalModelServers, localModelsRunnable } from '@/services/isolation/local-model-config.js';
...
export class OpenCodeProviderAuth implements IProviderAuth {
  ...
  async getStatus(userId?: string | number | null): Promise<ProviderAuthStatus> {
    ...
    const localConfigured = localModelsRunnable(userId) && authorizedLocalModelServers(userId).some(server => server.models.length > 0);
    const credentials = localConfigured
      ? { authenticated: true, email: null, method: 'local_configuration', error: undefined }
      : await this.checkCredentials(userId ?? null);
