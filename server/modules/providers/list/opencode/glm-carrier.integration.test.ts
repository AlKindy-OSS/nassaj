...
const ORIGINAL_XDG_CONFIG = process.env.XDG_CONFIG_HOME;
...
after(() => {
  ...
  if (ORIGINAL_XDG_CONFIG === undefined) delete process.env.XDG_CONFIG_HOME;
  else process.env.XDG_CONFIG_HOME = ORIGINAL_XDG_CONFIG;
...
describe('GL-10 — governance fail-closed (GL-5)', () => {
  ...
  beforeEach(() => {
    ...
    process.env.XDG_CONFIG_HOME = path.join(govTempHome, '.config');
  });
