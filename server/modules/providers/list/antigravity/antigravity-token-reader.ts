...
/**
 * Resolves the absolute path of the agy OAuth token file for `userId`.
 *
 * B-580: this used to read `os.homedir()` with no userId at all, so every
 * member's catalog fetch ran on the OPERATOR's token — their catalog, their
 * quota. `HOME` is the only knob that moves agy's tree (see
 * resolve-provider-env.js `case 'agy'`), so asking resolveProviderEnv for it is
 * exactly what a real agy turn does.
 *
 * `userId` is positionally REQUIRED, not optional: an optional id with a silent
 * fallback to the operator's home is the precise shape that produced this bug —
 * and the same shape was found in gemini/cursor (B-587). `null` is still
 * accepted and still means the operator, but the caller has to say so.
 *
