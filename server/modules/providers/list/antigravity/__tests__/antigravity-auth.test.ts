...
async function withFakeHome(
  ...
  // B-1138: agy now resolves through AGY_PATH and PATH too, so a host agy must
  // not leak into a test that pins "missing".
  const { PATH: originalPath, AGY_PATH: originalAgyPath } = process.env;
  process.env.PATH = path.join(tempHome, 'empty-path');
  ...
  try {
    ...
    if (originalAgyPath !== undefined) process.env.AGY_PATH = originalAgyPath;
    restoreHomeDir();
