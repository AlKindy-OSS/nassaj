...
// A synthetic capture of `agy models` (v1.1.1) stdout: one display LABEL per
...
test('parseAgyModelsOutput: parses the live tab-separated ID and display label shape', () => {
  ...
  assert.deepEqual(
    result.OPTIONS.find((option) => option.value === 'gemini-3.8-flash-high'),
