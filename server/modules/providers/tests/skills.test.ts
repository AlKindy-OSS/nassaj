...
test('providerSkillsService adds global skills for claude, codex, gemini, and cursor', { concurrency: false }, async () => {
  ...
  try {
    ...
    // Overwrite replaces the whole directory: stale supporting files must vanish.
    ...
    // A duplicate target inside one batch must reject before any write lands.
    ...
    // Round-trip: the installed skills are discoverable through listProviderSkills.
    ...
    // Supporting file traversal must be rejected.
    await assert.rejects(
      providerSkillsService.addProviderSkills('codex', {
        entries: [
          {
            content: '---\nname: unsafe-skill\n---\n',
            files: [{ relativePath: '../outside.js', content: '', encoding: 'utf8' }],
          },
        ],
      }),
...
/**
 * removeProviderSkill deletes an installed global skill directory, returns the
 * removed record, and reports a not-found error for an absent skill.
 */
test('providerSkillsService removes an installed global skill', { concurrency: false }, async () => {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'llm-skills-remove-'));
  const restoreHomeDir = patchHomeDir(tempRoot);
  ...
  try {
    await providerSkillsService.addProviderSkills('claude', {
      entries: [
        {
          directoryName: 'removable-skill',
          content: '---\nname: removable-skill\ndescription: Temp skill\n---\n\nBody.\n',
          files: [{ relativePath: 'scripts/run.js', content: 'x', encoding: 'utf8' }],
        },
      ],
    });
    const skillDir = path.join(tempRoot, '.claude', 'skills', 'removable-skill');
    assert.ok((await fs.stat(skillDir)).isDirectory());

    const removed = await providerSkillsService.removeProviderSkill('claude', 'removable-skill');
    assert.equal(removed.name, 'removable-skill');
    assert.equal(removed.command, '/removable-skill');
    assert.equal(removed.provider, 'claude');
    await assert.rejects(fs.stat(skillDir), { code: 'ENOENT' });

    // A subsequent list no longer surfaces the skill.
    const listed = await providerSkillsService.listProviderSkills('claude');
    assert.equal(listed.some((skill) => skill.name === 'removable-skill'), false);

    // Removing an absent skill is a 404-style not-found error.
    await assert.rejects(
      providerSkillsService.removeProviderSkill('claude', 'never-installed'),
      /is not installed/i,
    );
  } finally {
    restoreHomeDir();
...
test('providerSkillsService rejects managed skill writes for read-only providers', { concurrency: false }, async () => {
  const entries = [
    { directoryName: 'unsupported-dir', content: '---\nname: unsupported\ndescription: no\n---\n\nBody.\n' },
  ];

  for (const provider of ['opencode', 'antigravity', 'deepseek'] as const) {
    await assert.rejects(
      providerSkillsService.addProviderSkills(provider, { entries }),
      /does not support managed global skills/i,
      `${provider} addProviderSkills must reject`,
    );
    await assert.rejects(
      providerSkillsService.removeProviderSkill(provider, 'unsupported-dir'),
      /does not support managed global skills/i,
