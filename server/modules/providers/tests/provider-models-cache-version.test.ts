 ...
 * for the configured TTL. Ids that OUR code declares (the GLM carrier block that becomes
 ...
 * without bumping the version fails here instead of in the picker after the
 * configured TTL.
 *
...
test('the GLM carrier catalog and the cache version move together (B-234)', () => {
  ...
  if (actual !== PINNED.carrierFingerprint) {
    assert.notEqual(
      ...
      'GLM_CARRIER_MODELS changed but PROVIDER_MODELS_CACHE_VERSION did not. '
        ...
        + 'offering the old ids until the catalog TTL expires (B-234). Bump the version, then update '
  ...
  assert.equal(
    PROVIDER_MODELS_CACHE_TTL_MS,
    PINNED.cacheTtlMs,
    'The persisted catalog TTL changed without updating this cache-policy pin.',
  );
});
