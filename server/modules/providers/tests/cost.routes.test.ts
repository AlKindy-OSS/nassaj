...
const PUBLIC_CLAUDE_SID = '00000001-0000-4000-8000-000000000001';
const PUBLIC_OPENCODE_SID = 'ses_synthetic_06214044dffemD26QTiayDywHC';
const PRIVATE_CODEX_SID = '00000002-0000-4000-8000-000000000002';
const UNKNOWN_SID = '00000003-0000-4000-8000-000000000003';
...
test('ADR-089 fixture sanity: every project is visible to every authenticated user', () => {
  const projectId = projectsDb.getProjectPath(privateProjectPath)?.project_id as string;
  assert.equal(projectsDb.isProjectVisibleToUser(projectId, outsiderUser.id), true);
  assert.equal(projectsDb.isProjectVisibleToUser(projectId, memberUser.id), true);
});

test('an UNKNOWN session id is still refused — existence is not disclosed', async () => {
...
test('ADR-089: a teammate prices a session they did not create', async () => {
...
test('PUT stores the renewal day and answers with the stored row', async () => {
  ...
  assert.equal(subscriptionConfigService.getSettings('claude', ownerUser.id).anchorDay, 17);
  assert.equal(subscriptionConfigService.getSettings('claude', ownerUser.id).plan, 'Max 20x');
});

test('a plain member cannot rewrite the install-wide subscription config', async () => {
  const before = subscriptionConfigService.getSettings('claude', ownerUser.id).anchorDay;
  ...
  assert.equal(subscriptionConfigService.getSettings('claude', ownerUser.id).anchorDay, before);
});
...
test('PUT applies a partial patch without clearing the untouched fields', async () => {
  ...
  const settings = subscriptionConfigService.getSettings('claude', ownerUser.id);
...
test('unknown body fields cannot ride along into the store', async () => {
  ...
  const stored = subscriptionConfigService.getStored(ownerUser.id).claude as Record<string, unknown>;
