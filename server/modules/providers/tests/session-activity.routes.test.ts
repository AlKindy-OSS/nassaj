 ...
 *
 * Fixture provenance: the ids and the id ALPHABET come from the fixture database of
...
test('ADR-089 fixture sanity: the project is visible to every authenticated user', () => {
  ...
  assert.equal(projectsDb.isProjectVisibleToUser(projectId, null), false, 'anonymous still refused');
});
...
test('CRUX: an UNKNOWN session reports idle without echoing the id', async () => {
  const unknown = await get(`/api/providers/sessions/${UNKNOWN_SID}/activity`, outsiderUser);

  assert.equal(unknown.status, 200);
  assert.equal(unknown.text, IDLE_BODY);
  assert.ok(!unknown.text.includes(UNKNOWN_SID), 'the id is never echoed');
});

test('ADR-089: a teammate sees the real activity of a session they did not create', async () => {
  const { status, text } = await get(
  ...
  assert.equal(status, 200);
  assert.equal(text, '{"isProcessing":true}', 'project visibility is retired — the team shares it');
  assert.deepEqual(probeCalls, [PRIVATE_CODEX_SID], 'the liveness probe IS reached for a teammate');
});
