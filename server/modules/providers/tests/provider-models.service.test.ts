...
import {
  createProviderModelsService as createProviderModelsServiceRuntime,
...
const createProviderModelsService = (
  dependencies: Parameters<typeof createProviderModelsServiceRuntime>[0] = {},
) => createProviderModelsServiceRuntime({
  ...dependencies,
  runCatalogProbe: dependencies.runCatalogProbe
    ?? (async (_provider, _userId, _principal, probe) => probe()),
});
...
test('static Qwen catalog bypasses execution admission while live catalogs remain admitted', async () => {
  const admitted: LLMProvider[] = [];
  const service = createProviderModelsService({
    cachePath: createEphemeralCachePath(),
    runCatalogProbe: async (provider, _userId, _principal, probe) => {
  ...
  await service.getProviderModels('qwen', { bypassCache: true }, 1, { id: 1 });
  await service.getProviderModels('codex', { bypassCache: true }, 1, { id: 1 });

  assert.deepEqual(admitted, ['codex']);
});
...
test('provider models are cached for exactly the 24-hour TTL', async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'provider-model-cache-ttl-'));
  let currentTime = 1_000;
  let loadCount = 0;

  try {
    assert.equal(PROVIDER_MODELS_CACHE_TTL_MS, 24 * 60 * 60 * 1000);
...
test('stale-while-revalidate: an expired entry is served instantly while refreshing in the background', async () => {
  ...
  try {
    const service = createProviderModelsService({
      cachePath: path.join(tempRoot, 'models-cache.json'),
      now: () => currentTime,
      runCatalogProbe: async (_provider, _userId, principal, probe) => {
    ...
    // Prime the cache.
    const first = await service.getProviderModels(
      'claude', {}, authenticatedPrincipal.id, authenticatedPrincipal,
    );
    ...
    currentTime += PROVIDER_MODELS_CACHE_TTL_MS + 1;
    const stale = await service.getProviderModels(
      'claude', {}, authenticatedPrincipal.id, authenticatedPrincipal,
    );
    ...
    assert.deepEqual(observedPrincipals, [authenticatedPrincipal, authenticatedPrincipal],
      'the stale background refresh preserves the authenticated principal');

    // Let the background refresh finish, then the next read sees the fresh value.
    slowFetch.resolve?.();
    await new Promise((resolve) => setTimeout(resolve, 0));
    const refreshed = await service.getProviderModels(
      'claude', {}, authenticatedPrincipal.id, authenticatedPrincipal,
    );
...
test('a persisted three-day catalog is retired when the cache policy moves to 24 hours', async () => {
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'provider-model-cache-policy-retire-'));
  const cachePath = path.join(tempRoot, 'models-cache.json');
  let loadCount = 0;
  ...
  try {
    await writeFile(cachePath, JSON.stringify({
      ...
      entries: {
        'codex::system': {
          updatedAt: 1_000,
          expiresAt: 1_000 + (3 * 24 * 60 * 60 * 1000),
          models: createModels('codex-stale-three-day-entry'),
        },
      },
    }), 'utf8');
    ...
    const service = createProviderModelsService({
      ...
      resolveProvider: (provider) => ({
        models: {
          getSupportedModels: async () => {
            loadCount += 1;
            return createModels(`${provider}-fresh`);
          },
          getCurrentActiveModel: async () => createCurrentActiveModel(`${provider}-active`),
          changeActiveModel: async (input) => createSessionActiveModelChange(provider, input),
        },
    ...
    assert.equal(loadCount, 1, 'the version mismatch must force a live refresh');
    assert.equal(result.models.DEFAULT, 'codex-fresh');
...
test('bypassCache forces a fresh provider fetch and updates cache metadata', async () => {
  ...
  try {
    ...
    assert.equal(
      Date.parse(refreshed.cache.expiresAt) - Date.parse(refreshed.cache.updatedAt),
      PROVIDER_MODELS_CACHE_TTL_MS,
      'an explicit refresh must replace the entry with a full 24-hour TTL',
    );
