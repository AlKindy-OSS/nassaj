...
test(
  'create-only skill installation is complete and never clobbers a concurrent winner',
  { concurrency: false },
  async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'skills-create-only-'));
    const restoreHomeDir = patchHomeDir(tempRoot);
    try {
      const requests = ['First body.', 'Second body.'].map((body) => (
        providerSkillsService.addProviderSkills('claude', {
          createOnly: true,
          entries: [{ directoryName: 'exclusive', content: skillMarkdown('exclusive', body) }],
        })
      ));
      ...
      if (rejected?.status === 'rejected') {
        assert.equal((rejected.reason as { code?: string }).code, 'PROVIDER_SKILL_ALREADY_EXISTS');
      }

      const skillDir = path.join(tempRoot, '.claude', 'skills', 'exclusive');
      const markdown = await fs.readFile(path.join(skillDir, 'SKILL.md'), 'utf8');
      assert.ok(markdown.includes('First body.') || markdown.includes('Second body.'));
      const marker = JSON.parse(await fs.readFile(path.join(skillDir, MARKER_FILE), 'utf8')) as {
        managedBy: string;
        provider: string;
        name: string;
      };
      assert.deepEqual(
        { managedBy: marker.managedBy, provider: marker.provider, name: marker.name },
        { managedBy: 'nassaj', provider: 'claude', name: 'exclusive' },
      );
    } finally {
      restoreHomeDir();
      await fs.rm(tempRoot, { recursive: true, force: true });
