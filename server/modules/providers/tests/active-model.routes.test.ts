...
const PRIVATE_SID = randomUUID(); // created by another user, unpinned
const UNKNOWN_SID = '00000001-0000-4000-8000-000000000001';
...
test('an UNKNOWN session id is still refused, and echoes nothing back', async () => {
  const unknown = await get(activeModelUrl(PROVIDER, UNKNOWN_SID), outsiderUser);

  assert.equal(unknown.status, 404, 'refusal is a 404, not a 403');
  const unknownBody = JSON.parse(unknown.text);
  assert.equal(unknownBody.error.code, 'SESSION_NOT_FOUND');
  assert.ok(!unknown.text.includes(OVERRIDE_MODEL), 'no model leaks for an unknown id');
});

test('ADR-089: a teammate reads the active model of a session they did not create', async () => {
  const { status, text } = await get(activeModelUrl(PROVIDER, PRIVATE_SID), outsiderUser);

  assert.equal(status, 200, 'project visibility is retired — the team shares every project');
  const { data, error } = JSON.parse(text);
  assert.equal(error, undefined, 'the session resolves for a teammate; no refusal');
  assert.equal(data.sessionId, PRIVATE_SID);
  assert.ok(data.model.length > 0, 'the session model is served, not withheld');
});
