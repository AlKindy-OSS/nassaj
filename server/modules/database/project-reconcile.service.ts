...
/** Reads only a bounded package.json prefix; falls back to the directory name. */
async function detectProjectName(projectPath: string): Promise<string> {
  ...
  try {
    handle = await fs.open(path.join(projectPath, 'package.json'), 'r');
    const buffer = Buffer.alloc(64 * 1024);
    const { bytesRead } = await handle.read(buffer, 0, buffer.length, 0);
    const parsed = JSON.parse(buffer.subarray(0, bytesRead).toString('utf8')) as { name?: unknown };
    return typeof parsed.name === 'string' && parsed.name.trim() ? parsed.name.trim() : fallback;
  } catch {
    return fallback;
  } finally {
...
export async function reconcileProjects(deps: ReconcileDeps = {}): Promise<ReconcileResult> {
  ...
  const recoveries = trackedArchived.map(async (row) => {
    ...
    if (probe === 'exists') {
      projectsDb.updateProjectDirectoryState(row.project_id, true);
      projectsDb.updateProjectDetectedName(row.project_id, await detectProjectName(row.project_path));
    ...
    if (probe === 'missing') {
      ...
      projectsDb.updateProjectDirectoryState(row.project_id, null);
    }
...
export function startReconcileScheduler(): void {
  ...
  void runLocalUpdateBackground('project-reconcile', () => reconcileProjects()).then((result) => {
