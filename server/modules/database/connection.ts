...
// eslint-disable-next-line boundaries/dependencies -- pre-import admission must reuse the existing authority inspector without the connector barrel.
import { inspectExistingConnectorPolicyV2Substrate } from '../connectors/connector-substrate-only.production.js';
...
import { observeCompatibleForwardDatabase } from './compatible-forward-observation.js';
...
export function getConnection(): Database.Database {
  ...
  // Inspection before the root security grant must never open a writable SQLite handle.
  if (instance && admission && instance.readonly && admission.phase !== 'claimed') {
    if (!['security_startup_authorized', 'serving'].includes(admission.phase)) {
  ...
  if (admission) {
    instance = openExistingStartupConnection(dbPath, admission.database, admission.phase, admission.databaseTarget);
    return instance;
  }
...
type ExistingDatabaseIdentity = Readonly<{ realpath: string; device: string; inode: string }>;

const descriptorIdentities = (): Map<number, fs.BigIntStats> => {
  if (process.platform !== 'linux') throw new Error('existing_startup_descriptor_platform_unsupported');
  const result = new Map<number, fs.BigIntStats>();
  for (const name of fs.readdirSync('/proc/self/fd')) {
    try { result.set(Number(name), fs.fstatSync(Number(name), { bigint: true })); }
    catch (error) { if ((error as NodeJS.ErrnoException).code !== 'EBADF') throw error; }
  }
  return result;
};

const matchesExistingDatabase = (metadata: fs.BigIntStats, expected: ExistingDatabaseIdentity): boolean =>
  metadata.isFile() && String(metadata.dev) === expected.device && String(metadata.ino) === expected.inode;

function openExistingStartupConnection(dbPath: string, expected: ExistingDatabaseIdentity, phase: string, target: ReturnType<typeof observeCompatibleForwardDatabase>): Database.Database {
  const assertIdentity = () => {
    if (dbPath !== expected.realpath || fs.realpathSync(dbPath) !== expected.realpath
      || !matchesExistingDatabase(fs.lstatSync(dbPath, { bigint: true }), expected)) {
      throw new Error('existing_startup_database_identity_changed');
    }
  };
  assertIdentity();
  const before = descriptorIdentities();
  if ([...before.values()].some(metadata => matchesExistingDatabase(metadata, expected))) {
    throw new Error('existing_startup_database_descriptor_preexisting');
  }
  if (!['claimed', 'security_startup_authorized', 'serving'].includes(phase)) throw new Error('root_security_startup_admission_required');
  const recheckFiles = inspectExistingStartupFiles(expected);
  const database = new Database(dbPath, { fileMustExist: true, readonly: phase === 'claimed' });
  try {
    const descriptors = [...descriptorIdentities()].filter(([fd, metadata]) =>
      !before.has(fd) && matchesExistingDatabase(metadata, expected));
    if (descriptors.length !== 1) throw new Error('existing_startup_database_descriptor_ambiguous');
    assertIdentity();
    if (database.pragma('journal_mode', { simple: true }) !== 'wal') throw new Error('existing_startup_wal_required');
    database.pragma('foreign_keys = ON');
    recheckFiles();
    const observation = observeCompatibleForwardDatabase(database);
    if (!target || Object.entries(observation).some(([key, value]) => value !== target[key as keyof typeof target])) {
      throw new Error('existing_startup_database_target_drift');
    }
    inspectExistingSecurityState(database, process.env);
    inspectExistingConnectorPolicyV2Substrate(database, `${dbPath}.connector-runtime-authority.json`);
    assertIdentity();
    if (!matchesExistingDatabase(fs.fstatSync(descriptors[0][0], { bigint: true }), expected)) {
      throw new Error('existing_startup_database_identity_changed');
    }
    recheckFiles();
    return database;
