 ...
 * 29-message conversation sat in a second row beside it. Measured on the fixture DB:
 * 7 such orphan rows.
 *
...
const REAL = '00000001-0000-4000-8000-000000000001';
...
function seedStubWithRecords(uid: number) {
  ...
  db.prepare(
    `INSERT INTO message_coordination_ingress
     ...
     VALUES (?, 'client-adopt', ?, 'antigravity', 'hello', 'hash-1', 'fp-1', 'delegate', '2026-07-27T15:05:00Z')`,
  ).run(STUB, uid);
}
...
test('the stub is removed and its records follow the conversation id', async () => withIsolatedDatabase(() => {
  ...
  const coordination = db.prepare(
    'SELECT session_id AS sessionId FROM message_coordination_ingress WHERE client_msg_id = ?',
  ).get('client-adopt') as { sessionId: string };
  assert.equal(coordination.sessionId, REAL, 'coordination metadata must follow the adopted runtime id');
