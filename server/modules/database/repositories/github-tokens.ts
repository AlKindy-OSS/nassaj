...
import { decryptCredentialValue } from '@/modules/database/database-credential-crypto.js';
...
type GithubTokenLookup = Omit<CredentialRow, 'credential_value'> & {
  github_token: string;
};
...
export const githubTokensDb = {
  ...
  getGithubTokenById(userId: number, tokenId: number): GithubTokenLookup | null {
    ...
    try {
      const githubToken = decryptCredentialValue(row.credential_value, {
        id: row.id,
        userId: row.user_id,
        credentialType: row.credential_type,
      });
      const { credential_value: _encryptedValue, ...publicRow } = row;
      return { ...publicRow, github_token: githubToken };
    } catch {
      return null;
    }
  },
