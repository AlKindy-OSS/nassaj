...
test('B-138 / ADR-089: the WRITE gate outlives project visibility — read is shared, write is not', async () => {
  await withIsolatedDatabase(() => {
    const creator = userDb.createUser('creator', 'hash', 'user');
    const stranger = userDb.createUser('stranger', 'hash', 'user');

    const created = projectsDb.createProjectPath('/workspace/shared-proj', 'Shared', creator.id);
    const projectId = requireRow(created.project, 'created project row').project_id;

    assert.equal(projectsDb.isProjectWritableByUser(projectId, creator.id), true, 'creator writable');
    assert.equal(projectsDb.isProjectWritableByUser(projectId, stranger.id), false, 'non-member not writable');
    // ADR-089 retired project visibility: the read gate now shares every project
    // with the team, and the write gate is the ONLY thing still separating them.
    // This pair is the regression net against "shared read" quietly widening
    // into "shared write".
    assert.equal(projectsDb.isProjectVisibleToUser(projectId, stranger.id), true, 'read is shared');
  });
});
