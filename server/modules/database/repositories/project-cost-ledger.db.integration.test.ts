...
test('model totals merge the same model across vendors and harnesses into one row', async () => {
  await withIsolatedDatabase(() => {
    projectCostLedgerDb.replaceSource(
      { sourceKey: '/transcripts/claude.jsonl', provider: 'claude', mtimeMs: 1, sizeBytes: 1 },
      [row({ costUsd: 2, requests: 3, vendor: 'anthropic', harness: 'claude' })],
    );
    projectCostLedgerDb.replaceSource(
      { sourceKey: '/transcripts/codex.jsonl', provider: 'codex', mtimeMs: 1, sizeBytes: 1 },
      [row({ costUsd: 4, requests: 5, vendor: 'openai', harness: 'codex', assumed: true })],
    );
    projectCostLedgerDb.replaceSource(
      { sourceKey: '/transcripts/opencode.jsonl', provider: 'opencode', mtimeMs: 1, sizeBytes: 1 },
      [row({ costUsd: 0, requests: 2, vendor: 'proxy', harness: 'opencode', priced: false })],
    );

    const models = projectCostLedgerDb.getModelTotals(scope);
