...
export const messageAuthorsDb = {
   ...
   */
  listBySession(sessionId: string, lease?: HistoryReadLease): MessageAuthorRow[] {
    if (!sessionId) {
      return [];
    }
    const db = getConnection();
    const sql = `SELECT
         ...
         WHERE session_id = ?
         ORDER BY id ASC`;
    return lease ? lease.queryRows<MessageAuthorRow>(db, sql, [sessionId])
      : db.prepare(sql).all(sessionId) as MessageAuthorRow[];
  },
};
