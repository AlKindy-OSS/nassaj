...
export type LedgerGroupRow = {
  ...
  conversations: number;
  complete: boolean;
  /** True when at least one row in this group has no published model price. */
  unpriced: boolean;
  /** True when at least one row in this group uses an assumed price. */
  assumed: boolean;
};
...
const toGroupRow = (row: GroupDbRow): LedgerGroupRow => ({
  ...
  unpriced: num(row.unpriced_rows) > 0,
  assumed: num(row.assumed_rows) > 0,
});
