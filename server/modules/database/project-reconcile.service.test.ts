...
test('reconcileProjects: persists package display name for DB-first reads', async () => {
  await withIsolatedDatabase(async (tempDir) => {
    const db = getConnection();
    const existingPath = path.join(tempDir, 'folder-name');
    await mkdir(existingPath, { recursive: true });
    await writeFile(path.join(existingPath, 'package.json'), JSON.stringify({ name: 'detected-package' }));
    db.prepare(`INSERT INTO projects (project_id, project_path) VALUES ('proj-name', ?)`)
      .run(existingPath);

    await reconcileProjects();

    const row = db.prepare(`SELECT detected_name FROM projects WHERE project_id = 'proj-name'`)
      .get() as { detected_name: string | null };
    assert.equal(row.detected_name, 'detected-package');
  });
});
