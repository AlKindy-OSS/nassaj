...
test('every ecosystem*.config.cjs declares the same sentinel in stop_exit_codes', () => {
  ...
  const previousDeployRoot = process.env.NASSAJ_DEPLOY_ROOT;
  process.env.NASSAJ_DEPLOY_ROOT = path.join(REPO_ROOT, '.release-fixture');
  try {
    ...
    if (previousDeployRoot === undefined) delete process.env.NASSAJ_DEPLOY_ROOT;
    else process.env.NASSAJ_DEPLOY_ROOT = previousDeployRoot;
  }
});
