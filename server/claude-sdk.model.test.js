...
// T-1665 — a resumed session's transcript model is an API id (claude-opus-5),
// not a picker alias (opus[1m]); the resume path must accept it verbatim.
test('T-1665: a resumed session model added to the accepted set passes through verbatim (no coercion, no warn)', () => {
  const accepted = buildValidClaudeModelValues({
    OPTIONS: [{ value: 'default' }, { value: 'opus[1m]' }, { value: 'claude-fable-5-1[1m]' }],
    DEFAULT: 'default',
  });
  // Before the fix this exact call coerced to default with a warning.
  const before = mapAndCaptureWarn({ model: 'claude-opus-5' }, accepted);
  assert.equal(before.model, DEFAULT);
  assert.equal(before.warnings.length, 1);
  // The resume path (claude-sdk.js runClaudeSDKQuery) adds the transcript model
  // to the accepted set; the mapper must then pass it through untouched.
  accepted.add('claude-opus-5');
  const after = mapAndCaptureWarn({ model: 'claude-opus-5' }, accepted);
