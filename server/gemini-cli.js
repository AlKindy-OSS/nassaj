...
/**
 * Loads auth-only env keys from the OPERATOR's `~/.gemini/.env`.
 *
 * B-548 — this used to start from `process.env.GEMINI_CLI_HOME || os.homedir()`,
 * and that read was doubly inert: the server process never sets
 * GEMINI_CLI_HOME (only resolveProviderEnv did, on the CHILD's env, after this
 * runs), and no CLI reads the name anyway. The `os.homedir()` branch was the
 * only one that ever executed, so dropping the dead read is behaviour-identical.
 *
 * SCOPE, stated plainly rather than implied: this lookup is OPERATOR-scoped by
 * construction — it takes no userId and runs BEFORE resolveProviderEnv, so keys
 * found here are injected into every member's spawn. It is inert on a host with
 * no `~/.gemini/.env` (the case here), but it is a real shared-credential
 * channel the moment that file exists. Making it per-user means either reading
 * it from the resolved HOME (which would move the SEC-ENV-1 host-secret strip
 * off the last position) or resolving twice — a design call left to the owner,
 * NOT taken here.
 */
async function loadGeminiUserLevelEnv() {
    const geminiCliHome = os.homedir();
    const envCandidates = [
...
async function spawnGemini(command, options = {}, ws) {
  ...
  // Gemini retains measured conversation totals, but its live ID cannot join native history.
    ...
    // T-1315 (الموجة الثانية): مستوى تنسيق الجولة يركب هذه المطالبة — قناة gemini
    // الوحيدة. الإنفاذ **نصّي**: لا مفتاح عمقٍ للوكلاء موثَّقاً في gemini-cli، فلا
    // يُضبط اسمٌ بلا قارئ (‏B-548). `direct` يُعيد الأمر حرفياً فالسلوك كما كان.
    const promptToSend = withCoordinationDirective(command, options?.coordinationLevel);
    if (command && command.trim()) {
        args.push('--prompt', promptToSend);
    }
    ...
    if (images && images.length > 0) {
        try {
            ...
            if (tempImagePaths.length > 0 && command && command.trim()) {
                ...
                const modifiedCommand = promptToSend + imageNote;

                // Update the command in args
                const promptIndex = args.indexOf('--prompt');
                if (promptIndex !== -1 && args[promptIndex + 1] === promptToSend) {
    ...
    // GEMINI_PATH override, then PATH, then the well-known install dirs (B-1138).
    const geminiPath = resolveCliExecutablePath('gemini', { override: process.env.GEMINI_PATH });
    ...
    // Per-user credential isolation (B-ISO-GEMINI / B-548): the central resolver
    // points HOME at the user's isolated tree so each user has their own gemini
    // config/state and their own OAuth token, while conversations stay shared
    // via symlinks. It used to set GEMINI_CLI_HOME here instead — a variable the
    // CLI does not read, which left every member's turn on the operator's
    // credentials. Returns the base env unchanged when no userId is present.
    ...
    return new Promise((resolve, reject) => {
        ...
        // B-395: register the run with the process monitor so a Gemini turn gets
        // the same "Running" badge / busy dot / active-conversations entry a
        // Claude turn gets. The pid is known here, so frozen (kill -STOP)
        // detection and dead-child reaping come along for free.
        ...
        const notifyTerminalState = ({ code = null, error = null } = {}) => {
            ...
            // B-395: the one place every terminal path passes through — clear
            // the badge here or it outlives the run.
            runPresence.end();
        ...
        let responseHandler;
        if (ws) {
          responseHandler = new GeminiResponseHandler(observeProviderErrors(ws, reason => { terminalFailureReason = reason; }), {
                ...
                onInit: (event) => {
                    ...
                    if (!capturedSessionId) {
                        ...
                        // B-395: move the badge off the temporary key.
        ...
        geminiProcess.stderr.on('data', (data) => {
            ...
            const socketSessionId = typeof ws.getSessionId === 'function' ? ws.getSessionId() : (capturedSessionId || sessionId);
          terminalFailureReason = errorMsg;
        ...
        geminiProcess.on('close', async (code, signal) => {
            if (!providerSucceeded(code, terminalFailureReason, geminiProcess.nassajAborted || signal) && code === 0) code = 1;
            ...
            // Flush may supply the first model activity; stop before persistence begins.
            ...
            // forceFlush can emit a buffered error; recheck after parsing it.
            if (!providerSucceeded(code, terminalFailureReason, geminiProcess.nassajAborted || signal) && code === 0) code = 1;

            const durableTiming = code === 0 && sawAssistantOutput
                ? settleTurnTiming({
                    sessionId: finalSessionId,
            ...
            ws.send(createNormalizedMessage({ kind: 'complete', exitCode: code, isNewSession: !sessionId && !!command, sessionId: finalSessionId, provider: 'gemini', ...durableTiming }));
        ...
        geminiProcess.on('error', async (error) => {
          terminalFailureReason = error.message || 'Provider process error';
