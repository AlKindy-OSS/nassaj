 ...
 *
 * Two keys are seeded: `safe-restart` (defers on live sessions) and
 * `force-restart` (owner-confirmed, kills live sessions first — T-1677). A future
 * action is added by appending a frozen key here (+ qa-critic) — never by
 * accepting a command from the request. An entry may carry
 * `requiresConfirmation:true` to opt into the route's two-step confirm contract.
 */
export const SERVER_ACTIONS = Object.freeze({
  'safe-restart': Object.freeze({
    ...
    minRole: 'owner',
    cmd: 'bash',
    args: Object.freeze(['scripts/preview-safe-restart.sh', '--exec']),
    gateArgs: Object.freeze(['scripts/preview-safe-restart.sh', '--json']),
    detachExec: true,
    cwd: APP_ROOT,
    commandPreview: 'bash scripts/preview-safe-restart.sh --exec',
    description: 'Drain-safe restart with exact OID preview activation when queued (B-95). Owner-triggered from the command board.',
  ...
  // force-restart (ADR-066, T-1677): the owner-confirmed restart that RESTARTS
  // EVEN WHEN LIVE CHAT SESSIONS EXIST, by KILLING them first. This is NOT the
  // raw `pm2 restart` (still deliberately absent — it re-opens B-95/502). The
  // difference is that the kill is EXPLICIT, OWNER-CONFIRMED and LOGGED: the
  // route runs the read-only gate, and if live sessions are found it returns
  // `confirm_required` (never runs) until the client re-submits with
  // `{ confirmKillSessions:true, confirmedSessionCount:N }`. Only then does the
  // detachExec spawn set NASSAJ_RESTART_KILL_SESSIONS=1, which safe-restart.sh
  // turns into `--kill-sessions --force`: SIGTERM → ≤10s grace → SIGKILL →
  // re-detect → fail-closed (exit 6, no restart) if any session survives. The
  // kill is delivered by ENV VAR, not argv: the live OID path runs safe-restart
  // via the dist-server capsule, which decides its own argv (we cannot inject
  // flags there) and preview-oid-owner-action.mjs rejects unknown args — the env
  // var is the one channel that reaches safe-restart.sh through BOTH paths
  // unchanged. `requiresConfirmation` is surfaced in the client catalog so the UI
  // knows to run the two-step confirm; it NEVER exposes cmd/args/gateArgs.
  'force-restart': Object.freeze({
    labelKey: 'forceRestart',
    minRole: 'owner',
    cmd: 'bash',
    // SAME exec command as safe-restart. The kill is NOT an argv flag here (it
    // would crash preview-oid-owner-action.mjs's strict arg parser and be
    // dropped by the OID capsule); it is delivered via NASSAJ_RESTART_KILL_SESSIONS
    // on the detached spawn (see system.js executeActionRow). T-1677.
    args: Object.freeze(['scripts/preview-safe-restart.sh', '--exec']),
    // Confirmation gate = the mutable-root safe-restart.sh --json session detector.
    // NOT preview-safe-restart.sh (which REFUSES --json while an OID control
    // request exists), because the two-step confirm MUST be able to count live
    // sessions in EVERY state to ask the owner "kill N sessions?" before acting.
    gateArgs: Object.freeze(['scripts/safe-restart.sh', '--json']),
    detachExec: true,
    cwd: APP_ROOT,
    commandPreview: 'bash scripts/preview-safe-restart.sh --exec (kills live sessions)',
    description: 'Owner-confirmed restart that kills live chat sessions first (ADR-066, T-1677).',
    globalIdempotent: true,
    // Two-step execution: the route returns confirm_required when live sessions
    // are detected, and only kills after an explicit confirmed count. Surfaced in
    // the client catalog (toPublicCatalog) so the UI drives the confirm prompt.
...
/**
 * Two-step confirmation decision for a requiresConfirmation action (force-restart,
 * T-1677). Given the live count the read-only gate just measured and the client's
 * confirmation, returns whether the route must ASK the owner (again) BEFORE
 * killing anything. Pure — the single source of the "ask vs proceed" rule so the
 * route and its tests agree.
 *
 * Ask (return true) when ANY of:
 *   - the client did not confirm (`confirmKill !== true`);
 *   - no valid confirmed count was supplied (guards a confirm with no number);
 *   - the live set GREW past what the owner authorised (`currentCount > N`) — a
 *     stale confirmation must not silently kill MORE than the owner saw.
 * Proceed (return false) only when confirmed AND the live set did not grow.
 *
 * @param {{ confirmKill?: unknown, confirmedCount?: unknown, currentCount?: unknown }} input
 * @returns {boolean}
 */
export function needsKillConfirmation({ confirmKill, confirmedCount, currentCount } = {}) {
  if (confirmKill !== true) return true;
  if (!Number.isInteger(confirmedCount) || confirmedCount < 0) return true;
  const current = Number.isInteger(currentCount) ? currentCount : Number.POSITIVE_INFINITY;
  return current > confirmedCount;
}
...
export const SAFE_SERVER_BUILD_ID = /^[a-f0-9]{64}$/;
 ...
 *           requestedBy?: unknown, expectedServerBuildId?: unknown }} input
...
export function buildPendingAction(input, options = {}) {
  ...
  let expectedServerBuildId = null;
  if (src.expectedServerBuildId !== undefined && src.expectedServerBuildId !== null) {
    if ((actionType !== 'safe-restart' && actionType !== 'force-restart')
      || typeof src.expectedServerBuildId !== 'string'
      || !SAFE_SERVER_BUILD_ID.test(src.expectedServerBuildId)) {
      return { ok: false, error: 'Invalid expectedServerBuildId (must be 64 lowercase hex for safe-restart/force-restart)' };
    }
    expectedServerBuildId = src.expectedServerBuildId;
  }
 ...
 *           requestedAt: string, expectedServerBuildId?: string|null }} row
...
export function toPublic(row, resolve = getAction) {
  ...
  return {
    ...
    expectedServerBuildId: row.expectedServerBuildId ?? null,
 ...
 *
 * `requiresConfirmation` is a boolean DISPLAY hint (T-1677): it tells the client
 * an action runs a two-step confirm (e.g. force-restart asks "kill N sessions?"
 * before acting). It is advisory for the UI; the server enforces the confirm
 * contract at execution time. Still symbolic only — no cmd/args/gateArgs leak.
 *
 * @returns {Array<{ actionType: string, label: string, commandPreview: string,
 *                   minRole: string, requiresConfirmation: boolean }>}
 */
