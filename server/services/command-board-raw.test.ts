...
// ── (25) T-1684: every finished raw exec is remembered for one hour ───────────
//
// Executing a raw row DELETES it from the queue before the spawn, so before this
// the outcome lived only in the HTTP response: closing the tab lost it.
test('25: a finished raw exec is recorded in history and served to a raw-tier caller', async () => {
  resetState();
  enableRaw();
  nextExitCode = 0;
  spawnCalls = [];
  ...
  const { id, digest } = (await ins.json()).command;
  const exe = await call('POST', `/api/system/command-board-raw/${id}/execute`, {
    user: OWNER, body: { confirmationDigest: digest },
  });
  assert.equal(exe.status, 200);

  const stored = listRawHistory();
  assert.equal(stored.length, 1, 'exactly one record per execution');
  assert.equal(stored[0]!.id, id);
  assert.equal(stored[0]!.command, command);
  assert.equal(stored[0]!.outcome, 'success');
  assert.equal(stored[0]!.exitCode, 0);
  assert.equal(stored[0]!.stdoutTail, 'out-line\n');
  assert.equal(stored[0]!.stderrTail, 'err-line\n');
  assert.equal(stored[0]!.executedBy, 'owner');

  const board = await call('GET', '/api/system/pending', { user: OWNER });
  const item = (await board.json()).history.find((entry: { id: string }) => entry.id === id);
  assert.ok(item, 'the record reaches the command board history');
  assert.equal(item.kind, 'raw');
  assert.equal(item.commandPreview, command);
  // A raw exec has no allowlist entry, so its LABEL is the command itself — not
  // an i18n key the UI cannot resolve and would print raw (qa-critic, T-1684).
  assert.equal(item.label, command);
  assert.equal(item.outcome, 'success');
  assert.equal(item.retryable, false, 'a raw command is never re-run from history');
  assert.equal(Date.parse(item.expiresAt) - Date.parse(item.executedAt), 60 * 60 * 1000);
});

test('25b: a raw history label is the first line only, bounded to 80 characters', async () => {
  resetState();
  enableRaw();
  const long = `echo ${'x'.repeat(200)}`;
  recordRawExecution({ id: 'raw-long', command: long, outcome: 'success', exitCode: 0 });
  recordRawExecution({
    id: 'raw-multiline', command: 'echo first\necho second', outcome: 'success', exitCode: 0,
  });

  const history = (await (await call('GET', '/api/system/pending', { user: OWNER })).json()).history;
  const byId = (id: string) => history.find((entry: { id: string }) => entry.id === id);

  assert.equal(byId('raw-multiline').label, 'echo first', 'only the first line names the row');
  assert.equal(byId('raw-multiline').commandPreview, 'echo first\necho second',
    'the preview still carries the whole command');

  const label = byId('raw-long').label;
  assert.equal(label.length, 80, 'a long one-liner is bounded');
  assert.ok(label.endsWith('\u2026'), 'and marked as truncated');
  assert.equal(byId('raw-long').commandPreview, long, 'without truncating the preview');
});

test('26: a non-zero exit is remembered as a failure and a kill as unknown', async () => {
  resetState();
  enableRaw();
  nextExitCode = 3;
  const id = seedRow('exit 3');
  await call('POST', `/api/system/command-board-raw/${id}/execute`, {
    user: OWNER, body: { confirmationDigest: computeDigest('exit 3') },
  });
  assert.equal(listRawHistory()[0]!.outcome, 'failure');
  assert.equal(listRawHistory()[0]!.exitCode, 3);

  // A signalled process reports a null exit code: it proves nothing either way.
  resetState();
  enableRaw();
  nextExitCode = null as unknown as number;
  const killedId = seedRow('sleep 1');
  await call('POST', `/api/system/command-board-raw/${killedId}/execute`, {
    user: OWNER, body: { confirmationDigest: computeDigest('sleep 1') },
  });
  assert.equal(listRawHistory()[0]!.outcome, 'unknown');
  nextExitCode = 0;
});

test('27: raw history is withheld from a caller without the raw tier', async () => {
  resetState();
  enableRaw();
  recordRawExecution({ id: crypto.randomUUID(), command: 'echo hidden', outcome: 'success', exitCode: 0 });

  const armed = await call('GET', '/api/system/pending', { user: OWNER });
  assert.equal((await armed.json()).history.filter((e: { kind: string }) => e.kind === 'raw').length, 1);

  // Disarming the master switch lowers every role at once (ADR-072).
  setCommandBoardConfig({ rawExecEnabled: false });
  const disarmed = await call('GET', '/api/system/pending', { user: OWNER });
  const body = await disarmed.json();
  assert.equal(body.history.some((e: { kind: string }) => e.kind === 'raw'), false,
    'command text must not leak to a caller who may not read the raw queue');

  enableRaw();
  const admin = await call('GET', '/api/system/pending', { user: ADMIN });
  assert.equal((await admin.json()).history.some((e: { kind: string }) => e.kind === 'raw'), false,
    'a role without the raw tier sees no raw history');
});

test('28: history is capped, secret-redacted, output-tailed, and pruned after an hour', () => {
  resetState();
  for (let index = 0; index < MAX_RAW_HISTORY + 5; index++) {
    recordRawExecution({ id: `rec-${index}`, command: `echo ${index}`, outcome: 'success', exitCode: 0 });
  }
  const capped = listRawHistory();
  assert.equal(capped.length, MAX_RAW_HISTORY, 'the store is bounded');
  assert.equal(capped[0]!.id, `rec-${MAX_RAW_HISTORY + 4}`, 'newest first');

  resetState();
  recordRawExecution({
    id: 'secret-run', command: 'curl -H "Authorization: Bearer abcdef1234567890" https://x',
    outcome: 'success', exitCode: 0, stdout: 'y'.repeat(MAX_RAW_OUTPUT_TAIL_BYTES + 500) + 'END',
  });
  const entry = listRawHistory()[0]!;
  assert.ok(!entry.command.includes('abcdef1234567890'), 'a labelled credential is masked');
  assert.equal(Buffer.byteLength(entry.stdoutTail, 'utf8'), MAX_RAW_OUTPUT_TAIL_BYTES);
  assert.ok(entry.stdoutTail.endsWith('END'), 'the TAIL is kept — that is what diagnoses');

  // A record older than the retention window is dropped on read.
  appConfigDb.set(RAW_HISTORY_CONFIG_KEY, JSON.stringify([{
    id: 'stale', command: 'echo old', outcome: 'success', exitCode: 0,
    executedAt: new Date(Date.now() - 61 * 60 * 1000).toISOString(),
  }]));
  assert.deepEqual(listRawHistory(), []);
});

test('29: DELETE history/:id removes one record and needs the raw tier', async () => {
  resetState();
  enableRaw();
  recordRawExecution({ id: 'drop-me', command: 'echo x', outcome: 'success', exitCode: 0 });

  const denied = await call('DELETE', '/api/system/command-board-raw/history/drop-me', { user: ADMIN });
  assert.equal(denied.status, 403);
  assert.equal(listRawHistory().length, 1, 'a refused caller changes nothing');

  const ok = await call('DELETE', '/api/system/command-board-raw/history/drop-me', { user: OWNER });
