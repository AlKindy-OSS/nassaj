...
const outcomeStarts = [];
mock.module('../modules/websocket/services/session-outcome.service.js', {
  namedExports: {
    markRunStarted: (sessionId) => outcomeStarts.push(sessionId),
    markRunEnded: () => {},
  },
});
...
test('re-registering the same live process does not declare a new round', (t) => {
  outcomeStarts.length = 0;
  const sessionId = 'same-live-run';
  const firstWriter = fakeWriter();
  const replacementWriter = fakeWriter();
  t.after(() => monitor.unregisterSessionProcess(sessionId));

  monitor.registerSessionProcess(sessionId, { provider: 'claude', writer: firstWriter });
  monitor.registerSessionProcess(sessionId, { provider: 'claude', writer: replacementWriter });

  assert.deepEqual(outcomeStarts, [sessionId]);
});
