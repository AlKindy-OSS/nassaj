 ...
 * SESSION'S OWN ROOT, or nothing.
 *
 * SINGLE SOURCE for both injection paths (this module and
 * coordinator-session-start.js), so the rule cannot drift between them.
 *
 * NO ENV FALLBACK, BY DECISION (B-1250, closing T-1810). An operator override
 * (`NASSAJ_COORDINATOR_REPO_ROOT`) used to fill in for an unknown root. On the
 * live host that variable is set to the nassaj-dev root, so a session whose
 * root was unknown was handed nassaj-dev's commits and open tasks — exactly the
 * cross-project, cross-user leak T-1810 claimed to have stopped, just through a
 * different door. An override whose only purpose is to guess when the answer is
 * unknown IS the guess. `process.cwd()` is excluded for the same reason: the
 * server process is shared by every session on this instance, so its working
 * directory is never a stand-in for "this session's project".
 *
 * Unknown root ⇒ null ⇒ the caller injects nothing. There is no exception.
 *
 * @param {unknown} repoRoot the session's own project root, if the caller knows it
 * @returns {string|null} the trimmed root, or null when it is unknown
 */
export function resolveSessionRepoRoot(repoRoot) {
  return typeof repoRoot === 'string' && repoRoot.trim() ? repoRoot.trim() : null;
}
