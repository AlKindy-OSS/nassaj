...
/**
 * Locale/prompt defaults for the deliberately rc-free interactive shell.
 * C.UTF-8 is available on the supported Linux runtime and, unlike plain C,
 * lets bash/readline count and edit Arabic input as Unicode characters.
 */
const TERMINAL_UTF8_LOCALE = 'C.UTF-8';
const TERMINAL_PROMPT_INIT = "PS1='\\u@\\h:\\w\\$ '; unset PROMPT_COMMAND";
...
const INVALID_TERMINAL_MODE: StandaloneTerminalFailure = Object.freeze({
  ok: false,
  status: 400,
...
export function createStandaloneTerminal(
  ...
  const mode: ManagedClaudeTerminalMode = input.mode === undefined || input.mode === null
    ? 'general'
    : input.mode === 'general' || input.mode === 'session-bound'
      ? input.mode
      : 'general';
  if (input.mode !== undefined && input.mode !== null && input.mode !== mode) {
    return INVALID_TERMINAL_MODE;
  }
  const sessionId = typeof input.sessionId === 'string' && /^[A-Za-z0-9_.:-]{1,128}$/.test(input.sessionId)
    ? input.sessionId
    : null;
  if (mode === 'session-bound' && !sessionId) {
    return INVALID_TERMINAL_MODE;
  }
  if (mode === 'general' && input.sessionId !== undefined && input.sessionId !== null) {
    return INVALID_TERMINAL_MODE;
  }
  ...
  // T-1675: interactive terminals never run on a delegated credential.
  const isolatedEnv = resolveProviderEnv(userId, 'claude', process.env, 'chat', { honorGrants: false });
  ...
  const managedEnv = installManagedClaudeTerminalEnv(
    { ...isolatedEnv, [prioritizedPath.key]: prioritizedPath.value },
    { userId, mode, sessionId },
  );
  const principal = input.authenticatedPrincipal as { authenticationKind?: unknown; authorizationGeneration?: unknown } | undefined;
  const broker = principal && typeof principal.authenticationKind === 'string'
    && Number.isSafeInteger(principal.authorizationGeneration)
    ? registerManagedClaudeTerminal({
      authenticatedPrincipal: input.authenticatedPrincipal, userId, mode, sessionId, cwd,
      realBinary: managedEnv.NASSAJ_MANAGED_CLAUDE_REAL_BIN!, baseEnv: isolatedEnv,
    })
    : null;

  // Do not use a login/profile shell here: startup files can reorder PATH and
  // place the real Claude binary ahead of the managed wrapper. Because an
  // rc-free bash otherwise falls back to the unhelpful `bash-<version>$`
  // prompt, the spawn env below supplies a deterministic prompt explicitly.
  const shellArgs = initialCommand === null
    ? ['--noprofile', '--norc', '-i']
    : ['--noprofile', '--norc', '-c', initialCommand];
  ...
  const child = pty.spawn(ptyLaunch.cmd, ptyLaunch.args, {
    ...
    env: {
      ...
      // Interactive bash replaces an inherited PS1 with `bash-<version>$`.
      // PROMPT_COMMAND is inherited, runs before the first prompt, and then
      // removes itself so normal commands remain free to customize the prompt.
      PROMPT_COMMAND: TERMINAL_PROMPT_INIT,
    },
    encoding: 'utf8',
  });
  entry.pty = child;
  if (broker) bindManagedClaudeTerminal(broker.selector, child.pid);
