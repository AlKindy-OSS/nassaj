...
// The registry's PTY is mocked; avoid coupling the integration fixture to a
// globally installed Claude CLI on the host.
...
function createTerminal(userId: number, cwd: string): { id: string; fake: FakePty } {
  const created = registry.createStandaloneTerminal({
    userId,
    cwd,
    writerLease: { release() {} },
  });
