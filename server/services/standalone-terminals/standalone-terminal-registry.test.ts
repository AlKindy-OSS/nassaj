...
// The fake PTY never executes this binary. A real inert executable keeps the
// managed-launcher preflight deterministic on clean CI runners.
...
const testWriterLease = Object.freeze({ release() {} });
const createTerminal = (input: Omit<Parameters<typeof registry.createStandaloneTerminal>[0], 'writerLease'>) =>
  registry.createStandaloneTerminal({ ...input, writerLease: testWriterLease });
...
test('ownership: foreign user cannot list/rename/delete; wire carries NO userId', () => {
  ...
  try {
    const created = createTerminal({ userId: 1, cwd });
...
test('limit: 6th running terminal is 409; deleting one reopens; exited do not count', () => {
  ...
  try {
    ...
    const other = createTerminal({ userId: 4, cwd });
    ...
    assert.equal(createTerminal({ userId: 3, cwd }).ok, true);
...
test('spawn env passes resolveProviderEnv → prioritizeUserNpmGlobalBin → resolveCagedLaunch', () => {
  ...
  try {
    const created = createTerminal({ userId: 7, cwd });
    assert.equal(created.ok, true);
    ...
    assert.deepEqual(
      args,
      ['--cage', '--noprofile', '--norc', '-i'],
      'default spawn is interactive without profile PATH mutation, caged',
    );
    ...
    assert.equal(opts.env.LANG, 'C.UTF-8');
    assert.equal(opts.env.LC_ALL, 'C.UTF-8');
    assert.equal(opts.env.LC_CTYPE, 'C.UTF-8');
    assert.equal(opts.env.PROMPT_COMMAND, "PS1='\\u@\\h:\\w\\$ '; unset PROMPT_COMMAND");
    assert.equal(opts.encoding, 'utf8');
    assert.equal(opts.cwd, cwd);

    // Managed Claude launcher is first; the REAL prioritizeUserNpmGlobalBin
    // result stays directly behind it and ahead of every system path.
    const pathEntries = String(opts.env.PATH ?? '').split(path.delimiter).filter(Boolean);
    assert.match(pathEntries[0]!, /server\/bin$/, 'managed Claude launcher leads PATH');
    assert.equal(pathEntries[1], npmGlobalBin, 'isolated npm-global bin follows the launcher');
    assert.equal(opts.env.NASSAJ_MANAGED_CLAUDE_MODE, 'general');

    // Behavioural proof: start the same shell mode (without the mocked cage)
    // and ask bash itself which executable it resolves after startup.
    const resolved = spawnSync('/bin/bash', [
      ...cageCalls[0]!.args,
      '-c',
      'command -v claude',
    ], {
    ...
    assert.match(resolved.stdout.trim(), /server\/bin\/claude$/);

    // The rc-free shell still has a useful prompt and a multibyte character
    // locale, so readline can accept/edit Arabic instead of treating it as
    // bytes under the inherited C locale.
    const shellContract = spawnSync('/bin/bash', [
      ...cageCalls[0]!.args,
      '-c',
      'printf "%s\\n" "$(locale charmap)"',
    ], {
    ...
    assert.equal(shellContract.status, 0, shellContract.stderr);
    assert.equal(shellContract.stdout.trim(), 'UTF-8');

    // initialCommand variant: bash -c '<cmd>' (argv array), caged the same way.
    const withCmd = createTerminal({
      userId: 7,
      cwd,
    ...
    assert.deepEqual(spawnCalls[1].args, ['--cage', '--noprofile', '--norc', '-c', 'echo hi']);
  } finally {
...
test('terminal contract: mode is closed; session-bound is authorized and stamped before PTY spawn', () => {
  resetAll();
  const cwd = makeTempCwd('term-contract-');
  try {
    const bad = createTerminal({ userId: 7, cwd, mode: 'mystery' });
    assert.equal(bad.ok, false);
    assert.equal((bad as { code: string }).code, 'invalid_terminal_mode');
    assert.equal(spawnCalls.length, 0);

    const bound = createTerminal({
      userId: 7,
      cwd,
      mode: 'session-bound',
    ...
    assert.equal(spawnCalls.length, 1);
    assert.equal(spawnCalls[0]!.opts.env.NASSAJ_MANAGED_CLAUDE_MODE, 'session-bound');
    assert.equal(spawnCalls[0]!.opts.env.NASSAJ_MANAGED_CLAUDE_SESSION_ID, 'sess-owned');
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
});
...
test('validation: title and initialCommand bounds; server title fallback', () => {
  ...
  try {
    for (const badTitle of ['', '   ', 'x'.repeat(65), 42, {}]) {
      const result = createTerminal({ userId: 5, cwd, title: badTitle });
    ...
    for (const badCmd of ['x'.repeat(4097), 42, []]) {
      const result = createTerminal({ userId: 5, cwd, initialCommand: badCmd });
    ...
    assert.equal(
      createTerminal({ userId: 5, cwd, title: 'x'.repeat(64) }).ok,
      true
    );
    const emptyCmd = createTerminal({ userId: 5, cwd, initialCommand: '   ' });
    ...
    const fallback = createTerminal({ userId: 6, cwd });
...
test('cwd: nonexistent path and invisible private project return the IDENTICAL failure', () => {
  ...
  try {
    ...
    visibleResult = false;
    const invisible = createTerminal({ userId: 9, cwd: privateDir });
    ...
    visibleResult = true;
    assert.equal(createTerminal({ userId: 9, cwd: privateDir }).ok, true);
  } finally {
...
test('buffer: ordered replay; >2MiB head-drop sets truncated; detach never kills', () => {
  ...
  try {
    const created = createTerminal({ userId: 11, cwd });
...
test('second attach displaces the first and takes over the live stream', () => {
  ...
  try {
    const created = createTerminal({ userId: 12, cwd });
    const id = (created as { terminal: { id: string } }).terminal.id;
    const fake = spawnCalls[0].fake;
...
test('exit: frame to viewer, record+buffer survive, input reports instead of throwing', () => {
  ...
  try {
    const created = createTerminal({ userId: 13, cwd });
    const id = (created as { terminal: { id: string } }).terminal.id;
    const fake = spawnCalls[0].fake;
...
test('exited records are trimmed oldest-first beyond MAX_EXITED_PER_USER', () => {
  ...
  try {
    ...
    for (let batch = 0; batch < 3; batch += 1) {
      ...
      for (let i = 0; i < 4; i += 1) {
        const created = createTerminal({ userId: 14, cwd });
        assert.equal(created.ok, true);
        ids.push((created as { terminal: { id: string } }).terminal.id);
      }
      for (let i = 0; i < 4; i += 1) {
...
test('UTF-8 Arabic input and resize reach a running PTY; foreign input reports gone', () => {
  resetAll();
  const cwd = makeTempCwd('term-io-');
  try {
    const created = createTerminal({ userId: 15, cwd });
    const id = (created as { terminal: { id: string } }).terminal.id;
    const fake = spawnCalls[0].fake;

    assert.deepEqual(registry.writeStandaloneTerminalInput(15, id, 'printf "مرحبا"\r'), {
      outcome: 'written',
    });
    assert.deepEqual(fake.writes, ['printf "مرحبا"\r']);
    ...
    assert.deepEqual(fake.writes, ['printf "مرحبا"\r'], 'foreign input never reached the PTY');
  } finally {
    fs.rmSync(cwd, { recursive: true, force: true });
  }
});
