...
test('the board holds no duplicate issue ids', () => {
...
test('the board holds no duplicate task ids', () => {
  const dupes = duplicates(board().tasks);
  assert.deepEqual(dupes, [], `Duplicate T- ids.\n${dupes.join('\n')}`);
});

test('an id is never reused across issues and tasks', () => {
...
test('every entry carries a well-formed string id', () => {
  const { issues, tasks } = board();
  const bad = [
...
test('the board is parseable and non-empty', () => {
