 ...
 * isolation knobs CLAUDE_CONFIG_DIR / CODEX_HOME / KIMI_CODE_HOME / the vendor
 * KEY vars) is deliberately NOT listed here.
 *
...
export const NASSAJ_HOST_SECRET_EXACT_DENY = Object.freeze([
  ...
  //
  // B-378 — the two below are INHERITED Anthropic credentials, and they are here
  // because "EXACT only" left them behind: `API_KEY` does not match
  // `ANTHROPIC_API_KEY`, and no suffix rule covers it either. So an
  // ANTHROPIC_API_KEY (or _AUTH_TOKEN) present in nassaj's own process env
  // reached EVERY claude spawn, for EVERY user — and the CLI reads an env
  // credential at the TOP of its precedence, above the per-user config dir. One
  // operator-level variable would therefore have silently moved the whole fleet
  // off its subscription and onto metered billing, with nothing on any screen
  // saying so.
  //
  // Stripping them here is safe because nothing legitimately reads them from the
  // spawn env: the two readers in the codebase
  // (claude-auth.provider.ts:160, claude-onboarding.service.js:56) read the
  // per-user settings.json, not this environment. And the engine seam
  // (apply-claude-engine-provider-env.js) SETS both names on the already-
  // sanitized env AFTER this runs — claude-sdk.js:2643 sanitizes, :2764 injects
  // — so a deliberately engaged engine is untouched while an inherited one dies.
  //
  // Not a prefix rule: ANTHROPIC_BASE_URL must keep flowing to the iron-rule
  // guard (anthropic-base-url-guard.js), which REJECTS a competitor host loudly.
  // Silently deleting it would turn a fail-closed refusal into a silent
  // fallback to official Anthropic — trading a visible error for an invisible
