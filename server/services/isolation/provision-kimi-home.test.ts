...
describe('provisionUserDirs — Kimi config-home block (M-1)', () => {
  // B-383 corrected this contract. The old assertion demanded a `.kimi-code/` subdir
  // "the mcp.json home" — which kimi never reads: its user-global file is
  // <KIMI_CODE_HOME>/mcp.json, flat, and `.kimi-code/mcp.json` resolves against the
  // CWD instead (kimi v0.28.1 bundled /mcp-config skill; live tree confirms the flat
  // layout). The dir is no longer created, so this now pins the corrected shape.
  it('creates .kimi (owner-only 0700) with a flat layout and sessions/', () => {
    ...
    assert.equal(
      fs.existsSync(path.join(home, '.kimi-code')),
      false,
      'no .kimi-code/ subdir: the user-global mcp.json is flat in the home (B-383)',
    );
  });

  it('resolves the user-global mcp.json flat in the home, where kimi actually reads it', async () => {
    const { resolveKimiAgentMcpConfigPath } = await import('./kimi-agent-mcp-config.js');
    const home = kimiHome(USER_ID);
    assert.equal(resolveKimiAgentMcpConfigPath(home), path.join(home, 'mcp.json'));
  });
