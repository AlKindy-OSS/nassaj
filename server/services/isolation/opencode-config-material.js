 ...
 *
 * ADR-163: the GLM block above stays fixed. Additional reserved local blocks are
 * read from Nassaj's server table for (callerId, current credential principal),
 * with optional per-owner secrets. Provisioning, MCP writes and spawn checks all
 * regenerate that identity-dependent material; saved JSON is never its authority.
 */
...
import { localModelProviderBlocks } from './local-model-config.js';
import {
  ensurePrivateRealDirectory,
  recoverOpenCodeConfigMigration,
  secureNeutralizeOpenCodeFile,
  secureReadOpenCodeText,
  withOpenCodeConfigLockSync,
} from './opencode-config-lock.js';
...
/**
 * The opencode provider id of the carrier block — deliberately NOT `anthropic` (OCC-2
 * collision). Shared with the GL-3 guard so "the glm block may only use the carrier
 * origin" (B-1268) is expressed against one constant, never a repeated literal.
 */
export const GLM_CARRIER_PROVIDER_ID = 'glm';
 ...
 */
export function buildOpenCodeConfig(mcp, callerId = null) {
  const config = {
    provider: {
      [GLM_CARRIER_PROVIDER_ID]: {
        npm: OPENCODE_GLM_NPM,
        options: { baseURL: GLM_CARRIER_BASE_URL },
        models: GLM_CARRIER_MODELS,
      },
      ...localModelProviderBlocks(callerId),
    },
  };
  const overlay = normalizeOpenCodeMcpOverlay(mcp);
...
/**
 * Keeps only OpenCode MCP server definitions suitable for the governed overlay.
 * The carrier provider block remains generated; callers can mutate only `mcp`.
 *
 * @param {unknown} value
 * @returns {Record<string, Record<string, unknown>>}
 */
export function normalizeOpenCodeMcpOverlay(value) {
  const normalized = {};
...
export function openCodeConfigMatches(targetPath, expected = serializeOpenCodeConfig()) {
  try {
    ...
    if (arguments.length < 2) {
      ...
      effectiveExpected = serializeOpenCodeConfig(buildOpenCodeConfig(parsed?.mcp, callerIdFromConfigPath(targetPath)));
    }
    return fingerprintOf(current) === fingerprintOf(effectiveExpected);
  } catch {
    return false;
  }
}
...
const parseLegacyJsonc = (content) =>
  JSON.parse(stripJsonComments(content).replace(/,\s*([}\]])/g, '$1'));
 ...
 * @param {{ mode?: number, callerId?: string | number | null, mcp?: unknown, onError?: (err: unknown) => void, lockHeld?: boolean }} [options]
...
export function materializeOpenCodeConfig(configDir, options = {}) {
  ...
  if (options.lockHeld === true) {
    return materializeOpenCodeConfigUnlocked(configDir, options);
  }

  try {
    return withOpenCodeConfigLockSync(targetPath, () => {
      recoverOpenCodeConfigMigration(targetPath);
      const jsoncPath = `${targetPath}c`;
      const canonicalExists = fs.existsSync(targetPath);
      const jsoncExists = fs.existsSync(jsoncPath);
      if (canonicalExists && jsoncExists) {
        throw new Error('OpenCode has both opencode.json and opencode.jsonc.');
      }

      let migratingPath = null;
      let effectiveOptions = options;
      if (!canonicalExists && jsoncExists) {
        const legacy = parseLegacyJsonc(secureReadOpenCodeText(jsoncPath));
        effectiveOptions = { ...options, mcp: legacy?.mcp, lockHeld: true };
        migratingPath = `${jsoncPath}.migrating-${process.pid}-${crypto.randomUUID()}`;
        fs.renameSync(jsoncPath, migratingPath);
      }

      const ok = materializeOpenCodeConfigUnlocked(configDir, effectiveOptions);
      if (!ok) {
        if (migratingPath && !fs.existsSync(jsoncPath)) fs.renameSync(migratingPath, jsoncPath);
        return false;
      }
      if (migratingPath) secureNeutralizeOpenCodeFile(migratingPath);
      return true;
    });
  } catch (error) {
...
function materializeOpenCodeConfigUnlocked(configDir, options = {}) {
  const { mode = OPENCODE_CONFIG_FILE_MODE, onError } = options;
  const targetPath = path.join(configDir, OPENCODE_CONFIG_FILENAME);

  try {
    ensurePrivateRealDirectory(configDir);
  } catch (error) {
    if (typeof onError === 'function') onError(error);
    return false;
  }

  // Harden the real directory and a real existing file BEFORE the fingerprint
  // fast path. Matching bytes in a 0777 directory or 0666 file are not an
  // acceptable canonical state, and a symlink is never chmodded/followed.
  try {
  ...
  // Only after proving the config home is a real hardened directory may the
  // preservation path read an existing overlay. This avoids even read-following
  // an attacker-controlled directory symlink.
  let mcp = options.mcp;
  if (mcp === undefined) {
    try {
      const targetStat = fs.lstatSync(targetPath);
  ...
  const expected = serializeOpenCodeConfig(buildOpenCodeConfig(mcp, options.callerId ?? callerIdFromConfigPath(targetPath)));
  ...
  try {
    ...
    tmpPath = path.join(
      configDir,
      `.${OPENCODE_CONFIG_FILENAME}.tmp-${process.pid}-${crypto.randomUUID()}`,
    );
    // Atomic replacement prevents a live native reader from observing a torn
    // canonical file. The temporary file starts private and is made read-only
    // before rename, so there is no writable final-path window.
    fs.writeFileSync(tmpPath, expected, { mode: 0o600, flag: 'wx' });
...
/** Existing config writers infer identity only from the canonical per-user layout. */
function callerIdFromConfigPath(targetPath) {
  const matched = targetPath.replaceAll('\\', '/').match(/\/([1-9][0-9]*)\/\.config\/opencode\/opencode\.json$/u);
