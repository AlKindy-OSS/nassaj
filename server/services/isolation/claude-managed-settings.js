...
/**
 * Writes ONE policy key into the operator's own settings.json and re-syncs the
 * managed tier, so the change reaches every session without a restart.
 *
 * WHY THE OPERATOR FILE AND NOT THE MANAGED ONE (ADR-100): `settings.json` is
 * the SOURCE — `syncManagedSettingsFile` rewrites managed-settings.json from it
 * on every spawn, so a value written straight into the managed file is erased
 * moments later. Every other key here follows the same direction.
 *
 * The only caller today is the storage-policy route writing
 * `cleanupPeriodDays`; the measured reason it exists is that the key was set in
 * NO layer at all (2026-08-04), so the install was silently running Claude's
 * 30-day default while the operator believed a longer period was in force.
 * Retention that lives only in someone's memory is not a policy.
 *
 * @param {string} key    must be one of POLICY_KEYS
 * @param {unknown} value JSON-serialisable value
 * @param {string} [operatorHome]
 * @returns {boolean} true when written
 */
export function setOperatorPolicyKey(key, value, operatorHome = os.homedir()) {
  if (!POLICY_KEYS.includes(key)) {
    console.error(`[managed-settings] refusing to write non-policy key "${key}"`);
    return false;
  }
  try {
    const claudeDir = path.join(operatorHome, '.claude');
    const settingsPath = path.join(claudeDir, 'settings.json');
    const settings = safeReadJson(settingsPath);
    if (settings[key] === value) {
      // Already current: skip the write but still re-sync, since the managed
      // tier may have been reset independently.
      syncManagedSettingsFile(claudeDir);
      return true;
    }
    settings[key] = value;
    ...
    console.error(`[managed-settings] failed to write ${key}:`, error.message);
    return false;
  }
}
