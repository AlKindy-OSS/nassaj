...
/**
 * مجلّد مؤقّت **يُحذف عند خروج العملية**.
 *
 * ‏`os.tmpdir()` هنا نظام ملفات يعيش في الذاكرة، فمجلّدٌ لا يُحذف يحجز RAM إلى
 * الأبد لا قرصاً. وهذا الملف وحده كان قد خلّف **384 مجلّداً** مقيسة في
 * 2026-08-03 (‏B-419): ثمانية عشر في كل تشغيلة اختبارات، لا ينظّفها شيء —
 * وعتبة `systemd-tmpfiles` عشرة أيام فلا تصل إليها في أي أسبوع عملٍ عادي.
 */
const tmpDirs: string[] = [];

function tmpDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'vbi-'));
  tmpDirs.push(dir);
  return dir;
}
...
process.on('exit', () => {
  for (const dir of tmpDirs) {
    try {
      ...
      // خروجُ العملية ليس مكانَ رمي الأخطاء؛ المجلّد يبقى وتلتقطه المكنسة.
    }
  }
});
