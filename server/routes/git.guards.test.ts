...
// Construct the provider-shaped prefix at runtime so the public source never
// contains a string that secret scanners can mistake for a usable credential.
const SECRET_TOKEN = ['gh', 'p_', 'SyntheticRedactionFixture012345'].join('');
const TOKEN_PUSH_URL = `https://${SECRET_TOKEN}@github.com/owner/repo.git`;
let askpassCleanupCalls = 0;

mock.module('@/modules/projects/services/git-transport-security.service.js', {
  namedExports: {
    appendBoundedDiagnostic: (current: string, chunk: unknown) =>
      `${current}${String(chunk ?? '')}`.slice(-16 * 1024),
    parseCanonicalGitHubUrl: (raw: unknown) => {
      if (raw !== 'https://github.com/owner/repo.git') throw new Error('INVALID_GITHUB_URL');
      return { owner: 'owner', repo: 'repo', cloneUrl: String(raw) };
    },
    createGitAskpassLease: async (_token: string | null) => ({
      env: {
        ...process.env,
      ...
      cleanup: async () => { askpassCleanupCalls += 1; },
    }),
  },
});
...
test('B-GIT-SEC-3: direct push fails closed before credentials or Git are reached', async () => {
  currentUserId = 9;
  ...
  assert.match(String(res.json.error), /owner-authorized release broker/);
  assert.equal(spawnCalls.length, 0);
  assert.equal(askpassCleanupCalls, 0);
  assert.ok(!res.text.includes(SECRET_TOKEN));
});

test('B-GIT-SEC-3: direct publish also fails closed before credentials or Git', async () => {
  currentUserId = 9;
  storedGithubToken = SECRET_TOKEN;
