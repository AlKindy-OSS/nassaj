...
// B-42 end-to-end: the file that reaches disk must still carry the logo's colour.
// Sanitization removes <style>/style (they are the CSS-injection carriers), so
// without the colour migration the stored logo has no fill at all and renders as
// solid black — unreadable on a dark background. Asserted on the stored bytes,
// because those are what the browser will be served.
test('POST branding/logo keeps the colour of a real Illustrator logo (<style> class rule)', async () => {
  const srv = await buildServer();
  try {
    fs.rmSync(path.join(BRANDING_DIR, 'logo.svg'), { force: true });
    const { status } = await srv.uploadLogo('logo.svg', 'image/svg+xml', REAL_ILLUSTRATOR_SVG);
    assert.equal(status, 200);
    const stored = fs.readFileSync(path.join(BRANDING_DIR, 'logo.svg'), 'utf8');
    assert.match(stored, /fill="#edecec"/, 'the .st0 colour must survive as a presentation attribute');
    assert.doesNotMatch(stored, /<style/i, 'while the CSS carrier itself is still removed');
    assert.doesNotMatch(stored, /style=/i);
  } finally {
    await srv.close();
  }
});

test('POST branding/logo accepts a real Inkscape logo and keeps its style="" colour', async () => {
  const srv = await buildServer();
  try {
    fs.rmSync(path.join(BRANDING_DIR, 'logo.svg'), { force: true });
    const { status } = await srv.uploadLogo('logo.svg', 'image/svg+xml', REAL_INKSCAPE_SVG);
    assert.equal(status, 200, 'an Inkscape export (xmlns:svg declared) must not be rejected');
    const stored = fs.readFileSync(path.join(BRANDING_DIR, 'logo.svg'), 'utf8');
    assert.match(stored, /^<svg[\s>]/i);
    assert.match(stored, /fill="#ffffff"/, 'the white fill must survive for the dark-theme logo');
  } finally {
    await srv.close();
  }
});
