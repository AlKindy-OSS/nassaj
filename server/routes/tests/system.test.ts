...
import systemRouter, {
  ...
  round1,
  round2,
  setReleaseDiscoveryForTests,
  setStatfsReaderForTests,
  storageStatsFromStatfs,
} from '../system.js';
...
test('storageStatsFromStatfs uses bavail so reserved blocks count as unavailable', () => {
  assert.deepEqual(
    storageStatsFromStatfs({ bsize: 4096, blocks: 1000, bfree: 250, bavail: 200 }),
    { usedBytes: 3_276_800, totalBytes: 4_096_000, percent: 80 }
  );
});

test('storageStatsFromStatfs rejects unusable or overflowing filesystem counters', () => {
  assert.equal(storageStatsFromStatfs({ bsize: 0, blocks: 1000, bavail: 250 }), null);
  assert.equal(storageStatsFromStatfs({ bsize: 4096, blocks: 0, bavail: 0 }), null);
  assert.equal(storageStatsFromStatfs({ bsize: 4096, blocks: 1000, bavail: -1 }), null);
  assert.equal(storageStatsFromStatfs({ bsize: 4096, blocks: 1000, bavail: 1001 }), null);
  assert.equal(
    storageStatsFromStatfs({ bsize: Number.MAX_SAFE_INTEGER, blocks: 2, bavail: 1 }),
    null
  );
  assert.equal(storageStatsFromStatfs({ bsize: Number.POSITIVE_INFINITY, blocks: 2, bavail: 1 }), null);
});
...
test('GET /stats returns the documented contract with sane ranges', async () => {
  ...
  try {
    ...
    assert.ok(body.storage, 'the test filesystem should be measurable with statfs');
    assert.equal(typeof body.storage.usedBytes, 'number');
    assert.equal(typeof body.storage.totalBytes, 'number');
    assert.equal(typeof body.storage.percent, 'number');
    assert.ok(body.storage.totalBytes > 0);
    assert.ok(body.storage.usedBytes >= 0 && body.storage.usedBytes <= body.storage.totalBytes);
    assert.ok(body.storage.percent >= 0 && body.storage.percent <= 100);
    assert.equal(Number(body.storage.percent.toFixed(1)), body.storage.percent);
  } finally {
    server.close();
  }
});

test('GET /stats keeps serving other telemetry when statfs fails', async () => {
  setStatfsReaderForTests(async () => {
  ...
  try {
    ...
    server.close();
    setStatfsReaderForTests(null);
  }
});

test('GET /release/latest returns a sanitized OSS contract with no browser cache', async () => {
  setReleaseDiscoveryForTests(async () => ({
    release: {
      version: '1.42.0.5',
      tagName: 'v1.42.0.5',
      title: 'OSS release',
      notes: 'Release notes',
      htmlUrl: 'https://github.com/AlKindy-OSS/nassaj/releases/tag/v1.42.0.5',
      publishedAt: '2026-08-17T04:35:06.000Z',
    },
  }));
  const app = express();
  app.use('/api/system', systemRouter);
  ...
  try {
    const { port } = server.address() as AddressInfo;
    const res = await fetch(`http://127.0.0.1:${port}/api/system/release/latest`);
    assert.equal(res.status, 200);
    assert.equal(res.headers.get('cache-control'), 'private, no-store');
    const body = (await res.json()) as Record<string, unknown>;
    assert.equal(body.success, true);
    assert.match(String(body.version), /^\d+\.\d+\.\d+\.\d+$/);
    assert.equal(Object.hasOwn(body, 'htmlUrl'), false);
    assert.equal(Object.hasOwn(body, 'repository'), false);
    assert.equal(JSON.stringify(body).includes('AlKindy-OSS/nassaj'), false);
  } finally {
    server.close();
    setReleaseDiscoveryForTests(null);
  }
});
