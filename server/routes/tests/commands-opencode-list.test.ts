 ...
 * calls projectsDb, and the DB connection is lazy, so the fixture database is untouched.
...
test('cursor: /list returns portable commands and no Claude-only actions', async () => {
  const { builtIn } = await listFor('cursor');
  const names = new Set(builtIn.map((c) => c.name));
  assert.ok(!names.has('/memory'));
  assert.ok(!names.has('/compact'));
  assert.deepEqual([...names], ['/help', '/models', '/cost', '/status']);
});

test('codex: /list exposes native /compact with a server handler', async () => {
  const { builtIn } = await listFor('codex');
  const compact = builtIn.find((command) => command.name === '/compact') as {
