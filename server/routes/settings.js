...
import { ApiKeyInputError } from '../modules/database/repositories/api-keys.js';
import { apiKeysDb, auditLogDb, credentialsDb, notificationPreferencesDb, pushSubscriptionsDb, appConfigDb, uiPreferencesDb } from '../modules/database/index.js';
...
// Optional link target for the node icon. Only absolute http(s) URLs are
// accepted — a relative path would be ambiguous next to the logo's own link,
// and any other scheme (javascript:, data:, …) is an XSS vector since the value
// ends up in an href rendered for every user of this server.
const BRANDING_NODE_ICON_HREF_KEY = 'branding.node_icon_href';
const BRANDING_NODE_ICON_HREF_MAX_LENGTH = 2048;
...
const BRANDING_MIME_TO_EXT = IMAGE_MIME_TO_EXT;

// The magic-byte check itself lives in services/image-signature.js — the
// per-project logo upload (projects/services/project-logo.service.ts) performs
// the identical validation, and a security decision with two copies drifts.
// Re-exported here because branding-logo.test.ts imports it from this module.
...
// Validate a candidate node-icon link. Returns the normalized URL string, or
// null when the value is empty (clear the link). Throws on anything that is not
// an absolute http/https URL within the length limit.
function normalizeNodeIconHref(raw) {
  const trimmed = String(raw).trim();
  if (trimmed.length === 0) {
    return null;
  }
  if (trimmed.length > BRANDING_NODE_ICON_HREF_MAX_LENGTH) {
    throw new Error(`Link must be at most ${BRANDING_NODE_ICON_HREF_MAX_LENGTH} characters`);
  }
  let parsed;
  try {
...
// Read the stored link, or null when unset/empty.
function getNodeIconHref() {
  const stored = appConfigDb.get(BRANDING_NODE_ICON_HREF_KEY);
...
router.put('/branding', requireRole('owner'), async (req, res) => {
  try {
    ...
    // Icon link: an absolute http(s) URL opened when the icon is clicked.
    // null or an empty/whitespace string clears it (icon becomes non-clickable).
    if ('nodeIconHref' in (req.body ?? {})) {
      const href = req.body.nodeIconHref;
      if (href === null || typeof href === 'string') {
        try {
          appConfigDb.set(BRANDING_NODE_ICON_HREF_KEY, href === null ? '' : (normalizeNodeIconHref(href) ?? ''));
        } catch (linkError) {
...
router.post('/branding/logo', requireRole('owner'), (req, res) => {
  return new Promise((resolve, reject) => { logoUpload(req, res, (uploadError) => { void (async () => {
    try {
      if (uploadError) {
...
// ===============================
// Programmatic access master switch (ADR-102 / T-1242)
// ===============================

// Read the current state. Available to every authenticated user — the flag is
// not a secret, and the client needs it to decide whether the API-keys section
// is meaningful. `canManage` tells the UI whether to render the control as
// editable; the PUT below is what actually enforces it.
router.get('/external-api', async (req, res) => {
  try {
    ...
    console.error('Error reading external API state:', error);
    res.status(500).json({ error: 'Failed to read programmatic access state' });
  }
});

// Flip the switch. Owner-only, and audited: this opens or closes a path that
// runs an agent with bypassPermissions, so "who turned it on and when" has to
// survive the session that did it.
router.put('/external-api', requireRole('owner'), async (req, res) => {
  try {
    ...
    setExternalApiEnabled(enabled);

    auditLogDb.record('external_api_toggled', {
    ...
    console.error('Error updating external API state:', error);
...
router.post('/api-keys', async (req, res) => {
  try {
    ...
    if (error instanceof ApiKeyInputError) {
      const status = error.code === 'limit_reached' ? 409 : 400;
