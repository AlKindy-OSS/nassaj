 ...
 * throwaway SQLite database (its own temp dir; the fixture database is never
...
test('ADR-089: a teammate who did not create the project may run its command (200)', async () => {
  // registeredDir is owned by ownerId and otherId is not a member. Before
  // ADR-089 that was a 403; project visibility is retired, so the ONLY thing
  // this gate still enforces is "is the path a registered project?" — which the
  // arbitrary-path test above proves is still enforced.
