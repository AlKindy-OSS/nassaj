...
let lastProfileInput: Record<string, unknown> | null = null;
...
test('/btw resume profile fails closed before SDK query on unknown/ambiguous/read failure', async () => {
  for (const code of ['ENGINE_PIN_UNKNOWN', 'ENGINE_PIN_AMBIGUOUS', 'ENGINE_PIN_READ_FAILED']) {
    ...
    const errors: Array<{ code: string; message: string }> = [];
    await sdk.spawnClaudeSideQuery(
      { sessionId: LIVE_SID, question: 'safe question', userId: 7, cwd: process.cwd() },
      { onError: (errorCode, message) => errors.push({ code: errorCode, message }) },
    );
    assert.equal(lastQueryArg, null, `${code}: SDK child was not constructed`);
    assert.equal(errors.length, 1);
    assert.equal(lastProfileInput?.requireKnownResumePin, true);
    assert.equal(lastProfileInput?.failOnAmbiguous, true);
  }
  profileFailure = null;
});
...
// B-1084: regression — thinking-only response produces empty finalAnswer → blank overlay
test('B-1084(a): thinking-only + empty result.result calls onError(empty_response) not onComplete', async () => {
  // Reproduce the exact live failure: the model uses extended thinking and emits only
  // a `thinking` content block with no accompanying `text` block. The SDK `result`
  // field is an empty string (no visible text produced). Before B-1084 the server
  // called onComplete('') → btw-complete without `text` → blank answer box.
  scriptedMessages = [
    {
      type: 'assistant',
      session_id: FORK_SID,
      message: { content: [{ type: 'thinking', thinking: 'Thinking about the question…' }] },
    },
    { type: 'result', session_id: FORK_SID, subtype: 'success', is_error: false, result: '' },
  ];

  let completedWith: string | undefined;
  let errorCode: string | undefined;
  await sdk.spawnClaudeSideQuery(
    { sessionId: LIVE_SID, question: 'طولت', userId: null, cwd: process.cwd() },
    {
      onChunk: () => {},
  ...
  assert.equal(completedWith, undefined, 'onComplete must NOT fire for an empty visible answer');
  assert.equal(errorCode, 'empty_response',
    'empty visible text after success should surface as empty_response error');
});

test('B-1084(b): stream ends without result message and no chunks → onError(empty_response)', async () => {
  // Edge: the for-await-of loop completes with no result message (SDK terminated early)
  // and no text was accumulated (only thinking blocks or no messages at all).
  // Before B-1084 this called onComplete('') → same blank-overlay symptom.
  scriptedMessages = [
    {
      type: 'assistant',
      session_id: FORK_SID,
      message: { content: [{ type: 'thinking', thinking: 'Internal only.' }] },
    },
    // Deliberately NO result frame — simulates a stream that terminates without one.
  ];

  let completedWith: string | undefined;
  let errorCode: string | undefined;
  await sdk.spawnClaudeSideQuery(
    { sessionId: LIVE_SID, question: 'ping', userId: null, cwd: process.cwd() },
    {
      onChunk: () => {},
      onError: (code) => { errorCode = code; },
      onComplete: (full) => { completedWith = full; },
  ...
  assert.equal(completedWith, undefined, 'onComplete must NOT fire when stream ended with no text');
  assert.equal(errorCode, 'empty_response',
    'no-text stream termination should surface as empty_response error');
});

// B-1286(btw): regression — forking a busy session emits a num_turns:0 notification
// cycle BEFORE the real answer; the side query must skip it and read the real turn.
test('B-1286(btw): num_turns:0 notification-cycle result is skipped; the real answer wins', async () => {
  // Exact field repro (CLI 2.1.273, real fork of a session with a live subagent):
  //   task_notification → init → result{success,num_turns:0,''} → init →
  //   assistant(thinking) → assistant(text) → result{success,num_turns:1,<text>}
  // Before the fix the FIRST result (num_turns:0, empty) hit the B-1084 empty path
  // and broke the loop → onError('empty_response') even for a trivial question.
  const ANSWER = 'وصلتني تجربتك، فالسؤال الجانبي يعمل الآن.';
  scriptedMessages = [
    { type: 'system', session_id: FORK_SID, subtype: 'task_notification' },
    { type: 'system', session_id: FORK_SID, subtype: 'init' },
    { type: 'result', session_id: FORK_SID, subtype: 'success', is_error: false, result: '', num_turns: 0 },
    { type: 'system', session_id: FORK_SID, subtype: 'init' },
    { type: 'assistant', session_id: FORK_SID, message: { content: [{ type: 'thinking', thinking: '…' }] } },
    { type: 'assistant', session_id: FORK_SID, message: { content: [{ type: 'text', text: ANSWER }] } },
    { type: 'result', session_id: FORK_SID, subtype: 'success', is_error: false, result: ANSWER, num_turns: 1 },
  ];
  ...
  const chunks: string[] = [];
  await sdk.spawnClaudeSideQuery(
    { sessionId: LIVE_SID, question: 'تجربة', userId: null, cwd: process.cwd() },
    {
      onChunk: (t) => { chunks.push(t); },
      onError: (code) => { errorCode = code; },
      onComplete: (full) => { completedWith = full; },
  ...
  assert.equal(errorCode, undefined, 'the notification cycle must NOT surface as an error');
  assert.equal(completedWith, ANSWER, 'onComplete carries the real answer from the second result');
  assert.deepEqual(chunks, [ANSWER], 'only the real assistant text streams as a chunk');
});
