 ...
 *
 * ── T-1212: كل سطر هنا قرار حوكمة بيانات، لا إعداد شبكة ──────────────────────
 *
 * إضافة نقطة نهاية هنا تعني أن **محتوى المستودع** — كوداً ومسارات وما تحمله
 * المطالبات — سيُشحن إلى ذلك المزوّد. والحارس الحديدي
 * (`anthropic-base-url-guard`) يفحص **المضيف** لا **تصنيف البيانات**، فلا شيء
 * أدنى من هذا السطر يسأل «هل يجوز لهذا المشروع أن يخرج؟».
 *
 * وقيدان مقصودان في القيمة نفسها:
 *
 *  • **الكيان**: ‏`api.z.ai` (كيان سنغافورة) وحده — لا `open.bigmodel.cn`.
 *    وهما ليسا اتفاقية واحدة ولا ولاية قضائية واحدة، فمفتاح مأخوذ من الثانية
 *    ينقل البيانات إلى نظام قانوني آخر بشروط أخرى. تثبيت المضيف هنا هو ما
 *    يجعل القيد بنيوياً لا تعليمة في وثيقة.
 *
 *  • **نوع الاعتماد**: مفتاح API عادي، لا اعتماد اشتراك ‏GLM Coding Plan.
 *    سياسة المزوّد تقصر الاشتراك على «أدواته المعتمدة»، ونسّاج ليس منها، وقد
 *    عوقب هارنس آخر على نمط طلبات مشابه. وهذا قيد تشغيلي (خطر حظر الحساب)
 *    قبل أن يكون قانونياً.
 *
 * وما لا تحكمه هذه القائمة ويبقى قرار مالك: أي المشاريع يجوز توجيهها أصلاً.
 * ‏`sessions.engine_provider` يسجّل ما جرى فعلاً، و`engine_pin_decision` في
 * سجلّ التدقيق يحفظ القرار — فالأثر موجود، والسياسة وحدها غائبة.
 */
 ...
 *
 * `deepseek` was absent here on purpose since T-864: widening the engine axis to
 * it was an owner decision, not a side effect of any repair. That decision was
 * made on 2026-08-03 (B-424): the key is stored and decrypts, the catalog is
 * reachable (200 after B-422), and the messages wire authenticates (402
 * Insufficient Balance — an ACCOUNT response, which only an authenticated
 * request can produce). DeepSeek now rides the same path as GLM: a first-class
 * ENGINE under the Claude body, never a standalone agent body (it stays in
 * DISABLED_PROVIDERS, because it has no tool harness of its own). Adding or
 * removing an id here is the single switch that makes an engine selectable end
 * to end.
 */
export const ELIGIBLE_ENGINE_PROVIDERS = Object.freeze(['kimi', 'glm', 'deepseek'] as const);
